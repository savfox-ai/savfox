# Savfox MCP Server Interface [experimental]

This document describes Savfox’s experimental MCP server interface: a JSON‑RPC API that runs over the Model Context Protocol (MCP) transport to control a local Savfox engine.

- Status: experimental and subject to change without notice
- Server binary: `savfox mcp-server` (or `savfox-mcp-server`)
- Transport: standard MCP over stdio (JSON‑RPC 2.0, line‑delimited)

## Overview

Savfox exposes a small set of MCP‑compatible methods to create and manage conversations, send user input, receive live events, and handle approval prompts. The types are defined in `protocol/src/mcp_protocol.rs` and re‑used by the MCP server implementation in `mcp-server/`.

At a glance:

- Conversations
  - `newConversation` → start a Savfox session
...
- Notifications (server → client)
  ...
  - `savfox/event` stream with agent events

See code for full type definitions and exact shapes: `protocol/src/mcp_protocol.rs`.

## Starting the server

Run Savfox as an MCP server and connect an MCP client:

```bash
savfox mcp-server | your_mcp_client
```

For a simple inspection UI, you can also try:

```bash
npx @modelcontextprotocol/inspector savfox mcp-server
```

Use the separate `savfox mcp` subcommand to manage configured MCP server launchers in `config.toml`.
...
- `model`: string model id (e.g. "o3", "gpt-5.1", "gpt-5.1-savfox")
...
Fetch the catalog of models available in the current Savfox build with `model/list`. The request accepts optional pagination inputs:
...
- `savfox/event` with the serialized Savfox event payload. The shape matches `core/src/protocol.rs`’s `Event` and `EventMsg` types. Some notifications include a `_meta.requestId` to correlate with the originating request.
...
The `savfox` and `savfox-reply` tools return standard MCP `CallToolResult` payloads. For
compatibility with MCP clients that prefer `structuredContent`, Savfox mirrors the
...
```json
{
  "content": [{ "type": "text", "text": "Hello from Savfox" }],
  "structuredContent": {
    "threadId": "019bbed6-1e9e-7f31-984c-a05b65045719",
    "content": "Hello from Savfox"
  }
}
```
...
When Savfox needs approval to apply changes or run commands, the server issues JSON‑RPC requests to the client:
...
```json
{ "jsonrpc": "2.0", "id": 2, "method": "sendUserMessage", "params": { "conversationId": "c7b0…", "items": [{ "type": "text", "text": "Hello Savfox" }] } }
```

While processing, the server emits `savfox/event` notifications containing agent output, approvals, and status updates.
