...
use savfox_core::protocol::{
    AgentMessageEvent, AgentReasoningRawContentEvent, AgentStatus, BackgroundEventEvent,
    ...
    CollabWaitingEndEvent, DeprecationNoticeEvent, ErrorEvent, Event, EventMsg,
    ExecCommandBeginEvent, ExecCommandEndEvent, FileChange, ItemCompletedEvent, McpInvocation,
    McpToolCallBeginEvent, McpToolCallEndEvent, PatchApplyBeginEvent, PatchApplyEndEvent,
    SessionConfiguredEvent, StreamErrorEvent, TurnAbortReason, TurnCompleteEvent, TurnDiffEvent,
    WarningEvent, WebSearchEndEvent,
};
...
pub(crate) struct EventProcessorWithHumanOutput {
    ...
    last_total_token_usage: Option<savfox_core::protocol::TokenUsageInfo>,
...
impl EventProcessor for EventProcessorWithHumanOutput {
    ...
    fn print_config_summary(
        ...
        ts_msg!(self, "Savfox  v{} (research preview)\n--------", VERSION);
    ...
    fn process_event(&mut self, event: Event) -> SavfoxStatus {
        ...
        match msg {
            ...
            EventMsg::TurnComplete(TurnCompleteEvent { last_agent_message }) => {
                ...
                return SavfoxStatus::InitiateShutdown;
            }
            ...
            EventMsg::AgentMessage(AgentMessageEvent { message }) => {
                ts_msg!(
                    self,
                    "{}\n{}",
                    "savfox".style(self.italic).style(self.magenta),
                    message,
                );
            }
            ...
            EventMsg::SessionConfigured(session_configured_event) => {
                ...
                ts_msg!(
                    self,
                    "{} {}",
                    "savfox session".style(self.magenta).style(self.bold),
            ...
            EventMsg::CollabAgentSpawnEnd(CollabAgentSpawnEndEvent {
                ...
                let success = new_session_id.is_some() && !is_collab_status_failure(&status);
                ...
                if let Some(new_session_id) = new_session_id {
                    eprintln!("  agent: {}", new_session_id.to_string().style(self.dimmed));
                }
            }
            EventMsg::CollabAgentInteractionBegin(CollabAgentInteractionBeginEvent {
                ...
                eprintln!(
                    "  receiver: {}",
                    receiver_session_id.to_string().style(self.dimmed)
                );
            }
            ...
            EventMsg::CollabWaitingBegin(CollabWaitingBeginEvent {
                ...
                eprintln!(
                    "  receivers: {}",
                    format_receiver_list(&receiver_session_ids).style(self.dimmed)
                );
            }
            EventMsg::CollabWaitingEnd(CollabWaitingEndEvent {
                ...
                let mut sorted = statuses
                    .into_iter()
                    .map(|(session_id, status)| (session_id.to_string(), status))
                    .collect::<Vec<_>>();
                sorted.sort_by(|(left, _), (right, _)| left.cmp(right));
                for (session_id, status) in sorted {
                    eprintln!(
                        "  {} {}",
                        session_id.style(self.dimmed),
            ...
            EventMsg::ShutdownComplete => return SavfoxStatus::Shutdown,
