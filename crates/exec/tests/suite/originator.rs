...
async fn send_savfox_exec_originator() -> anyhow::Result<()> {
    ...
    responses::mount_sse_once_match(&server, header("Originator", "savfox_exec"), body).await;

    test.cmd_with_server(&server)
        .env_remove(SAVFOX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
...
async fn supports_originator_override() -> anyhow::Result<()> {
    ...
    responses::mount_sse_once_match(&server, header("Originator", "savfox_exec_override"), body)
        .await;

    test.cmd_with_server(&server)
        .env(
            "SAVFOX_INTERNAL_ORIGINATOR_OVERRIDE",
            "savfox_exec_override",
        )
        .arg("--skip-git-repo-check")
        .arg("tell me something")
        .assert()
