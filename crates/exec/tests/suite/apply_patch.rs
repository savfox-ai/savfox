...
/// While we may add an `apply-patch` subcommand to the `savfox` CLI multitool
/// at some point, we must ensure that the smaller `savfox-exec` CLI can still
...
fn test_standalone_exec_cli_can_use_apply_patch() -> anyhow::Result<()> {
    ...
    Command::new(savfox_utils_cargo_bin::cargo_bin("savfox-exec")?)
        .arg(SAVFOX_APPLY_PATCH_ARG1)
        .arg(
            r#"*** Begin Patch
