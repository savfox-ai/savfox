...
pub enum Command {
    /// Submit a new Savfox Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Savfox Cloud task.
    Status(StatusCommand),
    /// List Savfox Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Savfox Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Savfox Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Savfox Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `savfox cloud` to browse).
    ...
    /// Git branch to run in Savfox Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Savfox Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Savfox Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Savfox Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
