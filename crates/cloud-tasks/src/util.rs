...
pub async fn build_chatgpt_headers() -> HeaderMap {
    ...
    set_user_agent_suffix("savfox_cloud_tasks_tui");
    let ua = savfox_core::default_client::get_savfox_user_agent();
    let mut headers = HeaderMap::new();
    headers.insert(
        USER_AGENT,
        HeaderValue::from_str(&ua).unwrap_or(HeaderValue::from_static("savfox-cli")),
    );
...
pub fn task_url(base_url: &str, task_id: &str) -> String {
    ...
    if let Some(root) = normalized.strip_suffix("/backend-api") {
        return format!("{root}/savfox/tasks/{task_id}");
    }
    if let Some(root) = normalized.strip_suffix("/api/savfox") {
        return format!("{root}/savfox/tasks/{task_id}");
    }
    if normalized.ends_with("/savfox") {
        return format!("{normalized}/tasks/{task_id}");
    }
    format!("{normalized}/savfox/tasks/{task_id}")
}
