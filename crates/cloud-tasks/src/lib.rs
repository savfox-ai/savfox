...
use util::{append_error_log, format_relative_time, set_user_agent_suffix};

struct ApplyJob {
    task_id: savfox_cloud_tasks_client::TaskId,
    diff_override: Option<String>,
}

struct BackendContext {
    backend: Arc<dyn savfox_cloud_tasks_client::CloudBackend>,
    base_url: String,
}

async fn init_backend(user_agent_suffix: &str) -> anyhow::Result<BackendContext> {
    let use_mock = matches!(
        std::env::var("SAVFOX_CLOUD_TASKS_MODE").ok().as_deref(),
        Some("mock") | Some("MOCK")
    );
    ...
    if use_mock {
        return Ok(BackendContext {
            backend: Arc::new(savfox_cloud_tasks_client::MockClient),
            base_url,
        });
    }

    let ua = savfox_core::default_client::get_savfox_user_agent();
    let mut http =
        savfox_cloud_tasks_client::HttpClient::new(base_url.clone())?.with_user_agent(ua);
    ...
    let auth = match auth {
        ...
        None => {
            eprintln!(
                "Not signed in. Please run 'savfox login' to sign in with ChatGPT, then re-run 'savfox cloud'."
            );
            std::process::exit(1);
        }
    };
...
impl GitInfoProvider for RealGitInfo {
    async fn default_branch_name(&self, path: &std::path::Path) -> Option<String> {
        savfox_core::git_info::default_branch_name(path).await
    }

    async fn current_branch_name(&self, path: &std::path::Path) -> Option<String> {
        savfox_core::git_info::current_branch_name(path).await
    }
}
...
async fn run_exec_command(args: crate::cli::ExecCommand) -> anyhow::Result<()> {
    ...
    let ctx = init_backend("savfox_cloud_tasks_exec").await?;
    ...
    let created = savfox_cloud_tasks_client::CloudBackend::create_task(
        &*ctx.backend,
        &env_id,
        &prompt,
        &git_ref,
...
async fn resolve_environment_id(ctx: &BackendContext, requested: &str) -> anyhow::Result<String> {
    ...
    match label_matches.as_slice() {
        [] => Err(anyhow!(
            "environment '{trimmed}' not found; run `savfox cloud` to list available environments"
        )),
        ...
        [first, rest @ ..] => {
            ...
            if rest.iter().all(|row| row.id == *first_id) {
                ...
                Err(anyhow!(
                    "environment label '{trimmed}' is ambiguous; run `savfox cloud` to pick the desired environment id"
                ))
            }
        }
    }
...
fn parse_task_id(raw: &str) -> anyhow::Result<savfox_cloud_tasks_client::TaskId> {
    ...
    Ok(savfox_cloud_tasks_client::TaskId(id.to_string()))
}
...
async fn collect_attempt_diffs(
    backend: &dyn savfox_cloud_tasks_client::CloudBackend,
    task_id: &savfox_cloud_tasks_client::TaskId,
) -> anyhow::Result<Vec<AttemptDiffData>> {
    let text =
        savfox_cloud_tasks_client::CloudBackend::get_task_text(backend, task_id.clone()).await?;
    let mut attempts = Vec::new();
    if let Some(diff) =
        savfox_cloud_tasks_client::CloudBackend::get_task_diff(backend, task_id.clone()).await?
    {
    ...
    if let Some(turn_id) = text.turn_id {
        let siblings = savfox_cloud_tasks_client::CloudBackend::list_sibling_attempts(
            backend,
            task_id.clone(),
            turn_id,
        )
...
fn summary_line(summary: &savfox_cloud_tasks_client::DiffSummary, colorize: bool) -> String {
...
fn format_task_status_lines(
    task: &savfox_cloud_tasks_client::TaskSummary,
...
fn format_task_list_lines(
    tasks: &[savfox_cloud_tasks_client::TaskSummary],
    base_url: &str,
    now: chrono::DateTime<Utc>,
    colorize: bool,
) -> Vec<String> {
...
async fn run_status_command(args: crate::cli::StatusCommand) -> anyhow::Result<()> {
    let ctx = init_backend("savfox_cloud_tasks_status").await?;
    let task_id = parse_task_id(&args.task_id)?;
    let summary =
        savfox_cloud_tasks_client::CloudBackend::get_task_summary(&*ctx.backend, task_id).await?;
    let now = Utc::now();
...
async fn run_list_command(args: crate::cli::ListCommand) -> anyhow::Result<()> {
    let ctx = init_backend("savfox_cloud_tasks_list").await?;
    let env_filter = if let Some(env) = args.environment {
        Some(resolve_environment_id(&ctx, &env).await?)
    } else {
        None
    };
    let page = savfox_cloud_tasks_client::CloudBackend::list_tasks(
        &*ctx.backend,
        env_filter.as_deref(),
        Some(args.limit),
    ...
    if let Some(cursor) = page.cursor {
        let command = format!("savfox cloud list --cursor='{cursor}'");
        if colorize {
            println!(
...
async fn run_diff_command(args: crate::cli::DiffCommand) -> anyhow::Result<()> {
    let ctx = init_backend("savfox_cloud_tasks_diff").await?;
    let task_id = parse_task_id(&args.task_id)?;
...
async fn run_apply_command(args: crate::cli::ApplyCommand) -> anyhow::Result<()> {
    let ctx = init_backend("savfox_cloud_tasks_apply").await?;
    let task_id = parse_task_id(&args.task_id)?;
    let attempts = collect_attempt_diffs(&*ctx.backend, &task_id).await?;
    let selected = select_attempt(&attempts, args.attempt)?;
    let outcome = savfox_cloud_tasks_client::CloudBackend::apply_task(
        &*ctx.backend,
        task_id,
    ...
    if !matches!(
        outcome.status,
        savfox_cloud_tasks_client::ApplyStatus::Success
    ) {
        std::process::exit(1);
    }
    Ok(())
...
fn level_from_status(status: savfox_cloud_tasks_client::ApplyStatus) -> app::ApplyResultLevel {
    match status {
        savfox_cloud_tasks_client::ApplyStatus::Success => app::ApplyResultLevel::Success,
        savfox_cloud_tasks_client::ApplyStatus::Partial => app::ApplyResultLevel::Partial,
        savfox_cloud_tasks_client::ApplyStatus::Error => app::ApplyResultLevel::Error,
    }
}
...
fn spawn_preflight(
    ...
    tokio::spawn(async move {
        ...
        let result = savfox_cloud_tasks_client::CloudBackend::apply_task_preflight(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
fn spawn_apply(
    ...
    tokio::spawn(async move {
        ...
        let result = savfox_cloud_tasks_client::CloudBackend::apply_task(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
/// Entry point for the `savfox cloud` subcommand.
pub async fn run_main(cli: Cli, _savfox_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    let BackendContext { backend, .. } = init_backend("savfox_cloud_tasks_tui").await?;
    let backend = backend;
    ...
    let force_internal = matches!(
        std::env::var("SAVFOX_CLOUD_TASKS_FORCE_INTERNAL")
            .ok()
            .as_deref(),
    ...
    append_error_log(format!(
        ...
        force_internal,
        savfox_core::default_client::get_savfox_user_agent()
    ));
            ...
            Some(()) = redraw_rx.recv() => {
                ...
                if let Some(page) = app.new_task.as_mut() {
                    ...
                    if page.composer.is_in_paste_burst() {
                        let _ = frame_tx.send(Instant::now() + savfox_tui::ComposerInput::recommended_flush_delay());
                    }
                }
                            ...
                            if let Some(ov) = app.diff_overlay.as_mut() {
                                ...
                                if let (Some(turn_id), true) = (turn_id.clone(), !sibling_turn_ids.is_empty())
                                    && ov.attempts.len() == 1 {
                                        ...
                                        tokio::spawn(async move {
                                            match savfox_cloud_tasks_client::CloudBackend::list_sibling_attempts(
                                                &*backend,
                                                task_id.clone(),
                                                turn_id,
                                            )
                        ...
                        app::AppEvent::ApplyFinished { id, result } => {
                            ...
                            match result {
                                Ok(outcome) => {
                                    ...
                                    if matches!(outcome.status, savfox_cloud_tasks_client::ApplyStatus::Success) {
                        ...
                        if let Some(page) = app.new_task.as_mut() {
                            ...
                            match key.code {
                                ...
                                _ => {
                                        ...
                                        // Ignore input while submitting
                                    } else if let savfox_tui::ComposerAction::Submitted(text) = page.composer.input(key) {
                                            ...
                                            if let Some(env) = page.env_id.clone() {
                                                ...
                                                tokio::spawn(async move {
                                                    ...
                                                    let result = savfox_cloud_tasks_client::CloudBackend::create_task(&*backend, &env, &text, &git_ref, false, best_of_n).await;
                        ...
                        if app.apply_modal.is_some() {
                            ...
                            match key.code {
                                ...
                                KeyCode::Enter => {
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        ...
                                        let title = task.title.clone();
                                        {
                                            ...
                                            tokio::spawn(async move {
                                                match savfox_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, diff_id.clone()).await {
                                                    Ok(Some(diff)) => {
                                                        let _ = tx.send(app::AppEvent::DetailsDiffLoaded { id: diff_id, title: diff_title, diff });
                                                    }
                                                    Ok(None) => {
                                                        match savfox_cloud_tasks_client::CloudBackend::get_task_text(&*backend, diff_id.clone()).await {
                                                            Ok(text) => {
                                        ...
                                        // Also fetch conversation text even when diff exists
                                        {
                                            ...
                                            let msg_title = title;
                                            tokio::spawn(async move {
                                                if let Ok(text) = savfox_cloud_tasks_client::CloudBackend::get_task_text(&*backend, msg_id.clone()).await {
                                                    let evt = app::AppEvent::DetailsMessagesLoaded {
                                                        id: msg_id,
                                                        title: msg_title,
                                                        messages: text.messages,
                                ...
                                KeyCode::Char('a') => {
                                    ...
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        match savfox_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, task.id.clone()).await {
                                            Ok(Some(diff)) => {
...
mod tests {
    ...
    async fn collect_attempt_diffs_includes_sibling_attempts() {
        ...
        let task_id = parse_task_id("https://taidge.com/savfox/tasks/T-1000").expect("id");
    ...
    fn parse_task_id_from_url_and_raw() {
        ...
        let url =
            parse_task_id("https://taidge.com/savfox/tasks/task_i_123456?foo=bar").expect("url id");
