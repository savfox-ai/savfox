...
/// Convert a Windows absolute path (`C:\foo\bar` or `C:/foo/bar`) to a WSL mount path
/// (`/mnt/c/foo/bar`). Returns `None` if the input does not look like a Windows drive path.
...
mod tests {
    ...
    fn win_to_wsl_basic() {
        assert_eq!(
            win_path_to_wsl(r"C:\Temp\savfox.zip").as_deref(),
            Some("/mnt/c/Temp/savfox.zip")
        );
        assert_eq!(
            win_path_to_wsl("D:/Work/savfox.tgz").as_deref(),
            Some("/mnt/d/Work/savfox.tgz")
        );
        assert!(win_path_to_wsl("/home/user/savfox").is_none());
    }
