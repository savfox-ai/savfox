...
use savfox_core::config::{Config, find_savfox_home, load_global_mcp_servers};
use savfox_core::mcp::auth::{McpOAuthLoginSupport, compute_auth_statuses, oauth_login_support};
use savfox_core::protocol::McpAuthStatus;
use savfox_rmcp_client::{delete_oauth_tokens, perform_oauth_login};

/// Subcommands:
/// - `list`   — list configured servers (with `--json`)
/// - `get`    — show a single server (with `--json`)
/// - `add`    — add a server launcher entry to `~/.savfox/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "savfox mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn run_add(add_args: AddArgs) -> Result<()> {
    ...
    let mut servers = load_global_mcp_servers(&savfox_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", savfox_home.display()))?;
    ...
    ConfigEditsBuilder::new(&savfox_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", savfox_home.display()))?;
    ...
    match oauth_login_support(&transport).await {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `savfox mcp login {name}` to login."
        ),
    }
...
async fn run_list(list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `savfox mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: savfox mcp remove {}", get_args.name);
