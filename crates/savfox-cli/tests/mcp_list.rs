...
fn savfox_command(savfox_home: &Path) -> Result<assert_cmd::Command> {
    let mut cmd = assert_cmd::Command::new(savfox_utils::cargo_bin::cargo_bin("savfox")?);
...
fn list_shows_empty_state() -> Result<()> {
    ...
    let mut cmd = savfox_command(savfox_home.path())?;
...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = savfox_command(savfox_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(savfox_home.path()).await?;
    let docs_entry = servers
        .get_mut("docs")
        .expect("docs server should exist after add");
    match &mut docs_entry.transport {
        McpServerTransportConfig::Stdio { env_vars, .. } => {
            *env_vars = vec!["APP_TOKEN".to_owned(), "WORKSPACE_ID".to_owned()];
        }
    ...
    let mut list_cmd = savfox_command(savfox_home.path())?;
    ...
    let mut list_json_cmd = savfox_command(savfox_home.path())?;
    ...
    let mut get_cmd = savfox_command(savfox_home.path())?;
    ...
    assert!(stdout.contains("remove: savfox mcp remove docs"));

    let mut get_json_cmd = savfox_command(savfox_home.path())?;
    get_json_cmd
