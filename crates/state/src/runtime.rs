...
use sqlx::{ConnectOptions, QueryBuilder, Row, Sqlite, SqlitePool};
use tracing::warn;

use crate::migrations::MIGRATOR;
use crate::model::{SessionRow, anchor_from_item, datetime_to_epoch_seconds};
use crate::paths::file_modified_time_utc;
use crate::{
    DB_ERROR_METRIC, LogEntry, LogQuery, LogRow, SessionMetadata, SessionMetadataBuilder,
    SessionsPage, SortKey, apply_rollout_item,
};

pub const STATE_DB_FILENAME: &str = "state.sqlite";

const METRIC_DB_INIT: &str = "savfox.db.init";
...
impl StateRuntime {
    /// Initialize the state runtime using the provided Savfox home and default provider.
    ///
    /// This opens (and migrates) the SQLite database at `savfox_home/state.sqlite`.
    pub async fn init(
        savfox_home: PathBuf,
        default_provider: String,
        ...
        let state_path = savfox_home.join(STATE_DB_FILENAME);
    ...
    /// Load session metadata by id using the underlying database.
    pub async fn get_session(
        &self,
        id: SessionId,
    ) -> anyhow::Result<Option<crate::SessionMetadata>> {
        let row = sqlx::query(
            r#"
SELECT
    id,
...
WHERE id = ?
        ...
        .await?;
        row.map(|row| SessionRow::try_from_row(&row).and_then(SessionMetadata::try_from))
            .transpose()
    }

    /// Get dynamic tools for a session, if present.
...
ORDER BY position ASC
    ...
    /// Find a rollout path by session id using the underlying database.
    pub async fn find_rollout_path_by_id(
        ...
        let mut builder =
            QueryBuilder::<Sqlite>::new("SELECT rollout_path FROM sessions WHERE id = ");
    ...
    /// List sessions using the underlying database.
    pub async fn list_sessions(
        ...
        archived_only: bool,
    ) -> anyhow::Result<crate::SessionsPage> {
...
FROM sessions
        ...
        push_session_order_and_limit(&mut builder, sort_key, limit);

        let rows = builder.build().fetch_all(self.pool.as_ref()).await?;
        let mut items = rows
            .into_iter()
            .map(|row| SessionRow::try_from_row(&row).and_then(SessionMetadata::try_from))
    ...
    pub async fn insert_logs(&self, entries: &[LogEntry]) -> anyhow::Result<()> {
        ...
        let mut builder = QueryBuilder::<Sqlite>::new(
            "INSERT INTO logs (ts, ts_nanos, level, target, message, session_id, module_path, file, line) ",
        );
    ...
    pub async fn query_logs(&self, query: &LogQuery) -> anyhow::Result<Vec<LogRow>> {
        let mut builder = QueryBuilder::<Sqlite>::new(
            "SELECT id, ts, ts_nanos, level, target, message, session_id, file, line FROM logs WHERE 1 = 1",
        );
    ...
    /// List session ids using the underlying database (no rollout scanning).
    pub async fn list_session_ids(
        ...
        let mut builder = QueryBuilder::<Sqlite>::new("SELECT id FROM sessions");
        push_session_filters(
            &mut builder,
            archived_only,
            allowed_sources,
        ...
        rows.into_iter()
            .map(|row| {
                ...
                Ok(SessionId::try_from(id)?)
            })
            .collect()
    }

    /// Insert or replace session metadata directly.
    pub async fn upsert_session(&self, metadata: &crate::SessionMetadata) -> anyhow::Result<()> {
        sqlx::query(
            r#"
...
ON CONFLICT(id) DO UPDATE SET
    ...
    /// Persist dynamic tools for a session if none have been stored yet.
    ///
    /// Dynamic tools are defined at session start and should not change afterward.
    /// This only writes the first time we see tools for a given session.
    pub async fn persist_dynamic_tools(
        ...
        let existing: Option<i64> =
            sqlx::query_scalar("SELECT 1 FROM session_dynamic_tools WHERE session_id = ? LIMIT 1")
...
INSERT INTO session_dynamic_tools (
    ...
    pub async fn apply_rollout_items(
        &self,
        builder: &SessionMetadataBuilder,
        items: &[RolloutItem],
        otel: Option<&OtelManager>,
    ) -> anyhow::Result<()> {
        if items.is_empty() {
        ...
        // Keep the session upsert before dynamic tools to satisfy the foreign key constraint:
        // session_dynamic_tools.session_id -> sessions.id.
        if let Err(err) = self.upsert_session(&metadata).await {
    ...
    /// Mark a session as archived using the underlying database.
    pub async fn mark_archived(
        ...
        let Some(mut metadata) = self.get_session(session_id).await? else {
            return Ok(());
        };
        ...
        if metadata.id != session_id {
            warn!(
                "session id mismatch during archive: expected {session_id}, got {}",
                metadata.id
            );
        }
        self.upsert_session(&metadata).await
    }

    /// Mark a session as unarchived using the underlying database.
    pub async fn mark_unarchived(
        ...
        if metadata.id != session_id {
            warn!(
                "session id mismatch during unarchive: expected {session_id}, got {}",
                metadata.id
            );
        }
        self.upsert_session(&metadata).await
...
fn push_log_filters<'a>(builder: &mut QueryBuilder<'a, Sqlite>, query: &'a LogQuery) {
    ...
    let has_session_filter = !query.session_ids.is_empty() || query.include_sessionless;
    if has_session_filter {
        ...
        if query.include_sessionless {
            if needs_or {
                builder.push(" OR ");
            }
            builder.push("session_id IS NULL");
        }
        builder.push(")");
    }
...
fn push_session_order_and_limit(
