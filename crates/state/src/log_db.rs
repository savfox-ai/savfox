...
//!
//! # async fn example(state_db: std::sync::Arc<savfox_state::StateRuntime>) {
...
use crate::{LogEntry, StateRuntime};
    ...
    S: tracing::Subscriber + for<'a> LookupSpan<'a>,
{
    ...
    fn on_record(
        ...
        if let Some(span) = ctx.span(id) {
            ...
            if let Some(log_context) = extensions.get_mut::<SpanLogContext>() {
                log_context.session_id = visitor.session_id;
            } else {
    ...
    fn on_event(&self, event: &Event<'_>, ctx: tracing_subscriber::layer::Context<'_, S>) {
        ...
        let session_id = visitor
            .session_id
            .clone()
            .or_else(|| event_session_id(event, &ctx));
...
impl SpanFieldVisitor {
    fn record_field(&mut self, field: &Field, value: String) {
        if field.name() == "session_id" && self.session_id.is_none() {
    ...
    if let Some(scope) = ctx.event_scope(event) {
        for span in scope.from_root() {
            ...
            if let Some(log_context) = extensions.get::<SpanLogContext>()
                && log_context.session_id.is_some()
            {
