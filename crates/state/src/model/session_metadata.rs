...
/// The sort key to use when listing sessions.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SortKey {
    /// Sort by the session's creation timestamp.
    CreatedAt,
    /// Sort by the session's last update timestamp.
    UpdatedAt,
}
...
/// A single page of session metadata results.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SessionsPage {
    /// The session metadata items in this page.
...
pub struct ExtractionOutcome {
    /// The extracted session metadata.
    pub metadata: SessionMetadata,
    /// The number of rollout lines that failed to parse.
    pub parse_errors: usize,
}

/// Canonical session metadata derived from rollout files.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SessionMetadata {
    ...
    pub tokens_used: i64,
    /// Whether the session has observed a user message.
...
/// Builder data required to construct [`SessionMetadata`] without parsing filenames.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl SessionMetadataBuilder {
    ...
    /// Build canonical session metadata, filling missing values from defaults.
    pub fn build(&self, default_provider: &str) -> SessionMetadata {
...
pub(crate) fn anchor_from_item(item: &SessionMetadata, sort_key: SortKey) -> Option<Anchor> {
