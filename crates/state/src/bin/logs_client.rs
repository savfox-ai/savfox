...
#[command(about = "Tail Savfox logs from state.sqlite with simple filters")]
struct Args {
    /// Path to SAVFOX_HOME. Defaults to $SAVFOX_HOME or ~/.savfox.
    #[arg(long, env = "SAVFOX_HOME")]
    savfox_home: Option<PathBuf>,

    /// Direct path to the SQLite database. Overrides --savfox-home.
    #[arg(long)]
    db: Option<PathBuf>,
    ...
    /// Match one or more session ids. Repeat to include multiple sessions.
    #[arg(long = "session-id")]
    session_id: Vec<String>,

    /// Include logs that do not have a session id.
    #[arg(long)]
    sessionless: bool,
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let savfox_home = args.savfox_home.clone().unwrap_or_else(default_savfox_home);
    Ok(savfox_home.join(STATE_DB_FILENAME))
}

fn default_savfox_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".savfox");
    }
    PathBuf::from(".savfox")
}
...
fn build_filter(args: &Args) -> anyhow::Result<LogFilter> {
    ...
    Ok(LogFilter {
        ...
        file_like,
        session_ids,
        include_sessionless: args.sessionless,
    })
}
...
fn to_log_query(
    ...
    LogQuery {
        ...
        session_ids: filter.session_ids.clone(),
        include_sessionless: filter.include_sessionless,
        after_id,
        limit,
        descending,
    }
...
fn format_row(row: &LogRow) -> String {
    ...
    let session_id_colored = session_id.blue().dimmed().to_string();
    let target_colored = target.dimmed().to_string();
    let message_colored = heuristic_formatting(message);
    format!(
        "{timestamp_colored} {level_colored} [{session_id_colored}] {target_colored} - {message_colored}"
    )
}
