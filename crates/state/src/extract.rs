...
use savfox_protocol::protocol::{
    EventMsg, RolloutItem, SessionMetaLine, TurnContextItem, USER_MESSAGE_BEGIN,
};
use serde::Serialize;
...
fn apply_session_meta_from_item(metadata: &mut SessionMetadata, meta_line: &SessionMetaLine) {
    if metadata.id != meta_line.meta.id {
        // Ignore session_meta lines that don't match the canonical session ID,
...
fn apply_turn_context(metadata: &mut SessionMetadata, turn_ctx: &TurnContextItem) {
...
fn apply_event_msg(metadata: &mut SessionMetadata, event: &EventMsg) {
    match event {
...
fn apply_response_item(metadata: &mut SessionMetadata, item: &ResponseItem) {
...
mod tests {
    ...
    fn diff_fields_detects_changes() {
        let id = SessionId::from_string(&Uuid::now_v7().to_string()).expect("session id");
