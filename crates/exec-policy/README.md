...
- From the Savfox CLI, run `savfox execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
savfox execpolicy check --rules path/to/policy.rules git status
```
...
```bash
cargo run -p savfox-exec-policy -- check --rules path/to/policy.rules git status
```
