...
/// * `indices` – Optional list of character indices that matched the query. These are only filled
///   when the caller of [`run`] sets `options.compute_indices` to `true`. The indices vector
///   follows the guidance from `nucleo::pattern::Pattern::indices`: they are unique and sorted in
...
pub trait SessionReporter: Send + Sync + 'static {
    ...
    /// Called when the session becomes idle or is cancelled. Guaranteed to be called at least once
    /// per update_query.
    fn on_complete(&self);
}
...
/// The worker sessions will periodically check `cancel_flag` to see if they
