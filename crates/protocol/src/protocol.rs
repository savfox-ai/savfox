...
pub use crate::approvals::{
    ApplyPatchApprovalRequestEvent, ElicitationAction, ExecApprovalRequestEvent,
    ExecPolicyAmendment,
};
use crate::config_types::{
    CollaborationMode, ModeKind, Personality, ReasoningSummary as ReasoningSummaryConfig,
    WindowsSandboxLevel,
};
...
use crate::mcp::{
    CallToolResult, RequestId, Resource as McpResource, ResourceTemplate as McpResourceTemplate,
    Tool as McpTool,
};
use crate::message_history::HistoryEntry;
use crate::models::{BaseInstructions, ContentItem, ResponseItem, WebSearchAction};
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Savfox:";
...
pub enum Op {
    ...
    /// for a turn of a [`crate::savfox_session::SavfoxSession`].
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,
    ...
    OverrideTurnContext {
        ...
        /// Unified permission policy override.
        /// When set, overrides `approval_policy`, `sandbox_policy`, and also
        /// sets the tool access policy for subsequent turns.
        #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// The agent will use its existing context (either conversation history or previous response
    /// id) to generate a summary which will be returned as an AgentMessage event.
    Compact,

    /// Set a user-facing session name in the persisted rollout metadata.
    /// This is a local-only operation handled by savfox-core; it does not
    /// involve the model.
    SetSessionName { name: String },

    /// Request Savfox to undo a turn (turn are stacked so it is the same effect as CMD + Z).
    Undo,

    /// Request Savfox to drop the last N user turns from in-memory context.
    ///
    /// This does not attempt to revert local filesystem changes. Clients are
    /// responsible for undoing any edits on disk.
    SessionRollback { num_turns: u32 },

    /// Request a code review from the agent.
    Review { review_request: ReviewRequest },

    /// Request to shut down savfox instance.
    Shutdown,
...
/// running the command proposed by Savfox.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.savfox`, `.git`, notably `.git/hooks`) under
            ...
            Self::WorkspaceWrite {
                ...
                roots
                    ...
                    .map(|writable_root| {
                        ...
                        #[allow(clippy::expect_used)]
                        let top_level_savfox = writable_root
                            .join(".savfox")
                            .expect(".savfox is a valid relative path");
                        if top_level_savfox.as_path().is_dir() {
                            subpaths.push(top_level_savfox);
                        }
                        WritableRoot {
                            root: writable_root,
...
/// Unified permission policy that controls sandbox, approval, and tool access.
///
/// This is the single source of truth for what an agent/session is allowed to do.
/// Both the TUI (3 coarse presets) and the Gateway agent page (detailed presets)
/// map to this type.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, JsonSchema, TS)]
#[serde(rename_all = "snake_case")]
pub struct PermissionPolicy {
    /// Controls the filesystem and network sandbox.
    pub sandbox: SandboxPolicy,

    /// Controls when the user is prompted for approval.
...
///
/// Uses canonical short tool names (see [`tool_names`] module):
/// `"shell"`, `"read_file"`, `"write_file"`, `"list_dir"`, `"grep_files"`,
/// `"browser"`, `"web_fetch"`, `"web_search"`, `"md_memory"`, `"image_generate"`.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, JsonSchema, TS)]
#[serde(rename_all = "snake_case")]
pub struct ToolAccessPolicy {
    /// Explicit set of allowed tool identifiers.
    /// Empty means "all tools are allowed" (subject to sandbox constraints).
    #[serde(default)]
    pub allowed: Vec<String>,

    /// Explicit set of denied tool identifiers (takes precedence over `allowed`).
    #[serde(default)]
    pub denied: Vec<String>,

    /// Per-tool approval overrides.
    /// Key: canonical tool name. Value: `true` = requires approval, `false` = auto-approve.
    #[serde(default, skip_serializing_if = "HashMap::is_empty")]
    pub tool_approval_overrides: HashMap<String, bool>,
}
...
impl ToolAccessPolicy {
    ...
    pub fn is_tool_allowed(&self, tool_name: &str) -> bool {
        ...
        // If the allowed list is empty, everything (not denied) is allowed.
...
impl PermissionPolicy {
    /// Validate that the permission policy is internally consistent.
    ///
    /// Returns `Ok(())` when valid, or `Err(errors)` with a list of issues.
    pub fn validate(&self) -> Result<(), Vec<String>> {
        let mut errors = Vec::new();

        if let Some(ref tool_access) = self.tool_access {
            // Cannot allow write/shell tools with ReadOnly sandbox.
            if matches!(self.sandbox, SandboxPolicy::ReadOnly) {
                for tool in &tool_access.allowed {
                    match tool.as_str() {
                        "shell" | "write_file" => {
                            errors.push(format!(
...
/// NOTE: Make sure none of these values have optional types, as it will mess up the extension
/// code-gen.
...
pub enum EventMsg {
    ...
    SessionRolledBack(SessionRolledBackEvent),
    ...
    SessionNameUpdated(SessionNameUpdatedEvent),
...
fn collect_events_only(items: &[RolloutItem]) -> Vec<EventMsg> {
    items
        .iter()
...
fn collect_resume_initial_events(items: &[RolloutItem]) -> Vec<EventMsg> {
    let mut merged: Vec<(usize, EventMsg)> = items
        .iter()
        .enumerate()
        .filter_map(|(index, item)| match item {
            RolloutItem::EventMsg(ev) => Some((index, ev.clone())),
            _ => None,
        })
        .collect();

    let has_user_event = merged
        .iter()
        .any(|(_, ev)| matches!(ev, EventMsg::UserMessage(_)));
    let has_assistant_event = merged
        .iter()
        .any(|(_, ev)| matches!(ev, EventMsg::AgentMessage(_)));

    if has_user_event && has_assistant_event {
        return merged.into_iter().map(|(_, ev)| ev).collect();
    }
    ...
    for (index, item) in items.iter().enumerate() {
        ...
        let should_fallback =
            (role == "user" && !has_user_event) || (role == "assistant" && !has_assistant_event);
        if !should_fallback {
            continue;
        }
        let message = response_message_text(role.as_str(), content.as_slice());
        if message.is_empty() {
            continue;
        }
        ...
        merged.push((index, event));
    }

    merged.sort_by_key(|(index, _)| *index);
    merged.into_iter().map(|(_, ev)| ev).collect()
}

fn response_message_text(role: &str, content: &[ContentItem]) -> String {
    let mut parts = Vec::new();
    for item in content {
        match (role, item) {
            ("assistant", ContentItem::OutputText { text }) => parts.push(text.as_str()),
            ("assistant", _) => {}
            (_, ContentItem::InputText { text } | ContentItem::OutputText { text }) => {
                parts.push(text.as_str())
            }
            (_, ContentItem::InputImage { .. }) => {}
        }
    }
    parts.join("\n")
}
...
pub struct SessionMeta {
    pub id: SessionId,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub forked_from_id: Option<SessionId>,
    ...
    /// base_instructions for the session. This *should* always be present when creating a new
    /// session, but may be missing for older sessions. If not present, fall back to rendering
    /// the base_instructions from ModelsManager.
...
#[derive(Debug, Clone, Deserialize, Serialize, JsonSchema, TS)]
pub struct SessionRolledBackEvent {
...
#[derive(Debug, Clone, Deserialize, Serialize, JsonSchema, TS)]
pub struct SessionNameUpdatedEvent {
    pub session_id: SessionId,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    #[ts(optional)]
    pub session_name: Option<String>,
...
pub struct CollabWaitingBeginEvent {
    ...
    pub receiver_session_ids: Vec<SessionId>,
...
pub struct CollabWaitingEndEvent {
    ...
    pub statuses: HashMap<SessionId, AgentStatus>,
}
...
mod tests {
    ...
    #[test]
    fn resumed_initial_messages_fallback_to_response_items_when_roles_missing() {
        ...
        let history = InitialHistory::Resumed(ResumedHistory {
            ...
            rollout_path: PathBuf::from("resume.jsonl"),
        });

        let initial = history
            .get_event_msgs()
            .expect("resumed history should expose initial messages");
        match initial.as_slice() {
            [
                EventMsg::UserMessage(user),
                EventMsg::AgentMessage(assistant),
            ] => {
                assert_eq!(user.message, "legacy user");
                assert_eq!(assistant.message, "legacy assistant");
            }
    ...
    #[test]
    fn resumed_initial_messages_preserve_existing_user_events_without_duplicates() {
        let conversation_id = SessionId::new();
        let history = InitialHistory::Resumed(ResumedHistory {
            conversation_id,
            history: vec![
        ...
        match initial.as_slice() {
            [
                ...
                assert_eq!(user.message, "from event");
                assert_eq!(assistant.message, "assistant fallback");
            }
            other => panic!("unexpected merged initial messages: {other:#?}"),
        }
    }

    #[test]
    fn forked_initial_messages_remain_event_only() {
        let history =
        ...
        let initial = history
            .get_event_msgs()
            .expect("forked history should always return an event vector");
        assert!(
            initial.is_empty(),
            "forked history should not synthesize initial messages from response items",
        );
    }
}
