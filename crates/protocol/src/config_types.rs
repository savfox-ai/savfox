...
/// Collaboration mode for a Savfox session.
...
impl CollaborationMode {
    ...
    /// - `effort`: `Some(Some(e))` to set effort to `e`, `Some(None)` to clear effort, `None` to
    ///   keep current effort
    /// - `developer_instructions`: `Some(Some(s))` to set instructions, `Some(None)` to clear them,
