...
use savfox_utils::git::GhostCommit;
use savfox_utils::image::error::ImageProcessingError;
use savfox_utils::image::load_and_resize_to_fit;
use schemars::JsonSchema;
use serde::ser::Serializer;
use serde::{Deserialize, Deserializer, Serialize};
use ts_rs::TS;

use crate::config_types::{CollaborationMode, SandboxMode};
use crate::mcp::CallToolResult;
use crate::protocol::{
    AskForApproval, COLLABORATION_MODE_CLOSE_TAG, COLLABORATION_MODE_OPEN_TAG, NetworkAccess,
    SandboxPolicy, WritableRoot,
};
...
pub enum ResponseItem {
    ...
    // OpenAI /v1/responses endpoint is NOT the same shape as the objects the server returns on
    // the SSE stream. When *sending* we must wrap the string output inside an object that
    // includes a required `success` boolean. To ensure we serialize exactly the expected
    // shape we introduce a dedicated payload struct and flatten it here.
    FunctionCallOutput {
        call_id: String,
        output: FunctionCallOutputPayload,
    },
...
/// Base instructions for the model in a session. Corresponds to the `instructions` field in the
/// ResponsesAPI.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, JsonSchema, TS)]
...
fn local_image_error_placeholder(
    ...
    ContentItem::InputText {
        text: format!(
            "Savfox could not read the local image at `{}`: {}",
            path.display(),
            error
        ),
    }
...
fn unsupported_image_error_placeholder(path: &std::path::Path, mime: &str) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "Savfox cannot attach image at `{}`: unsupported image format `{}`.",
            path.display(),
            mime
        ),
    }
...
// (Moved event mapping logic into savfox-core to avoid coupling protocol to UI-facing events.)
...
<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"></svg>"#,
        ...
        match item {
            ResponseInputItem::Message { content, .. } => {
                assert_eq!(content.len(), 1);
                let expected = format!(
                    "Savfox cannot attach image at `{}`: unsupported image format `image/svg+xml`.",
                    svg_path.display()
                );
                match &content[0] {
