...
pub(crate) async fn save_tts_config(savfox_home: &Path, cfg: &TtsConfig) -> Result<(), String> {
    ...
    if let Some(parent) = path.parent() {
        tokio::fs::create_dir_all(parent)
            .await
            .map_err(|err| format!("failed to create tts config dir: {err}"))?;
    }
    let data = serde_json::to_string_pretty(cfg)
        .map_err(|err| format!("failed to serialize tts config: {err}"))?;
    tokio::fs::write(path, data)
        .await
        .map_err(|err| format!("failed to write tts config: {err}"))
}
...
pub(crate) async fn convert(
    ...
    let file_name = format!("tts-{}-{}.{}", now_epoch_ms(), uuid::Uuid::now_v7(), ext);
