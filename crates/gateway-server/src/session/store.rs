...
impl SessionStore {
    ...
    pub async fn upsert(&self, entry: SessionEntry) {
        ...
        // Note: Session metadata is persisted within the rollout file itself,
        // so we don't need to persist separately here.
        // The rollout file is created/updated by the RolloutRecorder.
    }
    ...
    pub async fn replace_all(&self, entries: HashMap<String, SessionEntry>) -> std::io::Result<()> {
        ...
        // Note: We're not persisting to sessions.json anymore.
        // The rollout files contain the session metadata.
        Ok(())
    }
