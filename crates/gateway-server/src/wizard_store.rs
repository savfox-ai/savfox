...
async fn load_state(savfox_home: &Path) -> Result<Option<WizardState>, String> {
    json_store::load_json_opt(&store_path(savfox_home), "wizard state").await
}

async fn save_state(savfox_home: &Path, state: &WizardState) -> Result<(), String> {
    json_store::save_json(&store_path(savfox_home), state, "wizard state").await
}
...
pub(crate) async fn next(
    ...
    state.updated_at_ms = json_store::now_ms();
