...
use super::{Channel, RichMessage, render_error, runtime};
...
impl Channel for IMessageChannel {
    ...
    async fn send_message(&self, channel: &str, message: &str) -> anyhow::Result<()> {
        ...
        check_response(response, "BlueBubbles API error").await?;
...
pub(crate) async fn webhook_handler(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    let Some(body) = super::parse_json_body(req, res, "imessage").await else {
        return;
    };
