...
async fn load_store(path: &Path) -> ApprovalStore {
    let content = match tokio::fs::read_to_string(path).await {
        Ok(content) => content,
        Err(_) => return ApprovalStore::default(),
    };
    serde_json::from_str::<ApprovalStore>(&content).unwrap_or_default()
}
...
async fn save_store(path: &Path, store: &ApprovalStore) -> Result<(), String> {
    if let Some(parent) = path.parent() {
        tokio::fs::create_dir_all(parent)
            .await
            .map_err(|err| format!("failed to create approvals directory: {err}"))?;
    }
    let content = serde_json::to_string_pretty(store)
        .map_err(|err| format!("failed to serialize approvals store: {err}"))?;
    tokio::fs::write(path, content)
        .await
        .map_err(|err| format!("failed to write approvals store: {err}"))
}
