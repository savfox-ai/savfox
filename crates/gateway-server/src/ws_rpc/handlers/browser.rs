...
async fn save_browser_profiles_config(
    ...
    let path = browser_profiles_path(channel);
    crate::json_store::ensure_parent_dir(&path)
        .await
...
fn browser_timeout_ms(params: &Value) -> u64 {
    opt_u64(params, "timeout_ms", DEFAULT_BROWSER_TIMEOUT_MS).max(1)
}
...
fn selected_profile_name(
    ...
    let raw = opt_str(params, "profile", cfg.default_profile.as_str());
...
pub(crate) async fn handle_browser_request(
    ...
    let use_browser_context = opt_bool(params, "use_browser_context", true);
    let fallback_direct = opt_bool(params, "fallback_direct", true);
...
pub(crate) async fn handle_browser_request_direct(params: &Value) -> RpcResult {
...
pub(crate) async fn handle_wizard_start(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let wizard_type = opt_str(params, "type", "setup");
    wizard_store::start(&channel.config().savfox_home, wizard_type)
        .await
        .map_err(|err| (INTERNAL_ERROR, err))
}

pub(crate) async fn handle_wizard_next(params: &Value, channel: &Arc<GatewayChannel>) -> RpcResult {
    let wizard_id = require_str(params, "wizard_id")?;
...
pub(crate) async fn handle_wizard_cancel(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
...
pub(crate) async fn handle_wizard_status(channel: &Arc<GatewayChannel>) -> RpcResult {
...
pub(crate) async fn handle_talk_mode(params: &Value, channel: &Arc<GatewayChannel>) -> RpcResult {
    let mode = opt_str(params, "mode", "text");
    voice_store::set_talk_mode(&channel.config().savfox_home, mode)
        .await
        .map_err(|err| (INTERNAL_ERROR, err))
}

pub(crate) async fn handle_voicewake_get(channel: &Arc<GatewayChannel>) -> RpcResult {
    voice_store::get_voicewake(&channel.config().savfox_home)
        .await
        .map_err(|err| (INTERNAL_ERROR, err))
}

pub(crate) async fn handle_voicewake_set(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let enabled = opt_bool(params, "enabled", false);
    let keyword = opt_str(params, "keyword", "hey savfox");
    voice_store::set_voicewake(&channel.config().savfox_home, enabled, keyword)
        .await
        .map_err(|err| (INTERNAL_ERROR, err))
}

pub(crate) async fn handle_update_run(_channel: &Arc<GatewayChannel>) -> RpcResult {
    Ok(json!({
        "status": "checking",
...
pub(crate) async fn handle_memory_list(params: &Value, channel: &Arc<GatewayChannel>) -> RpcResult {
    let home = memory_home(channel);
    let pr = memory_project_root();
    let layer_filter = params.get("layer").and_then(|v| v.as_str());
    let include_content = opt_bool(params, "include_content", false);
...
pub(crate) async fn handle_memory_get(params: &Value, channel: &Arc<GatewayChannel>) -> RpcResult {
    let slug = require_str(params, "slug")?;
    let layer_filter = params.get("layer").and_then(|v| v.as_str());
...
pub(crate) async fn handle_memory_create(
    ...
    let layer_str = require_str(params, "layer")?;
    let layer: savfox_core::md_memory::MemoryLayer = layer_str
        .parse()
        .map_err(|e: String| (INVALID_REQUEST, e))?;
    ...
    let slug_raw = require_str(params, "slug")?;
    ...
    let tags: Vec<String> = params
        ...
        .unwrap_or_default();
    let priority = opt_u64(params, "priority", 5) as u32;
...
pub(crate) async fn handle_memory_update(
    ...
    let new_body = opt_str(params, "content", &old_body);
...
pub(crate) async fn handle_memory_search(
    ...
    let limit = opt_u64(params, "limit", 10) as usize;
    let layer_filter = params.get("layer").and_then(|v| v.as_str());
...
pub(crate) async fn handle_memory_promote(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let slug = require_str(params, "slug")?;
    let from_str = require_str(params, "from_layer")?;
    let to_str = require_str(params, "to_layer")?;
...
pub(crate) async fn handle_memory_layers(channel: &Arc<GatewayChannel>) -> RpcResult {
    let home = memory_home(channel);
    let pr = memory_project_root();
    let dirs = savfox_core::md_memory::layer_dirs(&home, pr.as_deref(), "default");
...
pub(crate) async fn handle_webhooks_list(channel: &GatewayChannel) -> RpcResult {
...
pub(crate) async fn handle_webhooks_create(params: &Value, channel: &GatewayChannel) -> RpcResult {
...
pub(crate) async fn handle_skills_registry_search(
    params: &Value,
    channel: &GatewayChannel,
) -> RpcResult {
...
pub(crate) async fn handle_dm_policy_get(params: &Value, channel: &GatewayChannel) -> RpcResult {
...
pub(crate) async fn handle_dm_policy_set(params: &Value, channel: &GatewayChannel) -> RpcResult {
    let channel_id = params["channel"].as_str().unwrap_or("default");
    let store = crate::dm_policy::DmPolicyStore::new(&channel.config().savfox_home);
    store.load().await;
...
pub(crate) async fn handle_dm_allowlist_get(params: &Value, channel: &GatewayChannel) -> RpcResult {
    let channel_id = params["channel"].as_str().unwrap_or("default");
    let store = crate::dm_policy::DmPolicyStore::new(&channel.config().savfox_home);
    store.load().await;
    let policy = store.get_policy(channel_id).await;
...
pub(crate) async fn handle_dm_allowlist_set(params: &Value, channel: &GatewayChannel) -> RpcResult {
    let channel_id = params["channel"].as_str().unwrap_or("default");
...
pub(crate) async fn handle_config_validate(params: &Value, _channel: &GatewayChannel) -> RpcResult {
...
pub(crate) async fn handle_stt_transcribe(params: &Value, channel: &GatewayChannel) -> RpcResult {
    let result =
        crate::stt::transcribe(&channel.config().savfox_home, channel.http_client(), params)
            .await
            .map_err(|e| (INTERNAL_ERROR, e))?;
    Ok(result)
}

pub(crate) async fn handle_stt_providers() -> RpcResult {
...
pub(crate) async fn handle_canvas_render(params: &Value) -> RpcResult {
...
pub(crate) async fn handle_canvas_action(params: &Value) -> RpcResult {
...
pub(crate) async fn handle_canvas_state(params: &Value) -> RpcResult {
    let surface_id = params["surface_id"].as_str().unwrap_or("main");
...
pub(crate) async fn handle_canvas_close(params: &Value) -> RpcResult {
    let surface_id = params["surface_id"].as_str().unwrap_or("main");
...
pub(crate) async fn handle_plugins_list(channel: &GatewayChannel) -> RpcResult {
...
pub(crate) async fn handle_config_restore(params: &Value, channel: &GatewayChannel) -> RpcResult {
    let snapshot = params
        .get("snapshot")
        .and_then(|v| v.as_str())
        .unwrap_or("");
...
pub(crate) async fn handle_models_aliases_get(channel: &GatewayChannel) -> RpcResult {
    let aliases = load_model_aliases(channel).await;
    Ok(json!({ "aliases": aliases }))
}

pub(crate) async fn handle_models_aliases_set(
    params: &Value,
    channel: &GatewayChannel,
) -> RpcResult {
...
pub(crate) async fn handle_sessions_elevate(
    params: &Value,
...
pub(crate) async fn handle_sessions_unelevate(
    params: &Value,
    session_store: &SessionStore,
) -> RpcResult {
    let session_id = params
...
pub(crate) async fn handle_heartbeat_config_get(channel: &GatewayChannel) -> RpcResult {
    let path = heartbeat_config_path(channel);
    let config = crate::json_store::load_json_value(&path).await;
    Ok(json!({ "config": config }))
}

pub(crate) async fn handle_heartbeat_config_set(
    params: &Value,
    channel: &GatewayChannel,
) -> RpcResult {
...
pub(crate) async fn handle_browser_start(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let timeout_ms = browser_timeout_ms(params);
...
pub(crate) async fn handle_browser_stop(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let cfg = load_browser_profiles_config(channel).await;
...
pub(crate) async fn handle_browser_tabs_list(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let timeout_ms = browser_timeout_ms(params);
...
pub(crate) async fn handle_browser_tabs_open(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let timeout_ms = browser_timeout_ms(params);
...
pub(crate) async fn handle_browser_tabs_switch(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let timeout_ms = browser_timeout_ms(params);
...
pub(crate) async fn handle_browser_tabs_close(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let timeout_ms = browser_timeout_ms(params);
...
pub(crate) async fn handle_browser_snapshot(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let timeout_ms = browser_timeout_ms(params);
    ...
    ensure_browser_session_for_profile(channel, &profile, &settings).await?;
    let page = select_browser_page(&profile, timeout_ms, requested_target_id(params)).await?;
    let mode = opt_str(params, "mode", "dom");
...
pub(crate) async fn handle_browser_storage_get(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let timeout_ms = browser_timeout_ms(params);
    ...
    ensure_browser_session_for_profile(channel, &profile, &settings).await?;
    let page = select_browser_page(&profile, timeout_ms, requested_target_id(params)).await?;
    let scope = opt_str(params, "scope", "localStorage");
...
pub(crate) async fn handle_browser_storage_set(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let timeout_ms = browser_timeout_ms(params);
...
pub(crate) async fn handle_browser_storage_clear(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let timeout_ms = browser_timeout_ms(params);
...
pub(crate) async fn handle_browser_network_capture(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let timeout_ms = browser_timeout_ms(params);
    let duration_ms = opt_u64(params, "duration_ms", 2_000).max(1);
    let max_events = opt_u64(params, "max_events", 128).max(1) as usize;
    let (profile, settings) = resolve_browser_profile(params, channel).await?;
    ensure_browser_session_for_profile(channel, &profile, &settings).await?;
    let page = select_browser_page(&profile, timeout_ms, requested_target_id(params)).await?;
    let target_id = page.target_id().to_owned();
...
pub(crate) async fn handle_browser_profiles_list(channel: &Arc<GatewayChannel>) -> RpcResult {
    let cfg = load_browser_profiles_config(channel).await;
...
pub(crate) async fn handle_browser_profiles_create(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    ...
    if opt_bool(params, "default", false) || cfg.default_profile.trim().is_empty() {
...
pub(crate) async fn handle_browser_profiles_delete(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let requested = params
    ...
    if opt_bool(params, "delete_data", false) {
...
pub(crate) async fn handle_browser_profiles_default_set(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let requested = params
...
pub(crate) async fn handle_browser_goto(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let url = params.get("url").and_then(|v| v.as_str()).unwrap_or("");
    ...
    let open_new_tab = opt_bool(params, "open_new_tab", false);
...
pub(crate) async fn handle_browser_type(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let selector = params
...
pub(crate) async fn handle_browser_screenshot(
    ...
    let format = opt_str(params, "format", "png").to_ascii_lowercase();
...
pub(crate) async fn handle_browser_eval(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
...
pub(crate) async fn handle_browser_extension_relay_start(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let timeout_ms = browser_timeout_ms(params);
...
pub(crate) async fn handle_browser_extension_relay_status(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let timeout_ms = browser_timeout_ms(params);
...
pub(crate) async fn handle_browser_extension_relay_stop(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let timeout_ms = browser_timeout_ms(params);
...
pub(crate) async fn handle_browser_extension_relay_poll(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let timeout_ms = browser_timeout_ms(params);
...
pub(crate) async fn handle_browser_extension_relay_send(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let timeout_ms = browser_timeout_ms(params);
...
pub(crate) async fn handle_browser_content_script_inject(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
...
pub(crate) async fn handle_browser_page_extract(
    params: &Value,
    channel: &Arc<GatewayChannel>,
) -> RpcResult {
    let timeout_ms = browser_timeout_ms(params);
    let interactive_only = opt_bool(params, "interactive_only", false);
    let max_interactive = opt_u64(params, "max_interactive", 128).max(1) as usize;
    let max_text_chars = opt_u64(params, "max_text_chars", 8_000).max(128) as usize;
    let max_links = opt_u64(params, "max_links", 64).max(1) as usize;
    let include_html = opt_bool(params, "include_html", false);
    let max_html_chars = opt_u64(params, "max_html_chars", 40_000).max(256) as usize;
