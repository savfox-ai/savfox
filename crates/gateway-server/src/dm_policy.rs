...
impl DmPolicyStore {
    ...
    pub fn new(savfox_home: &Path) -> Self {
        let path = savfox_home.join(GATEWAY_SUBDIR).join("dm-policies.json");
        Self {
            path,
    ...
    pub async fn load(&self) {
        if let Ok(Some(policies)) = crate::json_store::load_json_opt::<
            HashMap<String, ChannelDmPolicy>,
        >(&self.path, "DM policies")
        .await
        {
    ...
    pub async fn save(&self) -> Result<(), String> {
        ...
        crate::json_store::save_json(&self.path, &*policies, "DM policies").await
    }
