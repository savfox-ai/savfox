...
/// Simple RPC latency benchmark for CI trend monitoring.
#[tokio::test]
#[ignore]
async fn rpc_latency_benchmark() {
    let (url, token) = require_gateway!();
    let mut ws = ws_connect_with_query_token(&url, &token).await;

    // Warmup.
    for i in 0..10_u64 {
        let _ = ws_rpc_call(&mut ws, 3000 + i, "status", json!({})).await;
    }

    let mut samples_ms = Vec::new();
    for i in 0..100_u64 {
        let start = std::time::Instant::now();
        let _ = ws_rpc_call(&mut ws, 4000 + i, "status", json!({})).await;
    ...
    eprintln!(
        "RPC latency benchmark (status): p50={:.2}ms p95={:.2}ms samples={}",
        p50,
        p95,
        samples_ms.len()
    );

    // Keep the threshold realistic for CI VMs.
    assert!(p95 < 250.0, "p95 latency too high: {p95:.2}ms");
