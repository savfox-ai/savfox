...
//! elapsed time expose `transcript_animation_tick()`, and code that mutates the active cell in
...
use savfox_core::protocol::{FileChange, McpAuthStatus, McpInvocation, SessionConfiguredEvent};
use savfox_core::web_search::web_search_detail;
use savfox_otel::RuntimeMetricsSummary;
...
impl HistoryCell for UpdateAvailableHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let update_instruction = if let Some(update_action) = self.update_action {
            ...
            line![
                "See ",
                "https://github.com/openai/savfox".cyan().underlined(),
        ...
        let content = text![
            line![
                ...
                " ",
                format!("{SAVFOX_CLI_VERSION} -> {}", self.latest_version).bold(),
            ],
            update_instruction,
            "",
            "See full release notes:",
            "https://github.com/openai/savfox/releases/latest"
                .cyan()
                .underlined(),
        ];
    ...
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        for process in &self.processes {
            ...
            if needs_suffix && budget > truncation_suffix_width {
                ...
                let (truncated, ..) = take_prefix_by_width(&snippet, available);
                out.push(vec![prefix.dim(), truncated.cyan(), truncation_suffix.dim()].into());
            } else {
                let (truncated, ..) = take_prefix_by_width(&snippet, budget);
            ...
            for (idx, chunk) in process.recent_chunks.iter().enumerate() {
                ...
                if !remainder.is_empty() && budget > truncation_suffix_width {
                    let available = budget.saturating_sub(truncation_suffix_width);
                    let (shorter, ..) = take_prefix_by_width(chunk, available);
                    out.push(
        ...
        if remaining > 0 {
            ...
            if wrap_width <= prefix_width {
                ...
                let budget = wrap_width.saturating_sub(prefix_width);
                let (truncated, ..) = take_prefix_by_width(&more_text, budget);
        ...
        ApprovedExecpolicyAmendment {
            ...
            (
                ...
                vec![
                    "You ".into(),
                    "approved".bold(),
                    " savfox to always run commands that start with ".into(),
                    snippet,
                ],
            )
        }
...
pub(crate) fn new_session_info(
    ...
    // On the startup/welcome screen, suppress the session header history box to
    // keep the hero view clean.
    if !is_first_event {
...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<savfox_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<savfox_protocol::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(savfox_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
///
/// Manual testing tip:
/// - Run the rmcp stdio test server (`savfox-rs/rmcp-client/src/bin/test_stdio_server.rs`) and
///   register it as an MCP server via `savfox mcp add`.
/// - Use its `image_scenario` tool with cases like `text_then_image`, `invalid_base64_then_image`,
///   or `invalid_image_bytes_then_image` to ensure this path triggers even when the first block is
///   not a valid image.
...
pub(crate) fn empty_mcp_output() -> PlainHistoryCell {
    let lines: Vec<Line<'static>> = vec![
        ...
        Line::from(vec![
            "    See the ".into(),
            "\u{1b}]8;;https://developers.openai.com/savfox/mcp\u{7}MCP docs\u{1b}]8;;\u{7}"
                .underlined(),
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, savfox_protocol::mcp::Tool>,
...
mod tests {
    ...
    #[tokio::test]
    async fn session_info_omits_header_on_startup_banner() {
        let config = test_config().await;
        let event = savfox_core::protocol::SessionConfiguredEvent {
        ...
        let cell = new_session_info(&config, "test-model", event, true, None);
        assert!(cell.display_lines(120).is_empty());
    }

    #[tokio::test]
    async fn session_info_includes_header_after_startup() {
        let config = test_config().await;
        let event = savfox_core::protocol::SessionConfiguredEvent {
            session_id: savfox_protocol::SessionId::new(),
            forked_from_id: None,
        ...
        let cell = new_session_info(&config, "test-model", event, false, None);
        let rendered = render_lines(&cell.display_lines(120)).join("\n");
        assert!(rendered.contains(">_ Savfox"));
    ...
    fn startup_session_header_uses_ascii_logo_and_centers_card() {
        ...
        assert!(
            ...
            "startup header should render ASCII logo"
        );
        assert!(
            lines.iter().any(|line| line.contains("vtest")),
            "startup header should keep version"
        );
        ...
        let top_border = lines
            ...
            .expect("top border");
        let left_padding = top_border.chars().take_while(|ch| *ch == ' ').count();
        assert!(left_padding > 0, "startup header should be centered");
    }
    ...
    fn plan_update_with_note_and_wrapping_snapshot() {
        // Long explanation forces wrapping; include long step text to verify step wrapping and
        // alignment.
