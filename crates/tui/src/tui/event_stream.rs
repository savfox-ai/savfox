...
//!
//! - [`EventBroker`] holds the shared crossterm stream so multiple callers reuse the same input
//!   source and can drop/recreate it on pause/resume without rebuilding consumers.
//! - [`TuiEventStream`] wraps a draw event subscription plus the shared [`EventBroker`] and maps
//!   crossterm events into [`TuiEvent`].
...
//!
//! The motivation for dropping/recreating the crossterm event stream is to enable the TUI to fully
//! relinquish stdin. If the stream is not dropped, it will continue to read from stdin even if it
//! is not actively being polled (due to how crossterm's EventStream is implemented), potentially
//! stealing input from other processes reading stdin, like terminal text editors. This race can
//! cause missed input or capturing terminal query responses (for example, OSC palette/size queries)
//! that the other process expects to read. Stopping polling, instead of dropping the stream, is
//! only sufficient when the pause happens before the stream enters a pending state; otherwise the
//! crossterm reader session may keep reading from stdin, so the safer approach is to drop and
//! recreate the event stream when we need to hand off the terminal.
//!
...
/// Shared crossterm input state for all [`TuiEventStream`] instances. A single crossterm
/// EventStream is reused so all streams still see the same input source.
///
/// This intermediate layer enables dropping/recreating the underlying EventStream (pause/resume)
...
impl<S: EventSource + Default + Unpin> TuiEventStream<S> {
        ...
        loop {
            let poll_result = {
                ...
                match Pin::new(events).poll_next(cx) {
                    ...
                    Poll::Pending => {
                        drop(state);
                        // Poll resume_stream so resume_events can wake us even while waiting on
                        // stdin
                        match Pin::new(&mut self.resume_stream).poll_next(cx) {
                            Poll::Ready(Some(())) => continue,
                            Poll::Ready(None) => return Poll::Ready(None),
    ...
    /// Poll the draw broadcast stream for the next draw event. Draw events are used to trigger a
    /// redraw of the TUI.
    ...
    fn map_crossterm_event(&mut self, event: Event) -> Option<TuiEvent> {
        match event {
            ...
            Event::Resize(..) => Some(TuiEvent::Draw),
