...
use super::{DisableAlternateScroll, EnableAlternateScroll, Terminal};
use crate::key_hint;
...
impl SuspendContext {
    ...
    ///
    /// - If the alt screen is active, exit alt-scroll/alt-screen and record `RestoreAlt`; otherwise
    ///   record `RealignInline`.
    ...
    pub(crate) fn suspend(&self, alt_screen_active: &Arc<AtomicBool>) -> Result<()> {
        if alt_screen_active.load(Ordering::Relaxed) {
            // Leave alt-screen so the terminal returns to the normal buffer while suspended; also
            // turn off alt-scroll.
