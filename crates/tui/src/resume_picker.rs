...
enum BackgroundEvent {
    PageLoaded {
        request_token: usize,
        search_token: Option<usize>,
        page: std::io::Result<SessionsPage>,
    },
}
...
struct PickerState {
    ...
    action: SessionPickerAction,
    session_name_cache: HashMap<SessionId, Option<String>>,
}
...
impl Row {
    fn display_preview(&self) -> &str {
        self.session_name.as_deref().unwrap_or(&self.preview)
    }
...
impl PickerState {
    fn new(
        ...
        Self {
            ...
            filter_cwd,
            action,
            session_name_cache: HashMap::new(),
        }
    }
    ...
    fn start_initial_load(&mut self) {
        ...
        (self.page_loader)(PageLoadRequest {
            savfox_home: self.savfox_home.clone(),
            cursor: None,
            request_token,
            search_token: None,
    ...
    async fn handle_background_event(&mut self, event: BackgroundEvent) -> Result<()> {
        match event {
            BackgroundEvent::PageLoaded {
                ...
                self.update_session_names().await;
    ...
    fn ingest_page(&mut self, page: SessionsPage) {
    ...
    async fn update_session_names(&mut self) {
        let mut missing_ids = HashSet::new();
        for row in &self.all_rows {
            let Some(session_id) = row.session_id else {
                continue;
            };
            if self.session_name_cache.contains_key(&session_id) {
                continue;
            }
            missing_ids.insert(session_id);
        }
        ...
        let names = find_session_names_by_ids(&self.savfox_home, &missing_ids)
            .await
            .unwrap_or_default();
        for session_id in missing_ids {
            let session_name = names.get(&session_id).cloned();
            self.session_name_cache.insert(session_id, session_name);
        }
        ...
        for row in self.all_rows.iter_mut() {
            ...
            let session_name = self.session_name_cache.get(&session_id).cloned().flatten();
...
fn rows_from_items(items: Vec<SessionItem>) -> Vec<Row> {
    items.into_iter().map(|item| head_to_row(&item)).collect()
}

fn head_to_row(item: &SessionItem) -> Row {
    let created_at = item
        .created_at
        .as_deref()
...
) -> (Option<PathBuf>, Option<String>, Option<SessionId>) {
    for value in head {
        if let Ok(meta_line) = serde_json::from_value::<SessionMetaLine>(value.clone()) {
            ...
            let session_id = Some(meta_line.meta.id);
...
mod tests {
    ...
    fn make_item(path: &str, ts: &str, preview: &str) -> SessionItem {
        SessionItem {
    ...
    fn rows_from_items_preserves_backend_order() {
        ...
        // Preserve the given order even if timestamps differ; backend already provides
        // newest-first.
    ...
    #[test]
    fn row_display_preview_prefers_session_name() {
        let row = Row {
            path: PathBuf::from("/tmp/a.jsonl"),
            preview: String::from("first message"),
            session_id: None,
            session_name: Some(String::from("My session")),
            created_at: None,
            updated_at: None,
            cwd: None,
            git_branch: None,
    ...
    #[tokio::test]
    async fn resume_picker_session_names_snapshot() {
        use ratatui::layout::{Constraint, Layout};
        ...
        let id1 =
            SessionId::from_string("11111111-1111-1111-1111-111111111111").expect("session id 1");
        let id2 =
            SessionId::from_string("22222222-2222-2222-2222-222222222222").expect("session id 2");
        let entries = vec![
            json!({
                "id": id1,
                "session_name": "Keep this for now",
        ...
        state.update_session_names().await;
        ...
        let snapshot = terminal.backend().to_string();
        assert_snapshot!("resume_picker_session_names", snapshot);
    }
