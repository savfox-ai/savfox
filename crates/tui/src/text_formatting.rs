...
/// Truncate a tool result to fit within the given height and width. If the text is valid JSON, we
/// format it in a compact way before truncating. This is a best-effort approach that may not work
/// perfectly for text where 1 grapheme is rendered as multiple terminal cells.
...
pub(crate) fn format_json_compact(text: &str) -> Option<String> {
    ...
    // Convert multi-line pretty JSON to compact single-line format by removing newlines and excess
    // whitespace
    ...
    // Iterate over the characters in the JSON string, adding spaces after : and , but only when not
    // in a string
...
pub(crate) fn truncate_text(text: &str, max_graphemes: usize) -> String {
    ...
    // Check if there's a grapheme at position max_graphemes (meaning there are more than
    // max_graphemes total)
...
mod tests {
    ...
    fn test_center_truncate_doesnt_truncate_short_path() {
        let sep = std::path::MAIN_SEPARATOR;
        let path = format!("{sep}Users{sep}savfox{sep}Public");
    ...
    fn test_center_truncate_truncates_long_windows_path() {
        let sep = std::path::MAIN_SEPARATOR;
        let path = format!(
            "C:{sep}Users{sep}savfox{sep}Projects{sep}super{sep}long{sep}windows{sep}path{sep}file.txt"
        );
        let truncated = center_truncate_path(&path, 36);

        let expected = format!("C:{sep}Users{sep}savfox{sep}…{sep}path{sep}file.txt");
