...
//! state diverging from the agent if a rollback fails or targets a different session.
//!
//! Backtrack operates as a small state machine:
//! - The first `Esc` in the main view "primes" the feature and captures a base session id.
...
//! - Only after receiving `EventMsg::SessionRolledBack` do we trim local transcript state and
...
//! tail derived from the current in-flight `ChatScreen.active_cell`.
//!
//! That live tail is kept in sync during `TuiEvent::Draw` handling for `Overlay::Transcript` by
//! asking `ChatScreen` for an active-cell cache key and transcript lines and by passing them into
...
use savfox_core::protocol::{ErrorEvent, EventMsg, Op, SavfoxErrorInfo};
...
pub(crate) struct BacktrackState {
    ...
    /// Session id of the base session to rollback.
    ///
    /// If the current session changes, backtrack selections become invalid and must be ignored.
    pub(crate) base_id: Option<SessionId>,
...
/// the same active session we issued the request for.
#[derive(Debug, Clone)]
...
impl App {
    ...
    pub(crate) fn handle_backtrack_esc_key(&mut self, tui: &mut tui::Tui) {
        if !self.chat_screen.composer_is_empty() {
            return;
        }
    ...
    /// Stage a backtrack and request session history from the agent.
    ///
    /// We send the rollback request immediately, but we only mutate the transcript after core
    /// confirms success so the UI cannot get ahead of the actual session state.
    ///
    ...
    pub(crate) fn apply_backtrack_rollback(&mut self, selection: BacktrackSelection) {
        ...
        if self.backtrack.pending_rollback.is_some() {
            self.chat_screen
                .add_error_message("Backtrack rollback already in progress.".to_owned());
            return;
        }
        ...
        self.backtrack.pending_rollback = Some(PendingBacktrackRollback {
            selection,
            session_id: self.chat_screen.session_id(),
        });
        self.chat_screen
            .submit_op(Op::SessionRollback { num_turns });
    ...
    fn prime_backtrack(&mut self) {
        ...
        self.backtrack.base_id = self.chat_screen.session_id();
        self.chat_screen.show_esc_backtrack_hint();
    }
    ...
    fn open_backtrack_preview(&mut self, tui: &mut tui::Tui) {
        ...
        self.chat_screen.clear_esc_backtrack_hint();
        self.step_backtrack_and_highlight(tui);
    }
    ...
    ///
    /// `TranscriptOverlay` owns committed transcript cells, while `ChatScreen` owns the current
    ...
    ///
    /// This logic lives here (instead of inside the overlay widget) because `ChatScreen` is the
    ...
    fn overlay_forward_event(&mut self, tui: &mut tui::Tui, event: TuiEvent) -> Result<()> {
        ...
        {
            let active_key = self.chat_screen.active_cell_transcript_key();
            let chat_screen = &self.chat_screen;
            tui.draw(u16::MAX, |frame| {
                let width = frame.area().width.max(1);
                t.sync_live_tail(width, active_key, |w| {
                    chat_screen.active_cell_transcript_lines(w)
                });
    ...
    pub(crate) fn handle_backtrack_event(&mut self, event: &EventMsg) {
        match event {
            EventMsg::SessionRolledBack(_) => self.finish_pending_backtrack(),
    ...
    ///
    /// We ignore events that do not correspond to the currently active session to avoid applying
    ...
    fn finish_pending_backtrack(&mut self) {
        ...
        if pending.session_id != self.chat_screen.session_id() {
            // Ignore rollbacks targeting a prior session.
            return;
        }
    ...
    fn backtrack_selection(&self, nth_user_message: usize) -> Option<BacktrackSelection> {
        ...
        if self.chat_screen.session_id() != Some(base_id) {
            return None;
        }
