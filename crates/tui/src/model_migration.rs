...
pub(crate) fn migration_copy_for_models(
    ...
    let heading_text = Span::from(format!(
        "Savfox just got an upgrade. Introducing {target_display_name}."
    ))
    .bold();
...
impl ModelMigrationScreen {
    ...
    fn render_menu(&self, column: &mut ColumnRenderable) {
        column.push(Line::from(""));
        column.push(
            Paragraph::new("Choose how you'd like Savfox to proceed.")
                .wrap(Wrap { trim: false })
                .inset(Insets::tlbr(0, 2, 0, 0)),
        );
        column.push(Line::from(""));
...
mod tests {
    ...
    use super::{ModelMigrationScreen, migration_copy_for_models};
    ...
    #[test]
    fn prompt_snapshot_gpt5_savfox() {
        ...
        terminal.flush().expect("flush");
        assert_snapshot!("model_migration_prompt_gpt5_savfox", terminal.backend());
    }

    #[test]
    fn prompt_snapshot_gpt5_savfox_mini() {
        let backend = VT100Backend::new(60, 22);
        let mut terminal = Terminal::with_options(backend).expect("terminal");
        terminal.set_viewport_area(Rect::new(0, 0, 60, 22));
        ...
        terminal.flush().expect("flush");
        assert_snapshot!(
            "model_migration_prompt_gpt5_savfox_mini",
            terminal.backend()
        );
    }
