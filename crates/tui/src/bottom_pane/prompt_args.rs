...
/// Extract positional arguments from a composer first line like "/name a b" for a given prompt
/// name. Returns empty when the command name does not match or when there are no args.
...
fn replace_text_elements_with_sentinels(
    ...
    for (idx, elem) in elements.iter().enumerate() {
        ...
        let mut sentinel = format!("__SAVFOX_ELEM_{idx}__");
