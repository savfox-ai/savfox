...
use super::footer::{
    CollaborationModeIndicator, FooterMode, FooterProps, SummaryLeft, can_show_left_with_context,
    context_window_line, esc_hint_mode, footer_height, footer_hint_items_width, footer_line_width,
    inset_footer_hint_area, render_context_right, render_footer_from_props,
    render_footer_hint_items, render_footer_line, render_model_status_line, render_path_info_line,
    render_shortcut_hints_line, reset_mode_after_activity, single_line_footer_layout,
    toggle_shortcut_mode,
};
use super::paste_burst::{CharDecision, PasteBurst};
use super::skill_popup::{MentionItem, SkillPopup};
use super::slash_commands;
use crate::app_event::{AppEvent, ConnectorsSnapshot};
...
use crate::key_hint::{KeyBinding, has_ctrl_or_alt};
...
impl ChatComposer {
    ...
    fn composer_top_inset(is_session_mode: bool) -> u16 {
...
impl ChatComposer {
    pub(crate) fn render_with_mask(&self, area: Rect, buf: &mut Buffer, mask_char: Option<char>) {
        ...
        if !self.is_session_mode && composer_rect.height > 0 {
            ...
            let rule = "─".repeat(composer_rect.width as usize);
        ...
        if !textarea_rect.is_empty() {
            let prompt = if self.input_enabled {
                if self.is_session_mode {
                    ...
                    "❯ ".fg(cursor_color).bg(composer_bg)
                }
            } else {
...
mod tests {
    ...
    fn footer_hint_row_is_separated_from_composer() {
        ...
        let separator_spacing =
            !trimmed_spacing.is_empty() && trimmed_spacing.chars().all(|ch| ch == '─');
        assert!(
