...
mod tests {
    ...
    fn wrapped_navigation_across_visual_lines() {
        ...
        // From the very start, moving down should go to the start of the next wrapped line (index
        // 4)
        t.set_cursor(0);
        t.move_cursor_down();
        assert_eq!(t.cursor(), 4);
        ...
        // Move down should return to same position on the next wrapped line -> back to index 6
        // ('g')
        t.move_cursor_down();
        assert_eq!(t.cursor(), 6);

        // Move down again should go to third wrapped line. Target col is 2, but the line has len 2
        // -> clamp to end
        t.move_cursor_down();
        assert_eq!(t.cursor(), t.text().len());
    }
    ...
    fn wrapped_navigation_with_newlines_and_spaces() {
        ...
        // Down again should cross the logical newline to the next visual line ("word3"), clamped to
    ...
    fn wrapped_navigation_with_wide_graphemes() {
        // Four thumbs up, each of display width 2, with width 3 to force wrapping inside grapheme
        // boundaries
        let mut t = ta_with("👍👍👍👍");
        ...
        // Move down should go to the start of the next wrapped line (same column preserved but
        // clamped)
        t.move_cursor_down();
        ...
        for _case in 0..500 {
            ...
            for _step in 0..60 {
                ...
                match rng.random_range(0..18) {
                    ...
                    15 => {
                        // Replace a range that intersects an element -> whole element should be
                        // replaced
                        if let Some(payload) = elem_texts.choose(&mut rng).cloned()
                            && let Some(start) = ta.text().find(&payload)
                        {
                    ...
                    16 => {
                        // Try setting the cursor to a position inside an element; it should clamp
                        // out
                        if let Some(payload) = elem_texts.choose(&mut rng).cloned()
                            && let Some(start) = ta.text().find(&payload)
                        {
                ...
                // After wrapping, desired height equals the number of lines we would render without
                // scroll
