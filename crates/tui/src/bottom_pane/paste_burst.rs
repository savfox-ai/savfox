...
//! - `buffer`: accumulated burst text that will eventually flush as a single `Paste(String)`. A
//!   non-empty buffer is treated as "in burst context" even if `active` has been cleared.
