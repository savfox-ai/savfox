...
//! widget (`ChatScreen`). This split matters for Ctrl+C/Ctrl+D: the bottom pane gives the active
//! view the first chance to consume Ctrl+C (typically to dismiss itself), and `ChatScreen` may
...
mod feedback_view;
pub(crate) use feedback_view::{
    FeedbackAudience, feedback_disabled_params, feedback_selection_params,
    feedback_upload_consent_params,
};
pub(crate) use skills_toggle_view::{SkillsToggleItem, SkillsToggleView};
mod paste_burst;
...
/// - `ChatScreen`: arming the double-press quit shortcut.
...
pub(crate) use chat_composer::{ChatComposer, ChatComposerConfig, InputResult};
pub(crate) use experimental_features_view::{ExperimentalFeatureItem, ExperimentalFeaturesView};
...
/// process-level decisions (quit, interrupt, shutdown) to `ChatScreen`.
...
impl BottomPane {
    ...
    /// was received, but it does not decide whether the process should exit; `ChatScreen` owns the
    ...
    ///
    /// `ChatScreen` owns the quit shortcut state machine (it decides when quit is
    ...
    pub(crate) fn show_quit_shortcut_hint(&mut self, key: KeyBinding) {
        ...
        if let Ok(handle) = tokio::runtime::Handle::try_current() {
            ...
            // In tests (and other non-Tokio contexts), fall back to a session so
    ...
    pub(crate) fn set_model_display(&mut self, model: String, provider: String) {
        self.composer.set_model_display(model, provider);
        self.request_redraw();
    }

    pub(crate) fn set_cwd_display(&mut self, cwd: String) {
        self.composer.set_cwd_display(cwd);
        self.request_redraw();
    }

    pub(crate) fn set_session_mode(&mut self, session_mode: bool) {
        self.composer.set_session_mode(session_mode);
        self.request_redraw();
    }

    pub(crate) fn set_alt_screen_active(&mut self, active: bool) {
        self.composer.set_alt_screen_active(active);
    }

    /// Set the border color for the composer area.
    pub(crate) fn set_border_color(&mut self, color: Option<ratatui::style::Color>) {
        self.composer.set_border_color(color);
        self.request_redraw();
    }
...
mod tests {
    ...
    fn composer_shown_after_denied_while_task_running() {
        ...
        // Give the animation session a moment to tick.
    ...
    fn status_and_composer_fill_height_without_bottom_padding() {
        ...
        // Use height == desired_height; expect spacer + status + composer rows without trailing
        // padding.
    ...
    fn esc_with_skill_popup_does_not_interrupt_task() {
        ...
        while let Ok(ev) = rx.try_recv() {
            assert!(
                !matches!(ev, AppEvent::SavfoxOp(Op::Interrupt)),
    ...
    fn esc_interrupts_running_task_when_no_popup() {
        ...
        assert!(
            matches!(rx.try_recv(), Ok(AppEvent::SavfoxOp(Op::Interrupt))),
