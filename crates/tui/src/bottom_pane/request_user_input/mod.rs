...
use crate::bottom_pane::selection_popup_common::{GenericDisplayRow, measure_rows_height};
use crate::bottom_pane::{CancellationEvent, ChatComposer, ChatComposerConfig, InputResult};
...
impl RequestUserInputOverlay {
    ...
    fn current_question(
        &self,
    ) -> Option<&savfox_protocol::request_user_input::RequestUserInputQuestion> {
    ...
    fn options_len_for_question(
        question: &savfox_protocol::request_user_input::RequestUserInputQuestion,
    ) -> usize {
    ...
    fn submit_answers(&mut self) {
        ...
        self.app_event_tx
            .send(AppEvent::SavfoxOp(Op::UserInputAnswer {
...
impl BottomPaneView for RequestUserInputOverlay {
    ...
    fn handle_key_event(&mut self, key_event: KeyEvent) {
        ...
        if matches!(key_event.code, KeyCode::Esc) {
            ...
            self.app_event_tx.send(AppEvent::SavfoxOp(Op::Interrupt));
            self.done = true;
            return;
        }
...
mod tests {
    ...
    fn expect_interrupt_only(rx: &mut tokio::sync::mpsc::UnboundedReceiver<AppEvent>) {
        ...
        let AppEvent::SavfoxOp(op) = event else {
            panic!("expected SavfoxOp");
        };
    ...
    fn options_can_submit_empty_when_unanswered() {
        ...
        let AppEvent::SavfoxOp(Op::UserInputAnswer { id, response, .. }) = event else {
    ...
    fn enter_commits_default_selection_on_last_option_question() {
        ...
        let event = rx.try_recv().expect("expected AppEvent");
        let AppEvent::SavfoxOp(Op::UserInputAnswer { response, .. }) = event else {
            panic!("expected UserInputAnswer");
        };
        let answer = response.answers.get("q1").expect("answer missing");
