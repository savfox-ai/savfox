...
pub enum UpdateAction {
    ...
    NpmGlobalLatest,
    /// Update via `bun install -g @openai/savfox@latest`.
    BunGlobalLatest,
    /// Update via `brew upgrade savfox`.
    BrewUpgrade,
}
...
pub(crate) fn get_update_action() -> Option<UpdateAction> {
    ...
    let managed_by_npm = std::env::var_os("SAVFOX_MANAGED_BY_NPM").is_some();
    let managed_by_bun = std::env::var_os("SAVFOX_MANAGED_BY_BUN").is_some();
...
mod tests {
    ...
    fn detects_update_action_without_env_mutation() {
        ...
        assert_eq!(
            detect_update_action(
                true,
                std::path::Path::new("/opt/homebrew/bin/savfox"),
                false,
                false
            ),
            Some(UpdateAction::BrewUpgrade)
        ...
        assert_eq!(
            detect_update_action(
                true,
                std::path::Path::new("/usr/local/bin/savfox"),
                false,
                false
            ),
            Some(UpdateAction::BrewUpgrade)
