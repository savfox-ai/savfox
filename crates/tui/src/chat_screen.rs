...
use savfox_core::config::provider_store::{
    ProviderStoreFile, provider_models_store_dir, read_provider_store_api_key,
};
...
mod skills;
use chrono::Local;
use savfox_common::approval_presets::{ApprovalPreset, builtin_approval_presets};
...
impl ChatScreen {
    ...
    fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::Logout => {
                if let Err(e) = savfox_core::auth::logout(
    ...
    fn model_provider_buckets_for_presets(
        ...
        let builtins = built_in_model_providers();
        let mut buckets: Vec<ModelProviderBucket> = provider_ids
            .into_iter()
            .map(|provider_id| {
                ...
                ModelProviderBucket {
                    ...
                    aliases: Self::provider_aliases(&provider_id, &provider_name),
                }
            })
            .collect();
...
async fn fetch_rate_limits(base_url: String, auth: SavfoxAuth) -> Option<RateLimitSnapshot> {
