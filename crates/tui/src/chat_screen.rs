...
//! cell (`ChatScreen.active_cell`) that can mutate in place while streaming (often representing a
...
use savfox_core::config::provider_store::{
    ProviderStoreFile, list_provider_store_files, provider_models_store_dir,
...
use savfox_core::project_doc::DEFAULT_PROJECT_DOC_FILENAME;
use savfox_core::protocol::{
    AgentMessageDeltaEvent, AgentMessageEvent, AgentReasoningDeltaEvent, AgentReasoningEvent,
    AgentReasoningRawContentDeltaEvent, AgentReasoningRawContentEvent,
    ApplyPatchApprovalRequestEvent, BackgroundEventEvent, CreditsSnapshot, DeprecationNoticeEvent,
...
use savfox_protocol::config_types::{
    CollaborationMode, CollaborationModeMask, ModeKind, Personality, Settings,
};
...
const STARTUP_BOTTOM_SPACER_LINES: u16 = 8;
...
use crate::bottom_pane::{
    ApprovalRequest, BottomPane, BottomPaneParams, CancellationEvent, CollaborationModeIndicator,
    DOUBLE_PRESS_QUIT_SHORTCUT_ENABLED, ExperimentalFeatureItem, ExperimentalFeaturesView,
    FeedbackAudience, InputResult, LocalImageAttachment, QUIT_SHORTCUT_TIMEOUT, SelectionAction,
    SelectionItem, SelectionViewParams,
};
use crate::clipboard_paste::paste_image_to_temp_png;
use crate::diff_render::display_path_for;
use crate::exec_cell::{CommandOutput, ExecCell, new_active_exec_command};
use crate::exec_command::{relativize_to_home, strip_bash_lc_and_escape};
use crate::get_git_diff::get_git_diff;
use crate::history_cell::{
    AgentMessageCell, HistoryCell, McpToolCallCell, PlainHistoryCell, WebSearchCell,
};
use crate::key_hint::KeyBinding;
use crate::markdown::append_markdown;
use crate::provider_connect::{
    ProviderConnectResult, connect_provider_candidates, provider_has_auth_in_env,
    provider_requires_api_key,
};
use crate::render::Insets;
use crate::render::renderable::{
    ColumnRenderable, FlexRenderable, Renderable, RenderableExt, RenderableItem,
};
...
pub(crate) use self::rate_limit_warning::get_limits_duration;
use self::rate_limit_warning::{
    NUDGE_MODEL_SLUG, RATE_LIMIT_SWITCH_PROMPT_THRESHOLD, RateLimitWarningState,
};
mod skills;
use chrono::Local;
use savfox_common::permission_presets::{PermissionPreset, builtin_tui_presets};
...
use savfox_protocol::openai_models::{
    InputModality, ModelPreset, ReasoningEffort as ReasoningEffortConfig, ReasoningEffortPreset,
...
use self::skills::{
    collect_tool_mentions, find_app_mentions, find_skill_mentions_with_tool_mentions,
};
use crate::streaming::controller::{PlanStreamController, StreamController};
...
/// Common initialization parameters shared by all `ChatScreen` constructors.
pub(crate) struct ChatScreenInit {
...
fn rate_limit_error_kind(info: &SavfoxErrorInfo) -> Option<RateLimitErrorKind> {
    match info {
        ...
        SavfoxErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        SavfoxErrorInfo::ResponseTooManyFailedAttempts {
...
///
/// `ChatScreen` owns the state derived from the protocol event stream (history cells, streaming
...
pub(crate) struct ChatScreen {
    ...
    /// flushing. It is intentionally allowed to wrap, which implies a rare one-time cache
    /// collision where the overlay may briefly treat new tail content as already cached.
    ...
    /// Tracks whether savfox-core currently considers an agent turn to be in progress.
    ///
    /// This is kept separate from `mcp_startup_status` so that MCP startup progress (or
    /// completion) can update the status header without accidentally clearing the spinner for
    /// an active turn.
    ...
    // User prompt cells captured before SessionConfigured arrives. These are
    // flushed after the session header so inline scrollback order is stable.
    pending_user_history_before_session: Vec<Box<dyn HistoryCell>>,
    ...
    // Whether the current turn performed "work" (exec commands, MCP tool calls, patch
    // applications).
    //
...
impl ChatScreen {
    ...
    fn on_session_configured(&mut self, event: savfox_core::protocol::SessionConfiguredEvent) {
        self.bottom_pane
        ...
        let suppress_session_header = self.show_welcome_banner || self.alt_screen_active;
        let session_info_cell = history_cell::new_session_info(
            &self.config,
            &model_for_header,
            event,
            suppress_session_header,
            self.auth_manager
                .auth_cached()
        ...
        if !self.pending_user_history_before_session.is_empty() {
            let pending_cells = std::mem::take(&mut self.pending_user_history_before_session);
        ...
        // Ask savfox-core to enumerate custom prompts for this session.
    ...
    fn on_session_name_updated(&mut self, event: savfox_core::protocol::SessionNameUpdatedEvent) {
    ...
    pub(crate) fn open_feedback_note(
        ...
        let snapshot = self.feedback.snapshot(self.session_id);
    ...
    fn on_plan_delta(&mut self, delta: String) {
        ...
        if self.plan_stream_controller.is_none() {
            self.plan_stream_controller = Some(PlanStreamController::new(
                self.content_width().map(|w| w.saturating_sub(4)),
            ));
        }
    ...
    fn on_task_complete(&mut self, last_agent_message: Option<String>, from_replay: bool) {
        ...
        // still show the prompt once after session switch replay.
        if !from_replay {
            self.saw_plan_item_this_turn = false;
        }
    ...
    fn on_terminal_interaction(&mut self, ev: TerminalInteractionEvent) {
        ...
        if ev.stdin.is_empty() {
            ...
            // Surface this in the status header (single "waiting" surface) instead of the
            // transcript.
    ...
    fn on_patch_apply_end(&mut self, event: savfox_core::protocol::PatchApplyEndEvent) {
        let ev2 = event.clone();
        self.defer_or_handle(
    ...
    fn on_get_history_entry_response(
        &mut self,
        event: savfox_core::protocol::GetHistoryEntryResponseEvent,
    ) {
        let savfox_core::protocol::GetHistoryEntryResponseEvent {
            offset,
            log_id,
            entry,
        } = event;
    ...
    fn handle_streaming_delta(&mut self, delta: String) {
        ...
        if self.stream_controller.is_none() {
            ...
            self.stream_controller = Some(StreamController::new(
                self.content_width().map(|w| w.saturating_sub(2)),
            ));
        }
        if let Some(controller) = self.stream_controller.as_mut()
            && controller.push(&delta)
    ...
    pub(crate) fn handle_patch_apply_end_now(
        &mut self,
        event: savfox_core::protocol::PatchApplyEndEvent,
    ) {
    ...
    pub(crate) fn new(common: ChatScreenInit, session_manager: Arc<SessionManager>) -> Self {
        let ChatScreenInit {
            config,
            frame_requester,
            app_event_tx,
        ...
        let startup_tooltip = if config.show_tooltips {
            crate::tooltips::get_tooltip(None)
        } else {
            None
        };
        ...
        let mut widget = Self {
            ...
            pending_user_history_before_session: Vec::new(),
            ...
            startup_hero: StartupHero::with_tooltip(
                ratatui::style::Color::Rgb(255, 82, 82),
        ...
        if let Some(user_message) = widget.initial_user_message.take() {
            widget.submit_user_message(user_message);
        }
    ...
    /// Create a ChatScreen attached to an existing conversation (e.g., a fork).
    pub(crate) fn new_from_existing(
        common: ChatScreenInit,
        conversation: std::sync::Arc<savfox_core::SavfoxSession>,
        ...
        widget.bottom_pane.set_session_mode(true);
        ...
        match key_event {
            ...
            _ => match self.bottom_pane.handle_key_event(key_event) {
                InputResult::Submitted {
                    ...
                    // Submitted is only emitted when steer is enabled (Enter sends
                    // immediately). Reset any reasoning header only
                    // when we are actually submitting a turn.
                    self.reasoning_buffer.clear();
                    self.full_reasoning_buffer.clear();
                    self.set_status_header(String::from("Working"));
                    self.submit_user_message(user_message);
    ...
    pub(crate) fn set_alt_screen_active(&mut self, active: bool) {
        ...
        self.bottom_pane.set_alt_screen_active(active);
    }
    ...
    fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::Compact => {
                ...
                self.app_event_tx.send(AppEvent::SavfoxOp(Op::Compact));
            }
            SlashCommand::Review => {
            ...
            SlashCommand::ElevateSandbox => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    let Some(preset) = builtin_tui_presets()
                        .into_iter()
                    ...
                    if let Err(err) = self.config.approval_policy.can_set(&preset.policy.approval) {
                        self.add_error_message(err.to_string());
                        return;
                    }

                    self.otel_manager.counter(
                        "savfox.windows_sandbox.setup_elevated_sandbox_command",
                        1,
                        &[],
                    );
                    self.app_event_tx
            ...
            // SlashCommand::Undo => {
            //     self.app_event_tx.send(AppEvent::SavfoxOp(Op::Undo));
            // }
            SlashCommand::Diff => {
    ...
    fn dispatch_command_with_args(
        ...
        match cmd {
            SlashCommand::Rename if !trimmed.is_empty() => {
                ...
                self.request_redraw();
                self.app_event_tx
                    .send(AppEvent::SavfoxOp(Op::SetSessionName { name }));
                self.bottom_pane.drain_pending_submission_state();
            }
    ...
    fn show_rename_prompt(&mut self) {
        ...
        let view = CustomPromptView::new(
            ...
            Box::new(move |name: String| {
                ...
                let cell = Self::rename_confirmation_cell(&name, session_id);
                tx.send(AppEvent::InsertHistoryCell(Box::new(cell)));
                tx.send(AppEvent::SavfoxOp(Op::SetSessionName { name }));
            }),
        );
    ...
    fn submit_user_message(&mut self, user_message: UserMessage) {
        ...
        self.savfox_op_tx.send(op).unwrap_or_else(|e| {
        ...
        if !text.is_empty() {
            ...
            let user_prompt_cell = Box::new(history_cell::new_user_prompt(
                text,
                text_elements,
                local_image_paths,
            ));
            ...
            if self.is_session_configured() {
                self.add_boxed_history(user_prompt_cell);
            } else {
                self.pending_user_history_before_session
                    .push(user_prompt_cell);
            }
        }
    ...
    fn replay_initial_messages(&mut self, events: Vec<EventMsg>) {
        for msg in events {
            if matches!(
                msg,
                EventMsg::SessionConfigured(_) | EventMsg::SessionNameUpdated(_)
            ) {
                continue;
            }
    ...
    pub(crate) fn handle_savfox_event(&mut self, event: Event) {
        let Event { id, msg } = event;
        self.dispatch_event_msg(Some(id), msg, false);
    }

    pub(crate) fn handle_savfox_event_replay(&mut self, event: Event) {
        let Event { msg, .. } = event;
    ...
    fn dispatch_event_msg(&mut self, id: Option<String>, msg: EventMsg, from_replay: bool) {
        ...
        match msg {
            ...
            _ => {
                tracing::trace!("handle_savfox_event: {:?}", msg);
            }
        }

        match msg {
            EventMsg::SessionConfigured(e) => self.on_session_configured(e),
            EventMsg::SessionNameUpdated(e) => self.on_session_name_updated(e),
            ...
            EventMsg::Error(ErrorEvent {
                ...
                if let Some(info) = savfox_error_info
            ...
            EventMsg::ItemCompleted(event) => {
                if let savfox_protocol::items::TurnItem::Plan(plan_item) = event.item {
    ...
    fn on_entered_review_mode(&mut self, review: ReviewRequest, from_replay: bool) {
        ...
        let hint = review
            .user_facing_hint
            .unwrap_or_else(|| savfox_core::review_prompts::user_facing_hint(&review.target));
    ...
    /// Return the width of the main content area based on
    /// the last rendered terminal width. Returns `None` when no frame has been
    /// rendered yet.
    ...
    fn prefetch_rate_limits(&mut self) {
        ...
        if !self
            ...
            .auth_cached()
            .as_ref()
            .is_some_and(SavfoxAuth::is_chatgpt_auth)
        {
            return;
        }
    ...
    pub(crate) fn open_connect_provider_popup(&mut self) {
        ...
        for candidate in candidates {
            let provider_id = candidate.id.clone();
            let provider_name = candidate.name.clone();
            let mut description = candidate.description.clone();
            if candidate.requires_api_key {
                description.push_str(" (credentialed)");
            }
            let actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
                tx.send(AppEvent::OpenConnectAuthPrompt {
            ...
            items.push(SelectionItem {
                ...
                is_current: candidate.id == self.config.model_provider_id,
                actions,
                dismiss_on_select: true,
                search_value: Some(
                    format!("{} {}", candidate.name, candidate.id).to_ascii_lowercase(),
                ),
                ..Default::default()
            });
        }
    ...
    pub(crate) fn open_connect_auth_prompt(&mut self, provider_id: String) {
        let Some(provider) = self
            ...
            self.add_error_message(format!("Unknown provider: {provider_id}"));
            return;
        };
        ...
        self.open_connect_api_key_prompt_for_provider(provider_id, provider);
    }

    pub(crate) fn open_connect_api_key_prompt(&mut self, provider_id: String) {
        let Some(provider) = self
            .config
            .model_providers
            .get(provider_id.as_str())
    ...
    fn open_openai_connect_auth_method_popup(
        &mut self,
        provider_id: String,
        provider_name: String,
    ) {
        let provider_id_browser = provider_id.clone();
        let browser_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::BeginOpenAiConnectAuth {
                provider_id: provider_id_browser.clone(),
                method: OpenAiConnectAuthMethod::Browser,
            });
        })];

        let provider_id_headless = provider_id.clone();
        let headless_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::BeginOpenAiConnectAuth {
                provider_id: provider_id_headless.clone(),
    ...
    fn open_connect_api_key_prompt_for_provider(
        &mut self,
        provider_id: String,
        ...
        let has_saved_auth =
            read_provider_store_api_key(self.config.savfox_home.as_path(), &provider_id).is_some();
        let has_env_auth = provider_has_auth_in_env(provider_id.as_str(), &provider);
        let prompt = if has_saved_auth || has_env_auth {
            "Paste API key and press Enter (leave empty to reuse existing credentials)"
        } else {
    ...
    /// Show a prompt asking the user to name this provider account.
    /// The callback sends `ProviderConnectNamed` with the typed name.
    /// Validation (slug conflict check) happens in the `App` event handler.
    pub(crate) fn show_provider_naming_prompt(
        &mut self,
    ...
    pub(crate) fn open_connected_provider_models_popup(&mut self, provider_id: &str) {
        let Some(mut presets) =
    ...
    fn open_personality_popup_for_current_model(&mut self) {
        ...
        header.push(Line::from(
            "Choose a communication style for Savfox. Disable in /experimental.".dim(),
        ));
    ...
    fn model_provider_buckets(&self) -> Vec<ModelProviderBucket> {
        ...
        let builtin_ids: HashSet<String> = builtins.into_keys().collect();
        let has_cached_auth = self.auth_manager.auth_cached().is_some();

        let mut buckets: Vec<ModelProviderBucket> = self
            .config
            .model_providers
            .iter()
        ...
        if !buckets
            .iter()
            .any(|bucket| bucket.id == self.config.model_provider_id)
        {
        ...
        buckets.sort_by(|left, right| {
            let left_is_current = left.id == current_provider_id;
            let right_is_current = right.id == current_provider_id;
            right_is_current
                .cmp(&left_is_current)
    ...
    fn model_provider_buckets_for_presets(
        &self,
        presets: &[ModelPreset],
    ) -> Vec<ModelProviderBucket> {
        let account_ids: HashSet<String> = presets
            .iter()
            .filter_map(|preset| preset.slug.split_once('/'))
            .map(|(account_id, _)| account_id.trim())
            .filter(|account_id| !account_id.is_empty())
            .map(ToOwned::to_owned)
            .collect();

        if account_ids.is_empty() {
            return self.model_provider_buckets();
        }

        // Load store files to get account display names and underlying provider ids.
        ...
        let builtins = built_in_model_providers();
        let mut buckets: Vec<ModelProviderBucket> = account_ids
            .into_iter()
            .map(|account_id| {
                let (display_name, base_pid) =
                    if let Some(file) = store_map.get(account_id.as_str()) {
                        let pid = file.provider_id.trim();
                        let pid = if pid.is_empty() { &account_id } else { pid };
                        let base = self
                            .config
                            .model_providers
                            .get(pid)
                        ...
                        let base = self
                            ...
                            .map(|p| p.name.clone())
                            .or_else(|| builtins.get(account_id.as_str()).map(|p| p.name.clone()))
    ...
    fn provider_is_configured(
        ...
        if let Some(env_key) = provider.env_key.as_deref()
            && Self::provider_has_env_value(env_key)
        {
            return true;
        }
        ...
        if let Some(env_headers) = &provider.env_http_headers
            && env_headers
                .values()
                .any(|env_key| Self::provider_has_env_value(env_key))
        {
            return true;
        }

        if provider.requires_openai_auth && has_cached_auth {
            return true;
        }

        !builtin_ids.contains(provider_id)
    }

    fn provider_has_env_value(env_key: &str) -> bool {
        std::env::var(env_key)
            .map(|value| !value.trim().is_empty())
            .unwrap_or(false)
    }

    fn provider_aliases(provider_id: &str, provider_name: &str) -> Vec<String> {
        let mut aliases = Self::normalized_alias_tokens(provider_id);
        aliases.extend(Self::normalized_alias_tokens(provider_name));

        for alias in match provider_id.to_ascii_lowercase().as_str() {
            "openai" => vec!["gpt", "savfox", "o1", "o3", "o4", "bengalfox", "boomslang"],
            "anthropic" => vec!["claude", "sonnet", "haiku", "opus"],
            "gemini" => vec!["gemini"],
            "deepseek" => vec!["deepseek"],
            "mistral" => vec!["mistral", "ministral"],
            "qwen" => vec!["qwen"],
    ...
    fn normalized_alias_tokens(value: &str) -> Vec<String> {
        value
            .to_ascii_lowercase()
    ...
    fn is_meaningful_provider_alias(token: &str) -> bool {
        matches!(token, "gpt" | "glm" | "o1" | "o3" | "o4") || token.len() >= 4
    }

    fn infer_provider_bucket_for_model(
        preset: &ModelPreset,
        buckets: &[ModelProviderBucket],
    ...
    fn is_auto_model(model: &str) -> bool {
        model.starts_with("savfox-auto-")
    }

    fn auto_model_order(model: &str) -> usize {
        match model {
            "savfox-auto-fast" => 0,
            "savfox-auto-balanced" => 1,
            "savfox-auto-thorough" => 2,
            _ => 3,
        }
    }

    fn model_matches_current_selection(current_model: &str, candidate_model: &str) -> bool {
        if candidate_model == current_model {
            return true;
        }
        if let Some((provider_id, _)) = savfox_core::parse_provider_prefixed_model(candidate_model)
        {
            return savfox_core::request_model_for_provider(candidate_model, provider_id)
                == current_model;
        }
        false
    }
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        let current_provider_idx = provider_buckets
            .iter()
            .position(|bucket| bucket.id == self.config.model_provider_id)
            .or_else(|| {
                current_model.split_once('/').and_then(|(provider_id, _)| {
                    provider_buckets
                        .iter()
                        .position(|bucket| bucket.id.eq_ignore_ascii_case(provider_id))
                })
        ...
        let mut grouped_presets: Vec<Vec<ModelPreset>> = vec![Vec::new(); provider_buckets.len()];
        for preset in presets {
            let bucket_idx = Self::infer_provider_bucket_for_model(
                &preset,
                &provider_buckets,
                current_provider_idx,
            );
            grouped_presets[bucket_idx].push(preset);
        }

        let mut items: Vec<SelectionItem> = Vec::new();
        let mut initial_selected_idx: Option<usize> = None;
        for (bucket_idx, bucket) in provider_buckets.iter().enumerate() {
            let bucket_models = &grouped_presets[bucket_idx];
            if bucket_models.is_empty() {
                continue;
            }
            ...
            items.push(SelectionItem {
                ...
                description: None,
                is_disabled: true,
                search_value: Some(bucket.name.to_ascii_lowercase()),
                ..Default::default()
            });
            ...
            for preset in bucket_models {
                ...
                } else if initial_selected_idx.is_none() {
                    initial_selected_idx = Some(item_index);
                }
                items.push(SelectionItem {
                    name: preset.name.clone(),
    ...
    fn model_selection_actions(
        ...
        vec![Box::new(move |tx| {
            ...
            tx.send(AppEvent::UpdateModel(normalized_model.clone()));
    ...
    fn apply_model_and_effort(&self, model: String, effort: Option<ReasoningEffortConfig>) {
        let normalized_model =
            savfox_core::request_model_for_provider(&model, &self.config.model_provider_id);
        self.app_event_tx
    ...
    fn open_approval_mode_popup(&mut self, include_read_only: bool) {
        ...
        let mut items: Vec<SelectionItem> = Vec::new();
        let presets: Vec<PermissionPreset> = builtin_tui_presets();
        ...
        for preset in presets.into_iter() {
            ...
            let actions: Vec<SelectionAction> = if requires_confirmation {
                ...
                #[cfg(target_os = "windows")]
                {
                        ...
                        self.world_writable_warning_details()
                    {
                        ...
                        Self::approval_preset_actions(
                            preset.policy.approval,
                            preset.policy.sandbox.clone(),
                        )
                    }
                }
                #[cfg(not(target_os = "windows"))]
                ...
                Self::approval_preset_actions(preset.policy.approval, preset.policy.sandbox.clone())
            };
            items.push(SelectionItem {
                name,
                description,
    ...
    fn preset_matches_current(
        ...
        if current_approval != preset.policy.approval {
            return false;
        }
        matches!(
            (&preset.policy.sandbox, current_sandbox),
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        ...
        match savfox_windows_sandbox::apply_world_writable_scan_and_denies(
            self.config.savfox_home.as_path(),
            cwd.as_path(),
            &env_map,
            self.config.sandbox_policy.get(),
            Some(self.config.savfox_home.as_path()),
        ) {
            Ok(_) => None,
    ...
    pub(crate) fn open_full_access_confirmation(
        ...
        let approval = preset.policy.approval;
        let sandbox = preset.policy.sandbox;
        let mut header_children: Vec<Box<dyn Renderable>> = Vec::new();
        let title_line = Line::from("Enable full access?").bold();
        let info_line = Line::from(vec![
            "When Savfox runs with full access, it can edit any file on your computer and run commands with network, without your approval. "
                .into(),
    ...
    pub(crate) fn open_world_writable_warning_confirmation(
        ...
        let (approval, sandbox) = match &preset {
            Some(p) => (Some(p.policy.approval), Some(p.policy.sandbox.clone())),
            None => (None, None),
        };
        let mut header_children: Vec<Box<dyn Renderable>> = Vec::new();
        ...
        let mode_label = preset
            .as_ref()
            .map(|p| describe_policy(&p.policy.sandbox))
    ...
    pub(crate) fn open_world_writable_warning_confirmation(
        &mut self,
        _preset: Option<PermissionPreset>,
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(&mut self, preset: PermissionPreset) {
        ...
        self.otel_manager
            .counter("savfox.windows_sandbox.elevated_prompt_shown", 1, &[]);
        ...
        stay_actions.insert(
            ...
            Box::new({
                ...
                move |_tx| {
                    otel.counter("savfox.windows_sandbox.elevated_prompt_decline", 1, &[]);
                }
            }),
        );
    ...
    #[cfg(not(target_os = "windows"))]
    pub(crate) fn open_windows_sandbox_enable_prompt(&mut self, _preset: PermissionPreset) {}
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(
        ...
        lines.push(line![
            "Learn more: https://developers.openai.com/savfox/windows"
        ]);
        ...
        stay_actions.insert(
            ...
            Box::new({
                let otel = self.otel_manager.clone();
                move |_tx| {
                    otel.counter("savfox.windows_sandbox.fallback_stay_current", 1, &[]);
                }
            }),
        );
        let items = vec![
            SelectionItem {
                ...
                actions: vec![Box::new({
                    ...
                    move |tx| {
                        otel.counter("savfox.windows_sandbox.fallback_retry_elevated", 1, &[]);
                        tx.send(AppEvent::BeginWindowsSandboxElevatedSetup {
                            preset: preset.clone(),
                        });
                    }
            ...
            SelectionItem {
                ...
                actions: vec![Box::new({
                    ...
                    move |tx| {
                        otel.counter("savfox.windows_sandbox.fallback_use_legacy", 1, &[]);
                        tx.send(AppEvent::EnableWindowsSandboxForAgentMode {
                            preset: preset.clone(),
    ...
    fn refresh_model_display(&mut self) {
        ...
        self.bottom_pane.set_cwd_display(cwd);
    }

    /// Format the cwd for footer display (`~`-relative when inside HOME).
    fn cwd_display_name(&self) -> String {
    ...
    fn update_collaboration_mode_indicator(&mut self) {
        ...
        // Set border color based on collaboration mode
        let border_color = match indicator {
            Some(CollaborationModeIndicator::Plan) => Some(ratatui::style::Color::Magenta),
        ...
        for connector in connectors {
            ...
            let (selected_label, missing_label, instructions) = if connector.is_accessible {
                ...
                (
                    ...
                    "Install this app in your browser, then reload Savfox.",
                )
            };
    ...
    ///
    /// This keeps the state machine (`quit_shortcut_*`) in `ChatScreen`, since
    ...
    /// Forward an `Op` directly to savfox.
    pub(crate) fn submit_op(&mut self, op: Op) {
        ...
        if let Err(e) = self.savfox_op_tx.send(op) {
        ...
        for option in branches {
            ...
            items.push(SelectionItem {
                ...
                actions: vec![Box::new(move |tx3: &AppEventSender| {
                    tx3.send(AppEvent::SavfoxOp(Op::Review {
                        review_request: ReviewRequest {
    ...
    pub(crate) async fn show_review_commit_picker(&mut self, cwd: &Path) {
        let commits = savfox_core::git_info::recent_commits(cwd, 100).await;
    ...
    fn as_renderable(&self) -> RenderableItem<'_> {
        ...
        if self.show_welcome_banner {
            flex.push(0, RenderableItem::Borrowed(&self.startup_hero));
        }

        let bottom_pane_item: RenderableItem<'_> = RenderableItem::Borrowed(&self.bottom_pane);

        flex.push(0, bottom_pane_item.inset(Insets::tlbr(1, 0, 0, 0)));
...
impl Renderable for ChatScreen {
    ...
    fn desired_height(&self, width: u16) -> u16 {
        let active_height = self.active_cell.as_ref().map_or(0, |cell| {
            // Keep this in sync with the top inset applied in `as_renderable`.
            cell.desired_height(width).saturating_add(1)
        });
        let startup_height = if self.show_welcome_banner {
            self.startup_hero
                .desired_height(width)
                .saturating_add(STARTUP_BOTTOM_SPACER_LINES)
        } else {
            0
        };
        let bottom_height = self.bottom_pane.desired_height(width).saturating_add(1);

        active_height
            .saturating_add(startup_height)
...
fn load_provider_store_model_presets(savfox_home: &Path) -> Option<Vec<ModelPreset>> {
    ...
    let mut presets: Vec<ModelPreset> = Vec::new();
        ...
        let data = match std::fs::read_to_string(&path) {
            ...
            Err(err) => {
                debug!(
                    path = %path.display(),
                    error = ?err,
                    "failed to read provider model file"
                );
                continue;
            }
        };

        let models = parse_provider_store_models(&data);
        for model in models {
    ...
    if !presets
        .iter()
        .any(|preset| preset.show_in_picker && preset.is_default)
        && let Some(first_visible) = presets.iter_mut().find(|preset| preset.show_in_picker)
    {
        first_visible.is_default = true;
    }
...
fn value_reasoning_effort(
    ...
    for key in keys {
        if let Some(raw) = value.get(*key)
            && let Ok(effort) = serde_json::from_value::<ReasoningEffortConfig>(raw.clone())
        {
            return Some(effort);
        }
    }
...
async fn fetch_rate_limits(_base_url: String, _auth: SavfoxAuth) -> Option<RateLimitSnapshot> {
    None
}
...
pub(crate) fn show_review_commit_picker_with_entries(
    chat: &mut ChatScreen,
    entries: Vec<savfox_core::git_info::CommitLogEntry>,
) {
