...
use savfox_common::permission_presets::builtin_tui_presets;
...
use savfox_core::protocol::{
    ...
    ExecApprovalRequestEvent, ExecCommandBeginEvent, ExecCommandEndEvent, ExecCommandSource,
    ExecPolicyAmendment, ExitedReviewModeEvent, FileChange, McpStartupCompleteEvent,
    McpStartupStatus, McpStartupUpdateEvent, Op, PatchApplyBeginEvent, PatchApplyEndEvent,
    RateLimitWindow, ReviewRequest, ReviewTarget, SessionSource, StreamErrorEvent,
    TerminalInteractionEvent, TokenCountEvent, TokenUsage, TokenUsageInfo, TurnCompleteEvent,
    TurnStartedEvent, UndoCompletedEvent, UndoStartedEvent, ViewImageToolCallEvent, WarningEvent,
};
...
use crate::bottom_pane::{FeedbackAudience, LocalImageAttachment};
...
async fn test_config() -> Config {
    ...
    let savfox_home = std::env::temp_dir();
...
async fn resumed_initial_messages_render_history() {
    let (mut chat, mut rx, _ops) = make_chat_screen_manual(None).await;

    let conversation_id = SessionId::new();
    let rollout_file = NamedTempFile::new().unwrap();
    let configured = savfox_core::protocol::SessionConfiguredEvent {
...
async fn submission_preserves_text_elements_and_local_images() {
    let (mut chat, mut rx, mut op_rx) = make_chat_screen_manual(None).await;
...
#[tokio::test]
async fn first_submission_is_sent_even_before_session_configured() {
    let (mut chat, _rx, mut op_rx) = make_chat_screen_manual(None).await;
...
#[tokio::test]
async fn first_submission_keeps_startup_composer_style_and_bottom_alignment() {
    let (mut chat, _rx, mut op_rx) = make_chat_screen_manual(None).await;
    chat.set_feature_enabled(Feature::Steer, false);
    ...
    assert!(
        prompt_row > 0,
        "prompt should render below header/status rows after first submit:\n{rendered}"
    );

    assert!(
        rows.iter().any(|row| {
            row.contains("Type @ to mention files, / for commands, or ? for shortcuts")
        }),
        "expected startup placeholder text to remain visible after first submit:\n{rendered}"
    );

    let horizontal_rule_rows = rows.iter().filter(|row| row.contains('─')).count();
    assert!(
        horizontal_rule_rows >= 2,
        "expected startup-style horizontal rules around the textarea:\n{rendered}"
    );
}
...
async fn interrupted_turn_restores_queued_messages_with_images_and_elements() {
    ...
    chat.handle_savfox_event(Event {
        id: "interrupt".into(),
        msg: EventMsg::TurnAborted(savfox_core::protocol::TurnAbortedEvent {
...
async fn token_count_none_resets_context_indicator() {
    let (mut chat, _rx, _ops) = make_chat_screen_manual(None).await;
...
async fn context_indicator_shows_used_tokens_when_window_unknown() {
    let (mut chat, _rx, _ops) = make_chat_screen_manual(Some("unknown-model")).await;
...
async fn helpers_are_available_and_do_not_panic() {
    ...
    let mut w = ChatScreen::new(init, session_manager);
    // Basic construction sanity.
    let _ = &mut w;
    let area = Rect::new(0, 0, 100, 20);
    let mut buf = Buffer::empty(area);
    w.render(area, &mut buf);
    let startup_header = (0..area.height)
        .map(|y| {
            let mut row = String::new();
            for x in 0..area.width {
                row.push(buf[(x, y)].symbol().chars().next().unwrap_or(' '));
        ...
        .join("\n");
    assert!(
        startup_header.contains(expected_startup_model.as_str()),
        "expected startup header model to be initialized: {startup_header}"
    );
    assert!(
        !startup_header.contains("model: loading"),
        "startup header should not show loading placeholder: {startup_header}"
    );
}
...
async fn make_chat_screen_manual(
    ...
    let auth_manager = AuthManager::from_auth_for_testing(SavfoxAuth::from_api_key("test"));
    let savfox_home = cfg.savfox_home.clone();
    let models_manager = Arc::new(ModelsManager::new(savfox_home, auth_manager.clone()));
...
// ChatScreen may emit other `Op`s (e.g. history/logging updates) on the same channel; this helper
...
fn set_chatgpt_auth(chat: &mut ChatScreen) {
    chat.auth_manager =
...
pub(crate) async fn make_chat_screen_manual_with_sender() -> (
    ChatScreen,
    AppEventSender,
    tokio::sync::mpsc::UnboundedReceiver<AppEvent>,
    tokio::sync::mpsc::UnboundedReceiver<Op>,
) {
    let (widget, rx, op_rx) = make_chat_screen_manual(None).await;
...
async fn rate_limit_switch_prompt_skips_when_on_lower_cost_model() {
    let (mut chat, ..) = make_chat_screen_manual(Some(NUDGE_MODEL_SLUG)).await;
    chat.auth_manager =
        AuthManager::from_auth_for_testing(SavfoxAuth::create_dummy_chatgpt_auth_for_testing());
...
async fn plan_implementation_popup_yes_emits_submit_message_event() {
    let (mut chat, mut rx, _op_rx) = make_chat_screen_manual(Some("gpt-5")).await;
    chat.open_plan_implementation_prompt();
...
async fn submit_user_message_with_mode_sets_coding_collaboration_mode() {
    let (mut chat, _rx, mut op_rx) = make_chat_screen_manual(Some("gpt-5")).await;
...
fn begin_exec(chat: &mut ChatScreen, call_id: &str, raw_cmd: &str) -> ExecCommandBeginEvent {
...
fn active_blob(chat: &ChatScreen) -> String {
    let lines = chat
        .active_cell
        .as_ref()
...
async fn slash_init_skips_when_project_doc_exists() {
    ...
    match op_rx.try_recv() {
        ...
        other => panic!("expected no Savfox op to be sent, got {other:?}"),
    }
...
async fn collaboration_modes_defaults_to_code_on_startup() {
    ...
    let chat = ChatScreen::new(init, session_manager);
    assert_eq!(chat.active_collaboration_mode_kind(), ModeKind::Code);
...
async fn set_model_updates_active_collaboration_mask() {
    let (mut chat, _rx, _op_rx) = make_chat_screen_manual(Some("gpt-5.1")).await;
    chat.set_feature_enabled(Feature::CollaborationModes, true);
    let plan_mask =
        collaboration_modes::mask_for_kind(chat.models_manager.as_ref(), ModeKind::Plan)
            .expect("expected plan collaboration mask");
    ...
    chat.set_model("gpt-5.1-savfox-mini");

    assert_eq!(chat.current_model(), "gpt-5.1-savfox-mini");
    assert_eq!(chat.active_collaboration_mode_kind(), ModeKind::Plan);
}
...
#[tokio::test]
async fn slash_connect_opens_provider_picker() {
    let (mut chat, _rx, _op_rx) = make_chat_screen_manual(None).await;

    chat.dispatch_command(SlashCommand::Connect);

    let popup = render_bottom_popup(&chat, 100);
    assert!(
        popup.contains("Connect Model Provider"),
        "expected connect popup title: {popup}"
    );
    assert!(
        popup.contains("OpenAI"),
        "expected OpenAI provider in connect popup: {popup}"
    );
}
...
async fn connect_auth_prompt_ollama_starts_without_api_key() {
    ...
    while let Ok(event) = rx.try_recv() {
        if let AppEvent::BeginProviderConnect {
            provider_id,
            api_key,
        } = event
        {
            begin_connect = Some((provider_id, api_key));
            break;
        }
    }
...
async fn connect_auth_prompt_openai_shows_auth_method_picker() {
    ...
    let popup = render_bottom_popup(&chat, 100);
    assert!(
        popup.contains("Select auth method"),
...
async fn connect_auth_prompt_openai_headless_emits_auth_event() {
    ...
    let mut headless_auth: Option<(String, OpenAiConnectAuthMethod)> = None;
    while let Ok(event) = rx.try_recv() {
        if let AppEvent::BeginOpenAiConnectAuth {
            provider_id,
            method,
        } = event
        {
            headless_auth = Some((provider_id, method));
            break;
        }
    }
...
async fn slash_rollout_displays_current_path() {
    let (mut chat, mut rx, _op_rx) = make_chat_screen_manual(None).await;
    let rollout_path = PathBuf::from("/tmp/savfox-test-rollout.jsonl");
...
async fn custom_prompt_submit_sends_review_op() {
    ...
    match evt {
        AppEvent::SavfoxOp(Op::Review { review_request }) => {
            assert_eq!(
                review_request,
                ReviewRequest {
...
async fn custom_prompt_enter_empty_does_not_send() {
    ...
    // No AppEvent::SavfoxOp should be sent
...
async fn review_custom_prompt_escape_navigates_back_then_dismisses() {
    ...
    let header = render_bottom_popup(&chat, 60);
    assert!(
...
fn render_bottom_popup(chat: &ChatScreen, width: u16) -> String {
    let height = chat.desired_height(width);
...
async fn model_selection_popup_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chat_screen_manual(Some("gpt-5-savfox")).await;
    chat.session_id = Some(SessionId::new());
...
#[tokio::test]
async fn model_popup_prefers_provider_store_models() {
    ...
    std::fs::write(
        models_dir.join("provider-a.json"),
        r#"{
  "version": 2,
  ...
  "disabled_models": []
    ...
    std::fs::write(
        models_dir.join("provider-b.json"),
        r#"{
  "version": 2,
  ...
  "disabled_models": []
    ...
    let popup = render_bottom_popup(&chat, 100);
    assert!(
        popup.contains("provider-a"),
        "expected provider-a group in popup:\n{popup}"
    );
    assert!(
        popup.contains("provider-b"),
    ...
    assert!(
        !popup.contains("Hidden Model"),
        "disabled models should not be listed:\n{popup}"
    );
    assert!(
...
#[test]
fn provider_store_loader_parses_legacy_array_files() {
    let savfox_home = tempdir().expect("tempdir");
    let models_dir = savfox_home.path().join("models");
    std::fs::create_dir_all(&models_dir).expect("create models dir");
    std::fs::write(
...
#[test]
fn provider_store_loader_returns_none_when_models_dir_missing() {
    let savfox_home = tempdir().expect("tempdir");
    let presets = load_provider_store_model_presets(savfox_home.path());
    assert!(
        presets.is_none(),
        "missing models dir should trigger fallback to models manager"
    );
}

#[tokio::test]
async fn model_cap_error_does_not_switch_models() {
    let (mut chat, mut rx, mut op_rx) = make_chat_screen_manual(Some("boomslang")).await;
...
async fn preset_matching_ignores_extra_writable_roots() {
    let preset = builtin_tui_presets()
        .into_iter()
        .find(|p| p.id == "auto")
    ...
    assert!(
        ChatScreen::preset_matches_current(AskForApproval::OnRequest, &current_sandbox, &preset),
        "WorkspaceWrite with extra roots should still match the Agent preset"
    );
    assert!(
        !ChatScreen::preset_matches_current(AskForApproval::Never, &current_sandbox, &preset),
...
async fn reasoning_popup_escape_returns_to_model_popup() {
    ...
    assert!(
        after_escape.to_ascii_lowercase().contains("select model"),
        "expected model popup after Esc, got: {after_escape:?}"
    );
...
async fn approvals_popup_navigation_skips_disabled() {
    ...
    // The approvals popup is the active bottom-pane view; drive navigation via chat
    // handle_key_event. Start selected at idx 0 (enabled), move down twice; the disabled option
    // should be skipped and selection should wrap back to idx 0 (also enabled).
    chat.handle_key_event(KeyEvent::from(KeyCode::Down));
    chat.handle_key_event(KeyEvent::from(KeyCode::Down));
    ...
    assert!(
        app_events.iter().any(|ev| matches!(
            ev,
            AppEvent::SavfoxOp(Op::OverrideTurnContext {
...
//
// Synthesizes a Savfox ExecApprovalRequest event to trigger the approval modal
...
// Snapshot test: ChatScreen at very small heights (idle)
// Ensures overall layout behaves when terminal height is extremely constrained.
#[tokio::test]
async fn ui_snapshots_small_heights_idle() {
    use ratatui::Terminal;
    use ratatui::backend::TestBackend;
    let (chat, _rx, _op_rx) = make_chat_screen_manual(None).await;
    for h in [1u16, 2, 3] {
...
// Snapshot test: ChatScreen at very small heights (task running)
...
async fn apply_patch_approval_sends_op_with_submission_id() {
    ...
    // Expect a SavfoxOp with PatchApproval carrying the submission id, not call id
    let mut found = false;
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::SavfoxOp(Op::PatchApproval { id, decision }) = app_ev {
            assert_eq!(id, "sub-123");
            assert_matches!(decision, savfox_core::protocol::ReviewDecision::Approved);
            found = true;
            break;
        }
    }
...
async fn apply_patch_full_flow_integration_like() {
    ...
    // 2) User approves via 'y' and App receives a SavfoxOp
    chat.handle_key_event(KeyEvent::new(KeyCode::Char('y'), KeyModifiers::NONE));
    let mut maybe_op: Option<Op> = None;
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::SavfoxOp(op) = app_ev {
            maybe_op = Some(op);
            break;
        }
    }
    let op = maybe_op.expect("expected SavfoxOp after key press");

    // 3) App forwards to widget.submit_op, which pushes onto savfox_op_tx
    chat.submit_op(op);
    let forwarded = op_rx
        .try_recv()
        .expect("expected op forwarded to savfox channel");
    match forwarded {
...
#[tokio::test]
async fn chat_screen_exec_and_status_layout_vt100_snapshot() {
    let (mut chat, mut rx, _op_rx) = make_chat_screen_manual(None).await;
    chat.handle_savfox_event(Event {
        id: "t1".into(),
...
#[tokio::test]
async fn chat_screen_markdown_code_blocks_vt100_snapshot() {
    let (mut chat, mut rx, _op_rx) = make_chat_screen_manual(None).await;
