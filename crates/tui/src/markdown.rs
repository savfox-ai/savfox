...
mod tests {
    ...
    fn indented_code_blocks_preserve_leading_whitespace() {
        // Basic sanity: indented code with surrounding blank lines should produce the indented
        // line.
        let src = "Before\n\n    code 1\n\nAfter\n";
        let mut out = Vec::new();
        append_markdown(src, None, &mut out);
    ...
    fn append_markdown_preserves_full_text_line() {
        let src = "Hi! How can I help with savfox-rs today? Want me to explore the repo, run tests, or work on a specific change?\n";
        let mut out = Vec::new();
        append_markdown(src, None, &mut out);
        assert_eq!(
            out.len(),
        ...
        assert_eq!(
            rendered,
            "Hi! How can I help with savfox-rs today? Want me to explore the repo, run tests, or work on a specific change?"
        );
    }
