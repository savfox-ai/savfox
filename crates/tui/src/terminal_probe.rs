...
//!
//! Crossterm's public helpers may wait longer than we want during startup. These probes send the
//! optional terminal queries with a caller-provided deadline and return `None` when no response is
//! available.
...
fn parse_osc_color(buffer: &[u8], slot: u8) -> Option<(u8, u8, u8)> {
    ...
    let (payload_end, _) = osc_payload_end(rest)?;
