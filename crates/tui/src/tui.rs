...
use crate::notifications::{DesktopNotificationBackend, detect_backend};
use crate::tui::event_stream::{EventBroker, TuiEventStream};
#[cfg(unix)]
...
impl Tui {
    ...
    /// terminal modes (optionally keeping raw mode enabled), then re-applies Savfox TUI modes and
        ...
        Fut: Future<Output = R>,
    {
        ...
        // After the external program `f` finishes, reset terminal state and flush any buffered
        // keypresses.
    ...
    pub fn draw(
        ...
        stdout().sync_update(|_| {
            ...
            if area != terminal.viewport_area {
                // TODO(nornagon): probably this could be collapsed with the clear +
                // set_viewport_area above.
                terminal.clear()?;
