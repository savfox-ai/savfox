    ...
    async fn handle_event(&mut self, tui: &mut tui::Tui, event: AppEvent) -> Result<AppRunControl> {
        match event {
            ...
            AppEvent::ProviderConnectCompleted(result) => {
                ...
                self.chat_screen.add_info_message(
                    format!(
                        "Connected to {} ({} model(s) found). Please name this account.",
                        result.provider_name,
                        result.models.len()
                    ),
                    None,
                );
                self.chat_screen.show_provider_naming_prompt(result, None);
            }
            AppEvent::ProviderConnectNamed {
                result,
                account_name,
            } => {
                use savfox_core::config::provider_store::{account_id_exists, slugify_account_id};

                // Compute the final account_id from the chosen name.
                let account_id = if account_name.is_empty() {
                    result.provider_id.clone()
                } else {
                    slugify_account_id(&result.provider_id, &account_name)
                };

                // Check for conflicts.
                if account_id != result.provider_id
                ...
                // Persist the provider connection with the final account_id.
                ...
                // Look up the base provider info, then register a virtual
                // entry under the account_id for multi-account support.
                let Some(mut provider_info) = self
                    ...
                    self.chat_screen.add_error_message(format!(
                        "Connected provider `{}` is not configured in this build.",
                        result.provider_id
                    ));
                    return Ok(AppRunControl::Continue);
                };
                ...
                self.config
                    .model_providers
                    .insert(account_id.clone(), provider_info.clone());
                self.config.model_provider_id = account_id.clone();
                self.config.model_provider = provider_info.clone();
                self.chat_screen
                    .set_model_provider(account_id.as_str(), provider_info);

                let Some(default_model) = select_default_model(&result.models, account_id.as_str())
                else {
                    self.chat_screen.add_error_message(format!(
                ...
                // The model may carry an old provider prefix from the initial
                // connect (before the user named the account). Strip any
                ...
                let model_to_persist = format!("{account_id}/{}", normalized_model.trim());
                ...
                self.chat_screen
                    .open_connected_provider_models_popup(account_id.as_str());
            }
            ...
            AppEvent::BeginWindowsSandboxElevatedSetup { preset } => {
                #[cfg(target_os = "windows")]
                {
                    let policy = preset.policy.sandbox.clone();
                        ...
                        Ok(()) => {
                                ...
                                self.chat_screen.world_writable_warning_details()
                            {
                                ...
                                self.app_event_tx.send(AppEvent::SavfoxOp(
                                    Op::OverrideTurnContext {
                                        cwd: None,
                                        approval_policy: Some(preset.policy.approval),
                                        sandbox_policy: Some(preset.policy.sandbox.clone()),
                                        windows_sandbox_level: Some(windows_sandbox_level),
                                        model: None,
                                        effort: None,
                                        summary: None,
