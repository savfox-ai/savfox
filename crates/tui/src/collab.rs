...
pub(crate) fn interaction_end(ev: CollabAgentInteractionEndEvent) -> PlainHistoryCell {
    ...
    let mut details = vec![
        detail_line("call", call_id),
        detail_line("receiver", receiver_session_id.to_string()),
        status_line(&status),
    ];
    if let Some(line) = prompt_line(&prompt) {
        details.push(line);
...
pub(crate) fn waiting_begin(ev: CollabWaitingBeginEvent) -> PlainHistoryCell {
    ...
    let details = vec![
        detail_line("call", call_id),
        detail_line("receivers", format_session_ids(&receiver_session_ids)),
    ];
...
fn format_session_ids(ids: &[SessionId]) -> Span<'static> {
    if ids.is_empty() {
...
fn wait_complete_lines(statuses: &HashMap<SessionId, AgentStatus>) -> Vec<Line<'static>> {
    ...
    lines.extend(entries.into_iter().map(|(session_id, status)| {
        let mut spans = vec![
            Span::from(session_id).dim(),
            Span::from(" ").dim(),
            status_span(status),
        ];
        match status {
