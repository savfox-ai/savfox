...
use savfox_core::protocol::{
    CreditsSnapshot, RateLimitSnapshot, RateLimitWindow, SandboxPolicy, TokenUsage, TokenUsageInfo,
};
...
async fn test_config(temp_home: &TempDir) -> Config {
    ConfigBuilder::default()
        .savfox_home(temp_home.path().to_path_buf())
        .build()
        .await
        .expect("load config")
}
...
async fn status_snapshot_includes_forked_from() {
    ...
    let token_info = token_info_for(&model_slug, &config, &usage);
    let session_id =
        SessionId::from_string("0f0f3c13-6cf9-4aa4-8b80-7d49c2f1be2e").expect("session id");
    let forked_from =
        SessionId::from_string("e9f18a88-8081-4e51-9d4e-8af5cde2d8dd").expect("forked id");
