...
use savfox_utils::cargo_bin::{CargoBinError, find_resource};
...
#[path = "test_agent.rs"]
pub mod test_savfox;
#[path = "test_agent_exec.rs"]
pub mod test_savfox_exec;
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\savfox\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.savfox`.
pub async fn load_default_config_for_test(savfox_home: &TempDir) -> Config {
...
pub async fn wait_for_event<F>(
    savfox: &SavfoxSession,
    predicate: F,
) -> savfox_core::protocol::EventMsg
where
    F: FnMut(&savfox_core::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(savfox, predicate, Duration::from_secs(1)).await
}

pub async fn wait_for_event_match<T, F>(savfox: &SavfoxSession, matcher: F) -> T
where
    F: Fn(&savfox_core::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(savfox, |ev| matcher(ev).is_some()).await;
    matcher(&ev).unwrap()
}
    ...
    F: FnMut(&savfox_core::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(10)), savfox.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    savfox_core::spawn::SAVFOX_SANDBOX_ENV_VAR
}
...
pub fn format_with_current_shell(command: &str) -> Vec<String> {
    savfox_core::shell::default_user_shell().derive_exec_args(command, true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    savfox_core::shell::default_user_shell().derive_exec_args(command, false)
}
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    savfox_utils::cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
