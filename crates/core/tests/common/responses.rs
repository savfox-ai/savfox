...
use crate::test_savfox::ApplyPatchModelOutput;
...
/// - Every `function_call_output` must match a prior `function_call` or `local_shell_call` with the
///   same `call_id` in the same `input`.
/// - Every `custom_tool_call_output` must match a prior `custom_tool_call`.
/// - Additionally, enforce symmetry: every `function_call`/`custom_tool_call` in the `input` must
///   have a matching output entry.
