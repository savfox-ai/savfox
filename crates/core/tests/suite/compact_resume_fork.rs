...
//! request payload that Savfox would send to the model and assert that the
//! model-visible history matches the expected sequence of messages.

use std::sync::Arc;

use core_test_support::responses::{
    ResponseMock, ev_assistant_message, ev_completed, mount_sse_once_match, sse,
};
...
use super::compact::{COMPACT_WARNING_MESSAGE, FIRST_REPLY, SUMMARY_TEXT};

const AFTER_SECOND_RESUME: &str = "AFTER_SECOND_RESUME";

fn network_disabled() -> bool {
    std::env::var(SAVFOX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn compact_resume_and_fork_preserve_model_history_view() {
    ...
    let expected_model = "gpt-5.1-savfox";
    ...
    let forked = fork_session(&manager, &config, resumed_path, 2).await;
...
async fn compact_resume_after_second_compaction_preserves_history() {
    ...
    let forked = fork_session(&manager, &config, resumed_path, 3).await;
    user_turn(&forked, "AFTER_FORK").await;
...
async fn start_test_conversation(
    ...
    (test.home, test.config, test.session_manager, test.savfox)
}

async fn user_turn(conversation: &Arc<SavfoxSession>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<SavfoxSession>) {
    conversation
        .submit(Op::Compact)
        .await
...
async fn fetch_conversation_path(conversation: &Arc<SavfoxSession>) -> std::path::PathBuf {
...
async fn resume_conversation(
    ...
    let auth_manager = savfox_core::AuthManager::from_auth_for_testing(
        savfox_core::SavfoxAuth::from_api_key("dummy"),
    );
    manager
...
async fn fork_session(
    ...
    manager
        .fork_session(nth_user_message, config.clone(), path)
        .await
