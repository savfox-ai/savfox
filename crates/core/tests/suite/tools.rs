...
use core_test_support::responses::{
    ev_assistant_message, ev_completed, ev_custom_tool_call, ev_function_call, ev_response_created,
...
async fn shell_escalated_permissions_rejected_then_ok() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder = test_savfox().with_model("gpt-5");
    let test = builder.build(&server).await?;
...
async fn shell_spawn_failure_truncates_exec_error() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder = test_savfox().with_config(|cfg| {
        cfg.sandbox_policy
            .set(SandboxPolicy::DangerFullAccess)
            .expect("set sandbox policy");
    });
