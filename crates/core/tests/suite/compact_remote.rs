...
use savfox_core::protocol::{
    EventMsg, ItemCompletedEvent, ItemStartedEvent, Op, RolloutItem, RolloutLine,
};
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestSavfoxHarness::with_builder(
        test_savfox()
    ...
    savfox.submit(Op::Compact).await?;
    wait_for_event(&savfox, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&savfox, |event| match event {
