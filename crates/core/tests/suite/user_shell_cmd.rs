...
use core_test_support::responses::{
    ev_assistant_message, ev_completed, ev_function_call, ev_response_created, mount_sse_once, sse,
    start_mock_server,
};
...
async fn user_shell_cmd_ls_and_cat_in_temp_dir() {
    ...
    savfox
        ...
        .await
        .unwrap();
    let msg = wait_for_event(&savfox, |ev| matches!(ev, EventMsg::ExecCommandEnd(_))).await;
...
async fn user_shell_cmd_can_be_interrupted() {
    ...
    let _ = wait_for_event(&savfox, |ev| matches!(ev, EventMsg::ExecCommandBegin(_))).await;
    savfox.submit(Op::Interrupt).await.unwrap();

    // Expect a TurnAborted(Interrupted) notification.
    let msg = wait_for_event(&savfox, |ev| matches!(ev, EventMsg::TurnAborted(_))).await;
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    let mut builder = core_test_support::test_savfox::test_savfox().with_config(move |config| {
    ...
    let begin_event = wait_for_event_match(&test.savfox, |ev| match ev {
    ...
    let delta_event = wait_for_event_match(&test.savfox, |ev| match ev {
    ...
    let end_event = wait_for_event_match(&test.savfox, |ev| match ev {
    ...
    let _ = wait_for_event(&test.savfox, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn user_shell_command_output_is_truncated_in_history() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let builder = core_test_support::test_savfox::test_savfox();
    let test = builder
        .with_config(|config| {
