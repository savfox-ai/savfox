...
use core_test_support::responses::{
    ev_assistant_message, ev_completed, ev_function_call, ev_response_created, mount_sse_once,
    mount_sse_sequence, sse, start_mock_server,
};
use core_test_support::test_savfox::test_savfox;
use core_test_support::{
    assert_regex_match, responses, skip_if_no_network, stdio_server_bin, wait_for_event,
};
...
async fn truncate_function_error_trims_respond_to_model() -> Result<()> {
    ...
    let mut builder = test_savfox().with_model("test-gpt-5.1-savfox");
...
async fn tool_call_output_configured_limit_chars_type() -> Result<()> {
    ...
    let mut builder = test_savfox().with_model("gpt-5.1").with_config(|config| {
...
async fn tool_call_output_exceeds_limit_truncated_for_model() -> Result<()> {
    ...
    // Use a model that exposes the shell_command tool.
    let mut builder = test_savfox().with_model("gpt-5.1-savfox");
    let fixture = builder.build(&server).await?;
...
async fn mcp_tool_call_output_exceeds_limit_truncated_for_model() -> Result<()> {
    ...
    let mut builder = test_savfox().with_config(move |config| {
        ...
        servers.insert(
            ...
            savfox_core::config::types::McpServerConfig {
                transport: savfox_core::config::types::McpServerTransportConfig::Stdio {
...
async fn mcp_image_output_preserves_image_and_no_text_summary() -> Result<()> {
    ...
    // Build the stdio rmcp server and pass a tiny PNG via data URL so it can construct
    // ImageContent.
    let rmcp_test_server_bin = stdio_server_bin()?;
    ...
    wait_for_event(&fixture.savfox, |ev| {
