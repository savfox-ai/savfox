...
use core_test_support::{
    responses, skip_if_no_network, wait_for_event, wait_for_event_with_timeout,
};
...
async fn view_image_tool_attaches_local_image() -> anyhow::Result<()> {
    ...
    wait_for_event_with_timeout(
        ...
        |event| match event {
            EventMsg::ItemStarted(event) => {
                if matches!(&event.item, savfox_protocol::items::TurnItem::ImageView(_)) {
    ...
    match item_started.expect("view image item started event emitted") {
        savfox_protocol::items::TurnItem::ImageView(item) => {
        ...
        .find_map(|item| {
            ...
            content.iter().find_map(|span| {
                if span.get("type").and_then(Value::as_str) == Some("input_text") {
                    ...
                    if text.contains("Savfox could not read the local image at")
