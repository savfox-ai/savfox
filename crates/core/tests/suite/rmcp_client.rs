...
use core_test_support::{responses, skip_if_no_network, stdio_server_bin, wait_for_event};
use savfox_core::config::types::{McpServerConfig, McpServerTransportConfig};
use savfox_core::protocol::{
    AskForApproval, EventMsg, McpInvocation, McpToolCallBeginEvent, Op, SandboxPolicy,
};
use savfox_protocol::config_types::ReasoningSummary;
use savfox_protocol::user_input::UserInput;
use savfox_utils::cargo_bin::cargo_bin;
...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.savfox, |ev| {
    ...
    let end_event = wait_for_event(&fixture.savfox, |ev| {
...
async fn streamable_http_with_oauth_round_trip() -> anyhow::Result<()> {
    ...
    let _guard = EnvVarGuard::set("SAVFOX_HOME", temp_home.path().as_os_str());
