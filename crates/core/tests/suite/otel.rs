...
use core_test_support::responses::{
    ...
    ev_message_item_added, ev_output_text_delta, ev_reasoning_item,
    ev_reasoning_summary_text_delta, ev_reasoning_text_delta, ev_response_created,
    mount_response_once, mount_sse_once, sse, sse_response, start_mock_server,
};
...
use savfox_protocol::protocol::{AskForApproval, EventMsg, Op, ReviewDecision, SandboxPolicy};
...
async fn responses_api_emits_api_request_event() {
    ...
    let TestSavfox { savfox, .. } = test_savfox().build(&server).await.unwrap();
...
async fn process_sse_emits_tracing_for_output_item() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| {
                line.contains("savfox.sse_event")
...
async fn handle_response_item_records_tool_result_for_custom_tool_call() {
    ...
    wait_for_event(&savfox, |ev| matches!(ev, EventMsg::TokenCount(_))).await;
...
async fn handle_response_item_records_tool_result_for_local_shell_call() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| line.contains("savfox.tool_result") && line.contains("call_id=shell-call"))
...
fn tool_decision_assertion<'a>(
    ...
    move |lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("savfox.tool_decision")
                    && line.contains(&format!("call_id={call_id}"))
            })
            .ok_or_else(|| format!("missing savfox.tool_decision event for {call_id}"))?;
...
async fn handle_container_exec_user_approved_records_tool_decision() {
    ...
    wait_for_event(&savfox, |ev| matches!(ev, EventMsg::ExecApprovalRequest(_))).await;
