...
async fn undo_harness() -> Result<TestSavfoxHarness> {
    let builder = test_savfox().with_model("gpt-5.1").with_config(|config| {
        config.include_apply_patch_tool = true;
        config.features.enable(Feature::GhostCommit);
    });
    TestSavfoxHarness::with_builder(builder).await
}
...
fn init_git_repo(path: &Path) -> Result<()> {
    ...
    git(path, &["config", "user.name", "Savfox Tests"])?;
    git(path, &["config", "user.email", "savfox-tests@example.com"])?;

    // Create README.txt
    let readme_path = path.join("README.txt");
    fs::write(&readme_path, "Test repository initialized by Savfox.\n")?;
...
async fn invoke_undo(savfox: &Arc<SavfoxSession>) -> Result<UndoCompletedEvent> {
    savfox.submit(Op::Undo).await?;
    let event = wait_for_event_match(savfox, |msg| match msg {
...
async fn expect_successful_undo(savfox: &Arc<SavfoxSession>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(savfox).await?;
    assert!(
        event.success,
...
async fn expect_failed_undo(savfox: &Arc<SavfoxSession>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(savfox).await?;
    assert!(
        !event.success,
...
async fn undo_removes_new_file_created_during_turn() -> Result<()> {
    ...
    let savfox = Arc::clone(&harness.test().savfox);
    let completed = expect_successful_undo(&savfox).await?;
...
async fn undo_sequential_turns_consumes_snapshots() -> Result<()> {
    ...
    let savfox = Arc::clone(&harness.test().savfox);
    expect_successful_undo(&savfox).await?;
    assert_eq!(fs::read_to_string(&story)?, "turn two\n");
    ...
    expect_failed_undo(&savfox).await?;
...
async fn undo_preserves_unrelated_staged_changes() -> Result<()> {
    ...
    // The second column will likely be 'M' because the worktree was reverted to v1 while index has
    // v2. So "MM user_file.txt" is expected.
