...
use core_test_support::responses::{
    ev_assistant_message, ev_completed, ev_completed_with_tokens, ev_function_call,
    ev_local_shell_call, ev_reasoning_item, mount_compact_json_once, mount_response_sequence,
    mount_sse_once, mount_sse_once_match, mount_sse_sequence, sse, sse_failed, sse_response,
    start_mock_server,
};
...
use savfox_core::protocol::{
    AskForApproval, EventMsg, ItemCompletedEvent, ItemStartedEvent, Op, RolloutItem, RolloutLine,
    SandboxPolicy, WarningEvent,
};
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn Savfox.
    ...
    savfox.submit(Op::Compact).await.unwrap();
    let warning_event = wait_for_event(&savfox, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    // Third request must contain the refreshed instructions, compacted user history, and new user
    // message.
    ...
    // Shut down Savfox to flush rollout entries before inspecting the file.
    savfox.submit(Op::Shutdown).await.unwrap();
    wait_for_event(&savfox, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn manual_compact_uses_custom_prompt() {
    ...
    savfox.submit(Op::Compact).await.expect("trigger compact");
    let warning_event = wait_for_event(&savfox, |ev| matches!(ev, EventMsg::Warning(_))).await;
    let EventMsg::Warning(WarningEvent { message }) = warning_event else {
        panic!("expected warning event after compact");
    };
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let savfox = builder.build(&server).await.unwrap().savfox;
    ...
    let first = wait_for_event_match(&savfox, |ev| match ev {
    ...
    let last = wait_for_event_match(&savfox, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn manual_compact_emits_context_compaction_items() {
    ...
    while !saw_turn_complete || started_item.is_none() || completed_item.is_none() {
        let event = savfox.next_event().await.unwrap();
        match event.msg {
...
async fn multiple_auto_compact_per_task_runs_after_token_limit_hit() {
    ...
    // test 1: after compaction, we should have one environment message, one user message, and one
    // user message with summary prefix
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let mut resume_builder = test_savfox().with_config(move |config| {
        set_test_compact_prompt(config);
        config.model_auto_compact_token_limit = Some(limit);
        config.features.enable(Feature::RemoteCompaction);
    });
...
async fn manual_compact_retries_after_context_window_error() {
    ...
    let EventMsg::BackgroundEvent(event) =
        wait_for_event(&savfox, |ev| matches!(ev, EventMsg::BackgroundEvent(_))).await
    else {
        panic!("expected background event after compact retry");
    };
    assert!(
        event.message.contains("Trimmed 1 older session item"),
