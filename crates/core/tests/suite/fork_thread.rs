...
async fn fork_session_twice_drops_to_first_message() {
    ...
    let session_manager = test.session_manager.clone();
    ...
    let base_path = savfox.rollout_path().expect("rollout path");
    ...
    let find_user_input_positions = |items: &[RolloutItem]| -> Vec<usize> {
        ...
        for (i, it) in items.iter().enumerate() {
            ...
            {
                // Consider any user message as an input boundary; recorder stores both EventMsg and
                // ResponseItem. We specifically look for input items, which are
                // represented as ContentItem::InputText.
                pos.push(i);
            }
        }
        pos
    ...
    let NewSession {
        session: savfox_fork1,
        ..
    } = session_manager
        .fork_session(1, config_for_fork.clone(), base_path.clone())
        .await
        .expect("fork 1");

    let fork1_path = savfox_fork1.rollout_path().expect("rollout path");
    ...
    let NewSession {
        session: savfox_fork2,
        ..
    } = session_manager
        .fork_session(0, config_for_fork.clone(), fork1_path.clone())
        .await
        .expect("fork 2");

    let fork2_path = savfox_fork2.rollout_path().expect("rollout path");
