...
const MODEL_WITH_TOOL: &str = "test-gpt-5.1-savfox";
...
async fn grep_files_tool_collects_matches() -> Result<()> {
    ...
    let test = build_test_savfox(&server).await?;
...
async fn build_test_savfox(server: &wiremock::MockServer) -> Result<TestSavfox> {
    let mut builder = test_savfox().with_model(MODEL_WITH_TOOL);
