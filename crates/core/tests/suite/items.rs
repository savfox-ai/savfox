...
use core_test_support::responses::{
    ...
    ev_reasoning_text_delta, ev_response_created, ev_web_search_call_added_partial,
    ev_web_search_call_done, mount_sse_once, sse, start_mock_server,
};
...
async fn user_message_item_is_emitted() -> anyhow::Result<()> {
    ...
    let started_item = wait_for_event_match(&savfox, |ev| match ev {
    ...
    .await;
    let completed_item = wait_for_event_match(&savfox, |ev| match ev {
    ...
    let legacy_message = wait_for_event_match(&savfox, |ev| match ev {
...
async fn assistant_message_item_is_emitted() -> anyhow::Result<()> {
    ...
    let started = wait_for_event_match(&savfox, |ev| match ev {
        EventMsg::ItemStarted(ItemStartedEvent {
    ...
    .await;
    let completed = wait_for_event_match(&savfox, |ev| match ev {
        EventMsg::ItemCompleted(ItemCompletedEvent {
            item: TurnItem::AgentMessage(item),
            ..
        }) => Some(item.clone()),
    ...
    let Some(savfox_protocol::items::AgentMessageContent::Text { text }) =
...
async fn web_search_item_is_emitted() -> anyhow::Result<()> {
    ...
    let begin = wait_for_event_match(&savfox, |ev| match ev {
...
async fn agent_message_content_delta_has_item_metadata() -> anyhow::Result<()> {
    ...
    let (started_turn_id, started_item) = wait_for_event_match(&savfox, |ev| match ev {
        EventMsg::ItemStarted(ItemStartedEvent {
            turn_id,
            item: TurnItem::AgentMessage(item),
            ..
    ...
    let delta_event = wait_for_event_match(&savfox, |ev| match ev {
        EventMsg::AgentMessageContentDelta(event) => Some(event.clone()),
        _ => None,
    })
    .await;
    let legacy_delta = wait_for_event_match(&savfox, |ev| match ev {
    ...
    assert_eq!(delta_event.session_id, session_id);
...
async fn plan_mode_emits_plan_item_from_proposed_plan_block() -> anyhow::Result<()> {
    ...
    savfox
        .submit(Op::UserTurn {
            ...
            sandbox_policy: savfox_core::protocol::SandboxPolicy::DangerFullAccess,
            model: session_configured.model.clone(),
            effort: None,
            summary: savfox_protocol::config_types::ReasoningSummary::Auto,
            collaboration_mode: Some(collaboration_mode),
            personality: None,
        })
        .await?;

    let plan_delta = wait_for_event_match(&savfox, |ev| match ev {
        EventMsg::PlanDelta(event) => Some(event.clone()),
        _ => None,
    })
    .await;

    let plan_completed = wait_for_event_match(&savfox, |ev| match ev {
        EventMsg::ItemCompleted(ItemCompletedEvent {
...
async fn plan_mode_strips_plan_from_agent_messages() -> anyhow::Result<()> {
    ...
    while plan_delta.is_none() || agent_item.is_none() || plan_item.is_none() {
        let ev = wait_for_event(&savfox, |_| true).await;
        match ev {
...
async fn reasoning_content_delta_has_item_metadata() -> anyhow::Result<()> {
    ...
    let reasoning_item = wait_for_event_match(&savfox, |ev| match ev {
        EventMsg::ItemStarted(ItemStartedEvent {
            item: TurnItem::Reasoning(item),
            ..
        }) => Some(item.clone()),
