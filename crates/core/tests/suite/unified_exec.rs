...
use core_test_support::process::{wait_for_pid_file, wait_for_process_exit};
use core_test_support::responses::{
    ev_assistant_message, ev_completed, ev_function_call, ev_response_created, mount_sse_sequence,
    sse, start_mock_server,
};
use core_test_support::test_savfox::{TestSavfox, TestSavfoxHarness, test_savfox};
...
async fn unified_exec_intercepts_apply_patch_exec_command() -> Result<()> {
    ...
    let harness = TestSavfoxHarness::with_builder(builder).await?;
...
async fn unified_exec_emits_exec_command_begin_event() -> Result<()> {
    ...
    let mut builder = test_savfox().with_model("gpt-5").with_config(|config| {
        config.use_experimental_unified_exec_tool = true;
        config.features.enable(Feature::UnifiedExec);
    });
    let TestSavfox {
...
async fn unified_exec_emits_exec_command_end_event() -> Result<()> {
    ...
    let end_event = wait_for_event_match(&savfox, |msg| match msg {
...
async fn unified_exec_full_lifecycle_with_background_end_event() -> Result<()> {
    ...
    loop {
        let msg = wait_for_event(&savfox, |_| true).await;
        match msg {
...
async fn unified_exec_emits_one_begin_and_one_end_event() -> Result<()> {
    ...
    loop {
        let event_msg = wait_for_event(&savfox, |_| true).await;
        match event_msg {
...
async fn unified_exec_timeout_and_followup_poll() -> Result<()> {
    ...
    loop {
        let event = savfox.next_event().await.expect("event");
