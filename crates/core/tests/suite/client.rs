...
use core_test_support::responses::{
    ev_completed_with_tokens, mount_sse_once, mount_sse_once_match, mount_sse_sequence, sse,
    sse_failed,
};
...
use savfox_protocol::config_types::{
    CollaborationMode, ModeKind, ReasoningSummary, Settings, Verbosity,
};
use savfox_protocol::models::{
    FunctionCallOutputPayload, MessagePhase, ReasoningItemContent, ReasoningItemReasoningSummary,
    WebSearchAction,
};
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    // Configure Savfox to resume from our file
    ...
    match initial_msgs.as_slice() {
        [
            EventMsg::UserMessage(user),
            EventMsg::AgentMessage(assistant),
        ] => {
            assert_eq!(user.message, "resumed user message");
            assert_eq!(assistant.message, "resumed assistant message");
        }
        other => panic!("unexpected initial messages for resumed session: {other:#?}"),
    }

    // 2) Submit new input; the request body must include the prior items, then initial context,
    //    then new user input.
    savfox
        .submit(Op::UserInput {
...
async fn includes_conversation_id_and_model_headers_in_request() {
    ...
    let mut builder = test_savfox().with_auth(SavfoxAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    model_provider.base_url = Some(format!("{}/api/savfox", server.uri()));
    let mut builder = test_savfox()
        .with_auth(create_dummy_savfox_auth())
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/savfox/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
    ...
    assert_eq!(session_id, session_id.to_string());
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth_manager =
        match SavfoxAuth::from_auth_storage(savfox_home.path(), AuthCredentialsStoreMode::File) {
            Ok(Some(auth)) => savfox_core::AuthManager::from_auth_for_testing(auth),
            Ok(None) => panic!("No SavfoxAuth found in savfox_home"),
            Err(e) => panic!("Failed to load SavfoxAuth: {e}"),
        };
...
async fn skills_append_to_instructions() {
    ...
    let savfox_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = savfox_home.path().join("skills/demo");
    ...
    let _savfox_home_guard = savfox_home;
}
...
-> anyhow::Result<()> {
    ...
    let resp_mock = mount_sse_once(&server, sse_completed("resp1")).await;
    let TestSavfox { savfox, .. } = test_savfox().with_model("gpt-5.1").build(&server).await?;
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let auth_manager = AuthManager::from_auth_for_testing(SavfoxAuth::from_api_key("Test API Key"));
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-savfox-primary-used-percent", "12.5")
        .insert_header("x-savfox-secondary-used-percent", "40.0")
        .insert_header("x-savfox-primary-window-minutes", "10")
        .insert_header("x-savfox-secondary-window-minutes", "60")
        .insert_header("x-savfox-primary-reset-at", "1704069000")
        .insert_header("x-savfox-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_savfox()
        .with_auth(SavfoxAuth::from_api_key("test"))
        .with_config(move |config| {
    ...
    let first_token_event =
        wait_for_event(&savfox, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    pretty_assertions::assert_eq!(
        ...
        json!({
            "info": {
                ...
                // Default model is gpt-5.1-savfox-max in tests → 95% usable context window
    ...
    wait_for_event(&savfox, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-savfox-primary-used-percent", "100.0")
        .insert_header("x-savfox-secondary-used-percent", "87.5")
        .insert_header("x-savfox-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-savfox-primary-window-minutes", "15")
        .insert_header("x-savfox-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let token_event = wait_for_event(&savfox, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&savfox, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&savfox, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&savfox, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = SavfoxError::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_savfox_auth() -> SavfoxAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Savfox sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
