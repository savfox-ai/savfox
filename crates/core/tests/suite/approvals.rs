...
use savfox_core::protocol::{
    ApplyPatchApprovalRequestEvent, AskForApproval, EventMsg, ExecApprovalRequestEvent,
    ExecPolicyAmendment, Op, SandboxPolicy,
};
...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestSavfox) -> (PathBuf, String) {
        match self {
...
impl Expectation {
    fn verify(&self, test: &TestSavfox, result: &CommandResult) -> Result<()> {
        match self {
...
async fn expect_exec_approval(
    ...
    let event = wait_for_event(&test.savfox, |event| {
        matches!(
            event,
...
async fn wait_for_completion_without_approval(test: &TestSavfox) {
    let event = wait_for_event(&test.savfox, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestSavfox) {
    wait_for_event(&test.savfox, |event| {
...
fn scenarios() -> Vec<ScenarioSpec> {
    ...
    vec![
        ...
        ScenarioSpec {
            name: "apply_patch_shell_command_auto_inside_workspace",
            approval_policy: UnlessTrusted,
        ...
        ScenarioSpec {
            ...
            sandbox_permissions: SandboxPermissions::UseDefault,
            features: vec![],
            model_override: Some("gpt-5.1-savfox"),
            outcome: Outcome::Auto,
            expectation: Expectation::PatchApplied {
                target: TargetPath::Workspace("apply_patch_function.txt"),
                content: "function-apply-patch",
        ...
        ScenarioSpec {
            name: "apply_patch_function_unless_trusted_auto_inside_workspace",
            approval_policy: UnlessTrusted,
            sandbox_policy: workspace_write(false),
            action: ActionKind::ApplyPatchFunction {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn symbolic_approve_reply_resolves_pending_exec_approval() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let mut builder = test_savfox()
        ...
        .with_config(move |config| {
            config.approval_policy = Constrained::allow_any(approval_policy_for_config);
            config.sandbox_policy = Constrained::allow_any(sandbox_policy_for_config.clone());
        });
    let test = builder.build(&server).await?;

    let call_id = "textual_approve_reply";
    let action = ActionKind::WriteFile {
        target: TargetPath::Workspace("textual_approve_reply.txt"),
    ...
    let command = expected_command
        .as_deref()
        .expect("write file scenario should produce a shell command");
    let _approval = expect_exec_approval(&test, command).await;

    assert!(!test.savfox.maybe_submit_textual_approval("approve").await?);
    assert!(test.savfox.maybe_submit_textual_approval("+").await?);
    wait_for_completion(&test).await;

    let output_item = results_mock.single_request().function_call_output(call_id);
    let result = parse_result(&output_item);
    assert_eq!(result.exit_code, Some(0));
    assert!(result.stdout.contains("textual-approval"));

    Ok(())
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn structured_approve_reply_resolves_pending_exec_approval() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let call_id = "structured_approve_reply";
    let action = ActionKind::WriteFile {
        target: TargetPath::Workspace("structured_approve_reply.txt"),
    ...
    let result = parse_result(&output_item);
    assert_eq!(result.exit_code, Some(0));
    assert!(result.stdout.contains("structured-approval"));
...
async fn run_scenario(scenario: &ScenarioSpec) -> Result<()> {
    ...
    let mut builder = test_savfox().with_model(model).with_config(move |config| {
