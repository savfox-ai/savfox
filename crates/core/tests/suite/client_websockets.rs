...
use core_test_support::responses::{
    WebSocketConnectionConfig, WebSocketTestServer, ev_completed, ev_response_created,
...
const MODEL: &str = "gpt-5.2-savfox";

struct WebsocketTestHarness {
    _savfox_home: TempDir,
    client: ModelClient,
...
async fn websocket_harness(server: &WebSocketTestServer) -> WebsocketTestHarness {
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "savfox-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )
