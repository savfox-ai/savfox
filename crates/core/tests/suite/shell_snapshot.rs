...
async fn wait_for_snapshot(savfox_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = savfox_home.join("shell_snapshots");
...
async fn run_snapshot_command(command: &str) -> Result<SnapshotRun> {
    ...
    let savfox_home = test.home.path().to_path_buf();
    ...
    .await;
    let snapshot_path = wait_for_snapshot(&savfox_home).await?;
    let snapshot_content = fs::read_to_string(&snapshot_path).await?;

    let end = wait_for_event_match(&savfox, |ev| match ev {
...
async fn shell_snapshot_deleted_after_shutdown_with_skills() -> Result<()> {
    ...
    let savfox = harness.test().savfox.clone();

    let snapshot_path = wait_for_snapshot(&savfox_home).await?;
    assert!(snapshot_path.exists());

    savfox.submit(Op::Shutdown {}).await?;
