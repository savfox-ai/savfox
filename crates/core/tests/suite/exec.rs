...
use savfox_core::error::Result;
use savfox_core::exec::{ExecParams, ExecToolCallOutput, SandboxType, process_exec_tool_call};
use savfox_core::get_platform_sandbox;
use savfox_core::protocol::SandboxPolicy;
use savfox_core::sandboxing::SandboxPermissions;
use savfox_core::spawn::SAVFOX_SANDBOX_ENV_VAR;
use savfox_protocol::config_types::WindowsSandboxLevel;
use tempfile::TempDir;

fn skip_test() -> bool {
    if std::env::var(SAVFOX_SANDBOX_ENV_VAR) == Ok("seatbelt".to_string()) {
        eprintln!("{SAVFOX_SANDBOX_ENV_VAR} is set to 'seatbelt', skipping test.");
        return true;
    }
