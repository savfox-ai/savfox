...
use core_test_support::responses::{
    ev_completed, ev_function_call, ev_response_created, mount_sse_sequence, start_mock_server,
};
...
async fn new_session_is_recorded_in_state_db() -> Result<()> {
    ...
    let rollout_path = test.savfox.rollout_path().expect("rollout path");
    let db_path = test.config.savfox_home.join(STATE_DB_FILENAME);
    ...
    let db = test.savfox.state_db().expect("state db enabled");

    let mut metadata = None;
    for _ in 0..100 {
        metadata = db.get_session(session_id).await?;
        if metadata.is_some() {
            break;
        }
        tokio::time::sleep(Duration::from_millis(25)).await;
    ...
    let metadata = metadata.expect("session should exist in state db");
...
async fn backfill_scans_existing_rollouts() -> Result<()> {
    ...
    let session_id = SessionId::from_string(&uuid.to_string())?;
    let rollout_rel_path = format!("sessions/{uuid}.jsonl");
    ...
    let mut builder = test_savfox()
        .with_pre_build_hook(move |savfox_home| {
            let rollout_path = savfox_home.join(&rollout_rel_path_for_hook);
    ...
    let db_path = test.config.savfox_home.join(STATE_DB_FILENAME);
    let rollout_path = test.config.savfox_home.join(&rollout_rel_path);
    ...
    let metadata = metadata.expect("backfilled session should exist in state db");
    assert_eq!(metadata.id, session_id);
    assert_eq!(metadata.rollout_path, rollout_path);
    ...
    for _ in 0..40 {
        stored_tools = db.get_dynamic_tools(session_id).await?;
...
async fn tool_call_logs_include_session_id() -> Result<()> {
    ...
    let subscriber = tracing_subscriber::registry().with(savfox_state::log_db::start(db.clone()));
    let dispatch = tracing::Dispatch::new(subscriber);
    let _guard = tracing::dispatcher::set_default(&dispatch);

    test.submit_turn("run a shell command").await?;
    {
        let span = tracing::info_span!("test_log_span", session_id = %expected_session_id);
    ...
    let mut found = None;
    for _ in 0..80 {
        let query = savfox_state::LogQuery {
            descending: true,
            limit: Some(20),
            ..Default::default()
        };
        ...
        if let Some(row) = rows.into_iter().find(|row| {
            ...
            found = Some((session_id, message));
            break;
        }
        tokio::time::sleep(Duration::from_millis(25)).await;
    }

    let (session_id, message) = found.expect("expected ToolCall log row");
