...
use core_test_support::responses::{
    ev_apply_patch_call, ev_apply_patch_custom_tool_call, ev_apply_patch_function_call,
    ev_assistant_message, ev_completed, ev_function_call, ev_response_created,
    ev_shell_command_call, mount_sse_sequence, sse,
};
...
pub(crate) async fn apply_patch_harness() -> Result<TestSavfoxHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestSavfoxBuilder) -> TestSavfoxBuilder,
) -> Result<TestSavfoxHarness> {
    let builder = configure(test_savfox()).with_config(|config| {
