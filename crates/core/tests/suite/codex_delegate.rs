...
use savfox_core::protocol::ReviewTarget;
...
async fn savfox_delegate_forwards_exec_approval_and_proceeds_on_approval() {
    ...
    let test = builder.build(&server).await.expect("build test savfox");
...
async fn savfox_delegate_ignores_legacy_deltas() {
    ...
    loop {
        let ev = wait_for_event(&test.savfox, |_| true).await;
        match ev {
