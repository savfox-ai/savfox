...
use savfox_utils::cargo_bin::find_resource;
...
fn repo_root() -> std::path::PathBuf {
    ...
    savfox_utils::cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    let bin2 = savfox_utils::cargo_bin::cargo_bin("savfox").unwrap();
