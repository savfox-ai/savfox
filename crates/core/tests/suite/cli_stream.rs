...
use savfox_core::protocol::GitInfo;
...
fn repo_root() -> std::path::PathBuf {
    ...
    savfox_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
async fn chat_mode_stream_cli() {
    ...
    let bin = savfox_utils_cargo_bin::cargo_bin("savfox").unwrap();
...
async fn responses_api_stream_cli() {
    ...
    cmd.env("SAVFOX_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("SAVFOX_RS_SSE_FIXTURE", fixture)
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the savfox CLI and invoke `exec`, which is what records a session.
    let bin = savfox_utils_cargo_bin::cargo_bin("savfox").unwrap();
    let mut cmd = AssertCommand::new(bin);
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
    ...
    let bin2 = savfox_utils_cargo_bin::cargo_bin("savfox").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("SAVFOX_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("SAVFOX_RS_SSE_FIXTURE", &fixture)
        .env("OPENAI_BASE_URL", "http://unused.local");

    let output2 = cmd2.output().unwrap();
    assert!(output2.status.success(), "resume savfox-cli run failed");
...
async fn integration_git_info_unit_test() {
    ...
    let git_info = savfox_core::git_info::collect_git_info(&git_repo).await;
