...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let mut cmd = Command::new(savfox_utils_cargo_bin::cargo_bin("savfox-rs").unwrap());
    ...
    //   • spawn a session for each stream that copies bytes into two sinks:
    ...
    let mut child = cmd.spawn().expect("failed to spawn savfox-rs");
    ...
    let stdout = stdout_handle.join().expect("stdout session panicked");
    let stderr = stderr_handle.join().expect("stderr session panicked");
