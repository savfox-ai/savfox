...
async fn savfox_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("savfox_output_schema".into()))
    ...
    let TestSavfox { savfox, cwd, .. } = test_savfox().build(&server).await?;
    ...
    let message = wait_for_event(&savfox, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;
