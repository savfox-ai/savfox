...
use core_test_support::responses::{
    ev_assistant_message, ev_completed, ev_response_created, mount_sse_once, sse, start_mock_server,
};
use core_test_support::skip_if_no_network;
use core_test_support::test_savfox::test_savfox;
use savfox_core::protocol::{AskForApproval, Op, SandboxPolicy};
...
async fn user_turn_includes_skill_instructions() -> Result<()> {
    ...
    let mut builder = test_savfox().with_pre_build_hook(|home| {
        write_skill(home, "demo", "demo skill", skill_body);
    });
    let test = builder.build(&server).await?;

    let skill_path = test.savfox_home_path().join("skills/demo/SKILL.md");
    ...
    core_test_support::wait_for_event(test.savfox.as_ref(), |event| {
        matches!(event, savfox_core::protocol::EventMsg::TurnComplete(_))
    })
    .await;
...
async fn skill_load_errors_surface_in_session_configured() -> Result<()> {
    ...
    test.savfox
        ...
        .await?;
    let response =
        core_test_support::wait_for_event_match(test.savfox.as_ref(), |event| match event {
            savfox_core::protocol::EventMsg::ListSkillsResponse(response) => Some(response.clone()),
            _ => None,
        })
        .await;
...
async fn list_skills_includes_system_cache_entries() -> Result<()> {
    ...
    let system_skill_path = system_skill_md_path(test.savfox_home_path(), SYSTEM_SKILL_NAME);
    assert!(
        system_skill_path.exists(),
    ...
    assert_eq!(skill.scope, savfox_protocol::protocol::SkillScope::System);
