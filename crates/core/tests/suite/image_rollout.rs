...
use core_test_support::responses::{
    ev_assistant_message, ev_completed, ev_response_created, sse, start_mock_server,
};
...
async fn copy_paste_local_image_persists_rollout_request_shape() -> anyhow::Result<()> {
    ...
    wait_for_event(&savfox, |event| matches!(event, EventMsg::ShutdownComplete)).await;

    let rollout_path = savfox.rollout_path().expect("rollout path");
