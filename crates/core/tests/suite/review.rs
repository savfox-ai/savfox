...
use savfox_core::protocol::{
    ...
    ReviewFinding, ReviewLineRange, ReviewOutputEvent, ReviewRequest, ReviewTarget, RolloutItem,
    RolloutLine,
};
...
async fn review_op_emits_lifecycle_and_review_output() {
    ...
    let savfox = new_conversation_for_server(&server, savfox_home.clone(), |_| {}).await;
    ...
    let _entered = wait_for_event(&savfox, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let closed = wait_for_event(&savfox, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    ...
    let path = savfox.rollout_path().expect("rollout path");
...
async fn review_filters_agent_message_related_events() {
    ...
    wait_for_event(&savfox, |event| match event {
...
async fn review_uses_custom_review_model_from_config() {
    ...
    // Wait for completion
    let _entered = wait_for_event(&savfox, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let _closed = wait_for_event(&savfox, |ev| {
        matches!(
            ev,
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = savfox_home.path().join("resume.jsonl");
    {
    ...
    let savfox =
        resume_conversation_for_server(&server, savfox_home.clone(), session_file.clone(), |_| {})
            .await;
    ...
    // Assert the request `input` contains the environment context followed by the user review
    // prompt.
    let request = request_log.single_request();
    assert_eq!(request.path(), "/v1/responses");
    let body = request.body_json();
...
async fn review_history_surfaces_in_parent_session() {
    ...
    // Critically, no messages from the review session should appear.
    ...
    // Ensure review-session content is present for downstream turns.
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let savfox = new_conversation_for_server(&server, savfox_home.clone(), move |config| {
...
/// Create a conversation resuming from a rollout file, configured to talk to the provided mock
/// server.
#[expect(clippy::expect_used)]
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, savfox_home, resume_path)
        .await
