...
use core_test_support::responses::{
    ev_assistant_message, ev_completed, ev_function_call, ev_local_shell_call, ev_response_created,
    mount_sse_sequence, sse, start_mock_server,
};
use core_test_support::test_savfox::{
...
use crate::suite::apply_patch_cli::{apply_patch_harness, mount_apply_patch};
...
fn configure_shell_model(
    builder: TestSavfoxBuilder,
    output_type: ShellModelOutput,
    include_apply_patch_tool: bool,
) -> TestSavfoxBuilder {
    let builder = match (output_type, include_apply_patch_tool) {
        (ShellModelOutput::ShellCommand, _) => builder.with_model("test-gpt-5-savfox"),
        (ShellModelOutput::LocalShell, true) => builder.with_model("gpt-5.1-savfox"),
        (ShellModelOutput::Shell, true) => builder.with_model("gpt-5.1-savfox"),
        (ShellModelOutput::LocalShell, false) => builder.with_model("savfox-mini-latest"),
...
async fn shell_output_stays_json_without_freeform_apply_patch(
    ...
    let mut builder = configure_shell_model(test_savfox(), output_type, false);
...
async fn shell_output_is_structured_with_freeform_apply_patch(
    ...
    let server = start_mock_server().await;
    let mut builder = configure_shell_model(test_savfox(), output_type, true);
    let test = builder.build(&server).await?;
...
async fn shell_output_reserializes_truncated_content(output_type: ShellModelOutput) -> Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder =
        configure_shell_model(test_savfox(), output_type, true).with_config(move |config| {
...
async fn shell_command_output_is_not_truncated_under_10k_bytes() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder = test_savfox().with_model("gpt-5.1");
    let test = builder.build(&server).await?;
