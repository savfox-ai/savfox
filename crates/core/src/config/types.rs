...
pub use savfox_protocol::config_types::{AltScreenMode, ModeKind, Personality, WebSearchMode};
...
pub struct McpServerConfig {
    ...
    /// When `false`, Savfox skips initializing this MCP server.
    ...
    /// Explicit allow-list of tools exposed from this server. When set, only these tools will be
    /// registered.
...
pub struct AnalyticsConfigToml {
    /// When `false`, disables analytics across Savfox product surfaces in this profile.
    pub enabled: Option<bool>,
}
...
pub struct FeedbackConfigToml {
    /// When `false`, disables the feedback flow across Savfox product surfaces.
    pub enabled: Option<bool>,
}
...
/// (primarily the Savfox IDE extension). NOTE: these are different from
...
pub struct Notice {
    ...
    /// Tracks whether the user has seen the gpt-5.1-savfox-max migration prompt
    #[serde(rename = "hide_gpt-5.1-savfox-max_migration_prompt")]
    pub hide_gpt_5_1_savfox_max_migration_prompt: Option<bool>,
...
impl From<SandboxWorkspaceWrite> for savfox_app_server_protocol::SandboxSettings {
...
/// 2. If `ignore_default_excludes` is false, filter the map using the default exclude pattern(s),
///    which are: `"*KEY*"`, `"*SECRET*"`, and `"*TOKEN*"`.
