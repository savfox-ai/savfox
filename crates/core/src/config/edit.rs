...
mod tests {
    ...
    fn blocking_set_model_top_level() {
        ...
        // model_provider should be removed (inferred from model.provider).
...
"#,
    ...
    fn blocking_set_model_writes_through_symlink_chain() {
        ...
        let link_path = savfox_home.join("config-link.toml");
...
"#;
    ...
    fn blocking_set_model_replaces_symlink_on_cycle() {
        let tmp = tempdir().expect("tmpdir");
        let savfox_home = tmp.path();
        let link_a = savfox_home.join("a.toml");
        let link_b = savfox_home.join("b.toml");
        let config_path = savfox_home.join(CONFIG_TOML_FILE);
...
"#;
        std::fs::write(savfox_home.join(CONFIG_TOML_FILE), original).expect("seed config");
...
"#;
    ...
    #[test]
    fn blocking_set_hide_gpt_5_1_savfox_max_migration_prompt_preserves_table() {
        let tmp = tempdir().expect("tmpdir");
        let savfox_home = tmp.path();
        std::fs::write(
            savfox_home.join(CONFIG_TOML_FILE),
...
"#,
        ...
        let expected = r#"[notice]
existing = "value"
"hide_gpt-5.1-savfox-max_migration_prompt" = true
"#;
        assert_eq!(contents, expected);
    }
...
"#;
    ...
    fn blocking_clear_path_noop_when_missing() {
        ...
        assert!(
            !savfox_home.join(CONFIG_TOML_FILE).exists(),
