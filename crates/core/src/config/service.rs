...
use savfox_app_server_protocol::{
    Config as ApiConfig, ConfigBatchWriteParams, ConfigLayerMetadata, ConfigLayerSource,
...
use super::{CONFIG_TOML_FILE, ConfigToml};
use crate::config::edit::{ConfigEdit, ConfigEditsBuilder};
use crate::config_loader::{
    CloudRequirementsLoader, ConfigLayerEntry, ConfigLayerStack, ConfigLayerStackOrdering,
    ConfigRequirementsToml, LoaderOverrides, load_config_layers_state, merge_toml_values,
};
use crate::path_utils;
use crate::path_utils::{SymlinkWritePaths, resolve_symlink_write_paths, write_atomically};
...
impl ConfigService {
    ...
    pub async fn read(
        ...
        let layers = match params.cwd.as_deref() {
            ...
            None => self.load_session_agnostic_config().await.map_err(|err| {
                ConfigServiceError::io("failed to read configuration layers", err)
            })?,
        };
    ...
    pub async fn load_user_saved_config(
        &self,
    ) -> Result<savfox_app_server_protocol::UserSavedConfig, ConfigServiceError> {
        let layers = self
            .load_session_agnostic_config()
            .await
    ...
    async fn apply_edits(
        ...
        let allowed_path =
            AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, &self.savfox_home)
        ...
        if !config_edits.is_empty() {
            ConfigEditsBuilder::new(&self.savfox_home)
    ...
    /// Loads a "session-agnostic" config, which means the config layers do not
    /// include any in-repo .savfox/ folders because there is no cwd/project root
    /// associated with this query.
    async fn load_session_agnostic_config(&self) -> std::io::Result<ConfigLayerStack> {
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_savfox_folder } => format!(
...
"#;
    ...
    async fn write_value_preserves_comments_and_order() -> Result<()> {
        ...
        let original = r#"# Savfox user configuration
model = { provider = "openai", slug = "gpt-5" }
...
"#;
        ...
        let expected = r#"# Savfox user configuration
model = { provider = "openai", slug = "gpt-5" }
approval_policy = "on-request"
...
"#;
    ...
    async fn read_includes_origins_and_layers() {
        ...
        std::fs::write(&user_path, "sandbox_mode = \"workspace-write\"").unwrap();
    ...
    async fn write_value_defaults_to_user_config_path() {
        ...
        let contents =
            std::fs::read_to_string(tmp.path().join(CONFIG_TOML_FILE)).expect("read config");
        assert!(
            contents.contains("slug = \"gpt-new\""),
