...
use crate::spawn::{SAVFOX_SANDBOX_ENV_VAR, StdioPolicy, spawn_child_async};
...
pub async fn spawn_command_under_seatbelt(
    ...
    let arg0 = None;
    env.insert(SAVFOX_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
    spawn_child_async(
...
mod tests {
    ...
    use super::{MACOS_SEATBELT_BASE_POLICY, create_seatbelt_command_args, macos_dir_params};
    use crate::protocol::SandboxPolicy;
    ...
    #[test]
    fn create_seatbelt_args_with_read_only_git_and_savfox_subpaths() {
        // Create a temporary workspace with two writable roots: one containing
        // top-level .git and .savfox directories and one without them.
        let tmp = TempDir::new().expect("tempdir");
        let PopulatedTmp {
            vulnerable_root,
            vulnerable_root_canonical,
            dot_git_canonical,
            dot_savfox_canonical,
            empty_root,
        ...
        // write to .savfox/config.toml in the vulnerable root.
        ...
        // - write access to WRITABLE_ROOT_0 (but not its .git or .savfox), WRITABLE_ROOT_1, and cwd
        //   as WRITABLE_ROOT_2.
...
"#,
        ...
        let mut expected_args = vec![
            ...
            format!(
                ...
                dot_savfox_canonical.to_string_lossy()
            ),
            format!(
                "-DWRITABLE_ROOT_1={}",
        ...
        // Verify that .savfox/config.toml cannot be modified under the generated
        // Seatbelt policy.
        let config_toml = dot_savfox_canonical.join("config.toml");
        ...
        // Verify that writing a file to the folder containing .git and .savfox is allowed.
    ...
    fn create_seatbelt_args_for_cwd_as_git_repo() {
        ...
        // `.savfox` checks are done properly for cwd.
        let policy = SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: false,
...
"#,
    ...
    struct PopulatedTmp {
        /// Path containing a .git and .savfox subfolder.
        ...
        /// ran `git commit` themselves, or modified .savfox/config.toml to
        ...
        dot_savfox_canonical: PathBuf,

        /// Path without .git or .savfox subfolders.
        empty_root: PathBuf,
    ...
    fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
        ...
        fs::create_dir_all(vulnerable_root.join(".savfox")).expect("create .savfox");
        fs::write(
            vulnerable_root.join(".savfox").join("config.toml"),
            "sandbox_mode = \"read-only\"\n",
        )
        .expect("write .savfox/config.toml");
        ...
        let dot_savfox_canonical = vulnerable_root_canonical.join(".savfox");
