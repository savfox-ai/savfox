...
pub fn sandbox_setup_is_complete(_savfox_home: &Path) -> bool {
    false
}
...
pub fn elevated_setup_failure_details(err: &anyhow::Error) -> Option<(String, String)> {
    ...
    let message = savfox_windows_sandbox::sanitize_setup_metric_tag_value(&failure.message);
    Some((code, message))
}
...
pub fn elevated_setup_failure_metric_name(err: &anyhow::Error) -> &'static str {
    if savfox_windows_sandbox::extract_setup_failure(err).is_some_and(|failure| {
        matches!(
            failure.code,
            savfox_windows_sandbox::SetupErrorCode::OrchestratorHelperLaunchCanceled
        )
    }) {
        "savfox.windows_sandbox.elevated_setup_canceled"
    } else {
        "savfox.windows_sandbox.elevated_setup_failure"
    }
}
...
pub fn run_elevated_setup(
    ...
    savfox_windows_sandbox::run_elevated_setup(
        policy,
        policy_cwd,
        command_cwd,
        env_map,
