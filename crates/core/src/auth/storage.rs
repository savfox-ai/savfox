...
pub(super) fn delete_file_if_exists(savfox_home: &Path) -> std::io::Result<bool> {
    remove_file_if_exists(get_auth_file(savfox_home).as_path())
}
...
pub(super) struct FileAuthStorage {
    savfox_home: PathBuf,
    resolved_auth_file: Arc<Mutex<Option<PathBuf>>>,
}
...
impl FileAuthStorage {
    pub(super) fn new(savfox_home: PathBuf) -> Self {
        Self {
            savfox_home,
            resolved_auth_file: Arc::new(Mutex::new(None)),
        }
    }
    ...
    fn set_resolved_auth_file(&self, path: PathBuf) {
        if let Ok(mut guard) = self.resolved_auth_file.lock() {
            *guard = Some(path);
        }
    }

    fn current_auth_file(&self) -> PathBuf {
        self.resolved_auth_file
            .lock()
            .ok()
            .and_then(|guard| guard.clone())
            .unwrap_or_else(|| get_auth_file(&self.savfox_home))
    }
    ...
    fn find_named_provider_auth_file(&self) -> std::io::Result<Option<PathBuf>> {
        ...
        for entry in entries.flatten() {
            ...
            if path == get_auth_file(&self.savfox_home) {
                continue;
            }
            ...
            if file.provider_id.trim() != AUTH_PROVIDER_ID || file.auth.is_none() {
                continue;
            }
    ...
    fn save_provider_store_auth_file(
        &self,
        auth_file: &Path,
        ...
        let existing_provider_file = std::fs::read_to_string(auth_file)
            .ok()
            .and_then(|contents| serde_json::from_str::<ProviderStoreFile>(&contents).ok());
        let auth_only_file = ProviderStoreFile::from(auth_dot_json);
        let mut provider_file = existing_provider_file.unwrap_or(auth_only_file.clone());
        provider_file.version = PROVIDER_STORE_FILE_VERSION;
        if provider_file.id.trim().is_empty() {
        ...
        file.flush()?;
        self.set_resolved_auth_file(auth_file.to_path_buf());
        Ok(())
    }

    fn delete_provider_auth_file(&self, auth_file: &Path) -> std::io::Result<bool> {
        let existing = match std::fs::read_to_string(auth_file) {
            Ok(data) => data,
        ...
        if let Ok(mut provider_file) = serde_json::from_str::<ProviderStoreFile>(&existing) {
            let should_preserve_file =
                !provider_file.models.is_empty() || provider_file.id.trim() != AUTH_PROVIDER_ID;
        ...
        remove_file_if_exists(auth_file)
    }
}
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_dot_json =
            match self.try_read_auth_json(get_auth_file(&self.savfox_home).as_path()) {
                Ok(auth) => {
                    self.set_resolved_auth_file(get_auth_file(&self.savfox_home));
                    auth
                }
                Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
                    if let Some(named_auth_file) = self.find_named_provider_auth_file()? {
                        let auth = self.try_read_auth_json(&named_auth_file)?;
                        self.set_resolved_auth_file(named_auth_file);
                        auth
                    } else {
                        return Ok(None);
                    }
    ...
    fn save(&self, auth_dot_json: &AuthDotJson) -> std::io::Result<()> {
        let auth_file = self.current_auth_file();
        self.save_provider_store_auth_file(&auth_file, auth_dot_json)?;
        Ok(())
    }

    fn delete(&self) -> std::io::Result<bool> {
        let auth_file = self.current_auth_file();
        let current_removed = self.delete_provider_auth_file(&auth_file)?;
        let default_removed = if auth_file != get_auth_file(&self.savfox_home) {
            self.delete_provider_auth_file(get_auth_file(&self.savfox_home).as_path())?
        } else {
            false
        };
        Ok(current_removed || default_removed)
    }
}
