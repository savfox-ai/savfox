...
/// Because there can only be one MCP server per process, it should be safe for this to be a global
/// static. However, future users of this should use this with caution as a result.
/// In addition, we want to be confident that this value is used for ALL clients and doing that
/// requires a lot of wiring and it's easy to miss code paths by doing so.
/// See https://github.com/openai/savfox/pull/3388/files for an example of what that would look like.
/// Finally, we want to make sure this is set for ALL mcp clients without needing to know a special
/// env var or having to set data that they already specified in the mcp initialize request
/// somewhere else.
///
/// A space is automatically added between the suffix and the rest of the User-Agent string.
/// The full user agent string is returned from the mcp initialize response.
/// Parenthesis will be added by Savfox. This should only specify what goes inside of the
/// parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "savfox_cli_rs";
pub const SAVFOX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "SAVFOX_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-openai-internal-savfox-residency";
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(SAVFOX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(SAVFOX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        ...
        || originator_value.starts_with("Savfox ")
}

pub fn get_savfox_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Savfox user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Savfox user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Savfox originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
pub fn create_client() -> SavfoxHttpClient {
    let inner = build_reqwest_client();
    SavfoxHttpClient::new(inner)
}
...
fn is_sandboxed() -> bool {
    std::env::var(SAVFOX_SANDBOX_ENV_VAR).as_deref() == Ok("seatbelt")
}
...
mod tests {
    ...
    fn is_first_party_originator_matches_known_values() {
        ...
        assert_eq!(is_first_party_originator("savfox_vscode"), true);
        assert_eq!(is_first_party_originator("Savfox Something Else"), true);
        assert_eq!(is_first_party_originator("savfox_cli"), false);
    ...
    async fn test_create_client_sets_default_headers() {
        ...
        // User-Agent matches the computed Savfox UA for that originator
        let expected_ua = get_savfox_user_agent();
    ...
    fn test_invalid_suffix_is_sanitized() {
        let prefix = "savfox_cli_rs/0.0.0";
        let suffix = "bad\rsuffix";

        assert_eq!(
            sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
            "savfox_cli_rs/0.0.0 (bad_suffix)"
        );
    }
