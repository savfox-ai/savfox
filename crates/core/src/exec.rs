...
use savfox_utils::pty::process_group::kill_child_process_group;
...
use crate::sandboxing::{CommandSpec, ExecEnv, SandboxManager, SandboxPermissions};
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: savfox_protocol::config_types::WindowsSandboxLevel,
...
pub async fn process_exec_tool_call(
    ...
    let exec_env = manager
        .transform(
            ...
            sandbox_type,
            sandbox_cwd,
            savfox_linux_sandbox_exe.as_ref(),
            windows_sandbox_level,
        )
        .map_err(SavfoxError::from)?;
...
fn record_windows_sandbox_spawn_failure(
    ...
    windows_sandbox_level: savfox_protocol::config_types::WindowsSandboxLevel,
    err: &str,
) {
    ...
    let level = if matches!(
        windows_sandbox_level,
        savfox_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    };
    if let Some(metrics) = savfox_otel::metrics::global() {
        let _ = metrics.counter(
            "savfox.windows_sandbox.createprocessasuserw_failed",
            1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_savfox_home;
    ...
    let policy_str = serde_json::to_string(sandbox_policy).map_err(|err| {
        SavfoxError::Io(io::Error::other(format!(
            "failed to serialize Windows sandbox policy: {err}"
        )))
    })?;
    let sandbox_cwd = cwd.clone();
    let savfox_home = find_savfox_home().map_err(|err| {
        SavfoxError::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve savfox_home: {err}"
        )))
    })?;
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(SavfoxError::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, SavfoxError>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(SavfoxError::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(SavfoxError::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(SavfoxError::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
                }));
            }
...
pub(crate) mod errors {
    use super::SavfoxError;
    use crate::sandboxing::SandboxTransformError;

    impl From<SandboxTransformError> for SavfoxError {
...
fn aggregate_output(
    ...
    // Under contention, reserve 1/3 for stdout and 2/3 for stderr; rebalance unused stderr to
    // stdout.
...
mod tests {
    ...
    async fn process_exec_tool_call_respects_cancellation_token() -> Result<()> {
        ...
        let output = match result {
            Err(SavfoxError::Sandbox(SandboxErr::Timeout { output })) => output,
