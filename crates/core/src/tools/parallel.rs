...
impl ToolCallRuntime {
    ...
    pub(crate) fn handle_tool_call(
    ...
    ) -> impl std::future::Future<Output = Result<ResponseInputItem, SavfoxError>> {
        ...
        async move {
            match handle.await {
                ...
                Ok(Err(FunctionCallError::Fatal(message))) => Err(SavfoxError::Fatal(message)),
                Ok(Err(other)) => Err(SavfoxError::Fatal(other.to_string())),
                Err(err) => Err(SavfoxError::Fatal(format!(
