...
impl ToolRouter {
    ...
    /// Filter tools according to a [`ToolAccessPolicy`].
    ///
    /// Removes tool specs that are denied or not in the allowed list.
    /// MCP tools (those with a `/` in the name) are exempt from filtering
    /// since they are managed by the MCP connection manager's own filter.
    pub fn apply_tool_access_policy(&mut self, policy: &ToolAccessPolicy) {
        self.specs.retain(|spec| {
            let name = spec.spec.name();
            // Don't filter MCP tools (qualified names contain '/')
            if name.contains('/') {
                return true;
            }
    ...
    fn failure_response(
        ...
        if payload_outputs_custom {
            ...
            ResponseInputItem::FunctionCallOutput {
                call_id,
                output: savfox_protocol::models::FunctionCallOutputPayload {
                    content: message,
                    success: Some(false),
                    ..Default::default()
                },
