...
//! `savfox --savfox-run-as-apply-patch`, and runs under the current
//! `SandboxAttempt` with a minimal environment.
use std::collections::HashMap;
use std::path::PathBuf;

use futures::future::BoxFuture;
use savfox_apply_patch::ApplyPatchAction;
use savfox_protocol::protocol::{AskForApproval, FileChange, ReviewDecision};
use savfox_utils::absolute_path::AbsolutePathBuf;

use crate::SAVFOX_APPLY_PATCH_ARG1;
use crate::exec::ExecToolCallOutput;
use crate::sandboxing::{CommandSpec, SandboxPermissions, execute_env};
...
impl ApplyPatchRuntime {
    ...
    fn build_command_spec(req: &ApplyPatchRequest) -> Result<CommandSpec, ToolError> {
        use std::env;
        let exe = if let Some(path) = &req.savfox_exe {
            path.clone()
        } else {
            env::current_exe()
                .map_err(|e| ToolError::Rejected(format!("failed to determine savfox exe: {e}")))?
        };
