...
use crate::unified_exec::{
    ExecCommandRequest, UnifiedExecContext, UnifiedExecProcessManager, UnifiedExecResponse,
    WriteStdinRequest,
};
...
impl ToolHandler for UnifiedExecHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<ToolOutput, FunctionCallError> {
        ...
        let arguments = match payload {
            ...
            _ => {
                return model_err("unified_exec handler received unsupported payload");
            }
        };
        ...
        let response = match tool_name.as_str() {
            ...
            other => {
                return model_err(format!("unsupported unified exec function {other}"));
            }
        };
...
mod tests {
    ...
    fn test_get_command_respects_explicit_bash_shell() -> anyhow::Result<()> {
        ...
        assert_eq!(command.last(), Some(&"echo hello".to_owned()));
        if command
            .iter()
            .any(|arg| arg.eq_ignore_ascii_case("-Command"))
        {
            assert!(command.contains(&"-NoProfile".to_owned()));
        }
        Ok(())
    }
