        ...
        let arguments = match payload {
            ...
            _ => {
                return Err(FunctionCallError::RespondToModel(
                    "process handler received unsupported payload".to_string(),
                ));
            }
        };
...
async fn handle_poll(
    ...
    let process_id = args.process_id.as_deref().ok_or_else(|| {
        FunctionCallError::RespondToModel("process_id is required for the poll action".to_string())
    })?;
    ...
    let response = manager
        ...
        .await
        .map_err(|err| FunctionCallError::RespondToModel(format!("poll failed: {err}")))?;
...
async fn handle_read_log(
    ...
    let process_id = args.process_id.as_deref().ok_or_else(|| {
        FunctionCallError::RespondToModel(
            "process_id is required for the read_log action".to_string(),
        )
    })?;
    ...
    let response = manager
        ...
        .await
        .map_err(|err| FunctionCallError::RespondToModel(format!("read_log failed: {err}")))?;
...
async fn handle_write(
    ...
    let process_id = args.process_id.as_deref().ok_or_else(|| {
        FunctionCallError::RespondToModel("process_id is required for the write action".to_string())
    })?;
...
async fn handle_send_keys(
    ...
    let process_id = args.process_id.as_deref().ok_or_else(|| {
        FunctionCallError::RespondToModel(
            "process_id is required for the send_keys action".to_string(),
        )
    })?;

    let keys = args.keys.as_deref().ok_or_else(|| {
        FunctionCallError::RespondToModel("keys is required for the send_keys action".to_string())
    })?;

    let mut input = String::new();
    for key in keys {
        let escape = key_to_escape(key)
            .ok_or_else(|| FunctionCallError::RespondToModel(format!("unknown key name: {key}")))?;
    ...
    let response = manager
        ...
        .await
        .map_err(|err| FunctionCallError::RespondToModel(format!("send_keys failed: {err}")))?;
...
async fn handle_kill(args: &ProcessArgs) -> Result<ToolOutput, FunctionCallError> {
    let process_id = args.process_id.as_deref().ok_or_else(|| {
        FunctionCallError::RespondToModel("process_id is required for the kill action".to_string())
    })?;
