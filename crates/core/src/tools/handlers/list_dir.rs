...
impl ToolHandler for ListDirHandler {
    ...
    async fn handle(&self, _invocation: ToolInvocation) -> Result<ToolOutput, FunctionCallError> {
        let arguments = match &_invocation.payload {
            ...
            _ => return model_err("ListDirHandler received unsupported payload"),
        };
        ...
        if offset == 0 {
            return model_err("offset must be a 1-indexed entry number");
        }
        ...
        if depth == 0 {
            return model_err("depth must be greater than zero");
        }

        let path = PathBuf::from(&dir_path);
        if !path.is_absolute() {
            return model_err("dir_path must be an absolute path");
        }
...
async fn list_dir_slice(
    ...
    if start_index >= entries.len() {
        return model_err("offset exceeds directory entry count");
    }
