...
use crate::tools::context::{SharedTurnDiffTracker, ToolInvocation, ToolOutput, ToolPayload};
...
impl ToolHandler for ApplyPatchHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<ToolOutput, FunctionCallError> {
        ...
        let patch_input = match payload {
            ...
            _ => {
                return model_err("apply_patch handler received unsupported payload");
            }
        };
        ...
        match savfox_apply_patch::maybe_parse_apply_patch_verified(&command, &cwd) {
                ...
                match apply_patch::apply_patch(turn.as_ref(), changes).await {
                    ...
                    InternalApplyPatchInvocation::DelegateToExec(apply) => {
                        ...
                        let req = ApplyPatchRequest {
                            ...
                            timeout_ms: None,
                            savfox_exe: turn.savfox_linux_sandbox_exe.clone(),
                        };
            ...
            savfox_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                model_err(format!("apply_patch verification failed: {parse_error}"))
            }
            savfox_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                model_err("apply_patch handler received invalid patch input")
            }
            savfox_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                model_err("apply_patch handler received non-apply_patch input")
            }
        }
    }
}
...
pub(crate) async fn intercept_apply_patch(
    ...
    match savfox_apply_patch::maybe_parse_apply_patch_verified(command, cwd) {
        ...
        savfox_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
