...
impl ToolHandler for GatewayToolHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<ToolOutput, FunctionCallError> {
        let arguments = match &invocation.payload {
            ...
            _ => return model_err("GatewayToolHandler received unsupported payload"),
        };
        ...
        if !status.is_success() {
            return model_err(format!("gateway returned HTTP {status}: {body_text}"));
        }
        ...
        if let Ok(rpc_response) = serde_json::from_str::<serde_json::Value>(&body_text) {
            if let Some(error) = rpc_response.get("error") {
                ...
                return model_err(format!("gateway method '{}' error: {message}", args.method));
            }
            if let Some(result) = rpc_response.get("result") {
                return Ok(ToolOutput::ok(
                    serde_json::to_string_pretty(result).unwrap_or(body_text),
                ));
            }
        }
