        ...
        let arguments = match invocation.payload {
            ...
            _ => {
                return Err(FunctionCallError::RespondToModel(
                    "browser requires function payload".to_string(),
                ));
            }
        };
        ...
        match args.action.as_str() {
            ...
            "close" => {
                ...
                let response = client.delete(&url).send().await.map_err(|err| {
                    FunctionCallError::RespondToModel(format!("browser request failed: {err}"))
                })?;
            ...
            "upload" => {
                let paths = args.paths.as_ref().ok_or_else(|| {
                    FunctionCallError::RespondToModel("missing required field: paths".to_string())
                })?;
            ...
            "act" => {
                let request = args.request.ok_or_else(|| {
                    FunctionCallError::RespondToModel(
                        "missing required field: request (BrowserActRequest JSON)".to_string(),
                    )
                })?;
...
async fn response_to_output(response: reqwest::Response) -> Result<ToolOutput, FunctionCallError> {
    ...
    let body_bytes = response.bytes().await.map_err(|err| {
        FunctionCallError::RespondToModel(format!("failed to read browser response: {err}"))
    })?;
