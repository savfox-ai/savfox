...
impl ToolHandler for GrepFilesHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<ToolOutput, FunctionCallError> {
        let arguments = match &invocation.payload {
            ...
            _ => return model_err("GrepFilesHandler received unsupported payload"),
        };
        ...
        if pattern.is_empty() {
            return model_err("pattern must not be empty");
        }
...
async fn verify_path_exists(path: &Path) -> Result<(), FunctionCallError> {
    tokio::fs::metadata(path)
        .await
        .or_else(|err| model_err(format!("unable to access `{}`: {err}", path.display())))?;
    Ok(())
}
...
async fn run_rg_search(
    ...
    let output = timeout(COMMAND_TIMEOUT, command.output())
        .await
        .or_else(|_| model_err("rg timed out after 30 seconds"))?
        .or_else(|err| {
            model_err(format!(
    ...
    match output.status.code() {
        ...
        _ => {
            ...
            model_err(format!("rg failed: {stderr}"))
        }
    }
}
