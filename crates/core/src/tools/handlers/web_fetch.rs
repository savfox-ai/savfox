...
impl ToolHandler for WebFetchHandler {
    ...
    async fn handle(&self, _invocation: ToolInvocation) -> Result<ToolOutput, FunctionCallError> {
        let arguments = match &_invocation.payload {
            ...
            _ => return model_err("WebFetchHandler received unsupported payload"),
        };
        let args: WebFetchArgs = parse_arguments(&arguments)?;
        // Parse and validate the URL.
        let parsed_url: url::Url = args
            .url
            .parse()
            .or_else(|err: url::ParseError| model_err(format!("invalid URL: {err}")))?;
        ...
        let response = {
            ...
            for _hop in 0..=MAX_REDIRECTS {
                ...
                let candidate = client
                    ...
                    .send()
                    .await
                    .or_else(|err| model_err(format!("HTTP request failed: {err}")))?;
                ...
                if candidate.status().is_redirection() {
                    let Some(location) = candidate
                        ...
                        return model_err("HTTP redirect missing Location header");
                    };
                    current_url = current_url
                        .join(location)
                        .or_else(|err| model_err(format!("invalid redirect target: {err}")))?;
                    continue;
                }
            ...
            let Some(response) = response else {
                return model_err(format!("redirect limit exceeded (max {MAX_REDIRECTS})"));
            };
            response
        };
        ...
        if !status.is_success() {
            return model_err(format!("HTTP {status} for {}", args.url));
        }
        ...
        let body_bytes = response
            .bytes()
            .await
            .or_else(|err| model_err(format!("failed to read response body: {err}")))?;
...
async fn validate_target_url(url: &url::Url) -> Result<(), FunctionCallError> {
    match url.scheme() {
        "http" | "https" => {}
        scheme => {
            return model_err(format!("unsupported URL scheme: {scheme}"));
        }
    }
    ...
    if is_blocked_hostname(host) {
        return model_err("fetching private/metadata hosts is not allowed");
    }
    let Some(port) = url.port_or_known_default() else {
        return model_err("URL missing port");
    };
