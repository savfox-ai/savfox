...
impl ToolHandler for McpResourceHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<ToolOutput, FunctionCallError> {
        ...
        let arguments = match payload {
            ...
            _ => {
                return model_err("mcp_resource handler received unsupported payload");
            }
        };
        ...
        match tool_name.as_str() {
            ...
            other => model_err(format!("unsupported MCP resource tool: {other}")),
        }
    }
}
    ...
    let payload_result: Result<ListResourcesPayload, FunctionCallError> = async {
        if let Some(server_name) = server.clone() {
            ...
            let result = session
                ...
                .await
                .or_else(|err| model_err(format!("resources/list failed: {err:#}")))?;
            ...
            if cursor.is_some() {
                return model_err("cursor can only be used when a server is specified");
            }
    ...
    let payload_result: Result<ListResourceTemplatesPayload, FunctionCallError> = async {
        if let Some(server_name) = server.clone() {
            ...
            let result = session
                ...
                .await
                .or_else(|err| model_err(format!("resources/templates/list failed: {err:#}")))?;
...
fn normalize_required_string(field: &str, value: String) -> Result<String, FunctionCallError> {
    match normalize_optional_string(Some(value)) {
        ...
        None => model_err(format!("{field} must be provided")),
    }
}
    ...
    T: Serialize,
{
    let content = serde_json::to_string(&payload)
        .or_else(|err| model_err(format!("failed to serialize MCP resource response: {err}")))?;
...
fn parse_arguments(raw_args: &str) -> Result<Option<Value>, FunctionCallError> {
    if raw_args.trim().is_empty() {
        ...
        let value: Value = serde_json::from_str(raw_args)
            .or_else(|err| model_err(format!("failed to parse function arguments: {err}")))?;
        if value.is_null() {
            Ok(None)
        } else {
            Ok(Some(value))
    ...
    T: DeserializeOwned,
{
    match arguments {
        Some(value) => serde_json::from_value(value)
            .or_else(|err| model_err(format!("failed to parse function arguments: {err}"))),
        None => model_err("failed to parse function arguments: expected value"),
    }
}
