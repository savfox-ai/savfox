...
impl ToolHandler for SessionsSendA2AHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<ToolOutput, FunctionCallError> {
        let arguments = match &invocation.payload {
            ...
            _ => {
                return model_err("SessionsSendA2AHandler received unsupported payload");
            }
        };
        let args: SessionsSendA2AArgs = parse_arguments(&arguments)?;

        if args.source_session_id.is_empty() || args.target_session_id.is_empty() {
            return model_err("source_session_id and target_session_id must not be empty");
        }
        if args.message.is_empty() {
            return model_err("message must not be empty");
        }
...
async fn run_agent_step(
    ...
    let response = client
        ...
        .send()
        .await
        .or_else(|err| model_err(format!("agent step failed: {err}")))?;
