...
impl ToolHandler for AgentStepHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<ToolOutput, FunctionCallError> {
        let arguments = match invocation.payload {
            ...
            _ => {
                return Err(FunctionCallError::RespondToModel(
                    "agent_step requires function payload".to_string(),
                ));
            }
        };
        ...
        let response = client
            ...
            .await
            .map_err(|err| {
                FunctionCallError::RespondToModel(format!("agent invocation failed: {err}"))
            })?;
        ...
        let run_result: serde_json::Value = response.json().await.map_err(|err| {
            FunctionCallError::RespondToModel(format!("failed to parse agent response: {err}"))
        })?;
