...
/// 1. Maintain an in-memory baseline snapshot of files when they are first seen. For new additions,
///    do not create a baseline so that diffs are shown as proper additions (using /dev/null).
/// 2. Keep a stable internal filename (uuid) per external path for rename tracking.
/// 3. To compute the aggregated unified diff, compare each baseline snapshot to the current file on
///    disk entirely in-memory using the `similar` crate and emit unified diffs with rewritten
///    external paths.
#[derive(Default)]
pub struct TurnDiffTracker {
    ...
    /// Internal filename -> external path as of current accumulated state (after applying all
    /// changes). This is where renames are tracked.
...
impl TurnDiffTracker {
    ...
    /// - Creates an in-memory baseline snapshot for files that already exist on disk when first
    ///   seen.
    /// - For additions, we intentionally do not create a baseline snapshot so that diffs are proper
    ///   additions.
    ...
    pub fn on_patch_begin(&mut self, changes: &HashMap<PathBuf, FileChange>) {
        for (path, change) in changes.iter() {
            ...
            if !self.external_to_temp_name.contains_key(path) {
                ...
                // If the file exists on disk now, snapshot as baseline; else leave missing to
                // represent /dev/null.
    ...
    /// Find the git worktree root for a file/directory by walking up to the first ancestor
    /// containing a `.git` entry. Uses a simple cache of known roots and avoids negative-result
    /// caching for simplicity.
...
mod tests {
    ...
    fn normalize_diff_for_test(input: &str, root: &Path) -> String {
        ...
        // Split into blocks on lines starting with "diff --git ", sort blocks for determinism, and
        // rejoin
