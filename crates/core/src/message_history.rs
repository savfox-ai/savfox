...
//! Note: `conversation_id` stores the session id; the field name is preserved for
...
/// Filename that stores the message history inside `~/.savfox`.
...
fn history_filepath(config: &Config) -> PathBuf {
    let mut path = config.savfox_home.clone();
...
pub(crate) async fn append_entry(
    ...
    // Resolve `~/.savfox/history.jsonl` and ensure the parent directory exists.
...
mod tests {
    ...
    async fn append_entry_trims_history_when_beyond_max_bytes() {
        ...
        let history_path = savfox_home.path().join("history.jsonl");
