...
use savfox_protocol::models::{FunctionCallOutputPayload, ResponseInputItem, ResponseItem};
...
pub(crate) async fn handle_output_item_done(
    ...
    match ToolRouter::build_tool_call(ctx.sess.as_ref(), item.clone()).await {
        // The model emitted a tool call; log it, persist the item immediately, and queue the tool
        // execution.
        Ok(Some(call)) => {
            let payload_preview = call.payload.log_payload().into_owned();
            tracing::info!(
                session_id = %ctx.sess.conversation_id,
                "ToolCall: {} {}",
                call.tool_name,
                payload_preview
            );
        ...
        // Guardrail: the model issued a LocalShellCall without an id; surface the error back into
        // history.
        ...
        // The tool request should be answered directly (or was denied); push that response into the
        // transcript.
        ...
        Err(FunctionCallError::Fatal(message)) => {
            return Err(SavfoxError::Fatal(message));
        }
    }
        ...
        | ResponseItem::WebSearchCall { .. } => {
            ...
            if plan_mode && let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                ...
                agent_message.content =
                    vec![savfox_protocol::items::AgentMessageContent::Text { text: stripped }];
            }
            Some(turn_item)
        }
        ...
        && role == "assistant"
    {
        let combined = content
            ...
            .filter_map(|ci| match ci {
                savfox_protocol::models::ContentItem::OutputText { text } => Some(text.as_str()),
                _ => None,
            })
            .collect::<String>();
