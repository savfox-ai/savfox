...
use savfox_rmcp_client::perform_oauth_login;
use tokio_util::sync::CancellationToken;
use tracing::warn;

use super::auth::{McpOAuthLoginSupport, oauth_login_support};
use super::effective_mcp_servers;
use crate::config::edit::ConfigEditsBuilder;
use crate::config::types::{McpServerConfig, McpServerTransportConfig};
use crate::config::{Config, load_global_mcp_servers};
use crate::default_client::{is_first_party_originator, originator};
...
pub(crate) async fn maybe_install_mcp_dependencies(
    ...
    let mut servers = match load_global_mcp_servers(&savfox_home).await {
        Ok(servers) => servers,
        Err(err) => {
    ...
    if let Err(err) = ConfigEditsBuilder::new(&savfox_home)
