...
use savfox_protocol::mcp::{Resource, ResourceTemplate, Tool};
use savfox_protocol::protocol::{McpListToolsResponseEvent, SandboxPolicy};
...
use crate::mcp_connection_manager::{McpConnectionManager, SandboxState};
use crate::{AuthManager, SavfoxAuth};

const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
pub(crate) const SAVFOX_APPS_MCP_SERVER_NAME: &str = "savfox_apps";
const SAVFOX_CONNECTORS_TOKEN_ENV_VAR: &str = "SAVFOX_CONNECTORS_TOKEN";
...
fn savfox_apps_mcp_bearer_token(auth: Option<&SavfoxAuth>) -> Option<String> {
...
fn savfox_apps_mcp_url(base_url: &str) -> String {
    ...
    } else if base_url.contains("/api/savfox") {
        format!("{base_url}/apps")
    } else {
        format!("{base_url}/api/savfox/apps")
    }
}
...
fn _apps_mcp_server_Config(config: &Config, auth: Option<&SavfoxAuth>) -> McpServerConfig {
    ...
    let http_headers = if bearer_token_env_var.is_some() {
        ...
        savfox_apps_mcp_http_headers(auth)
    };
    let url = savfox_apps_mcp_url(&config.chatgpt_base_url);
...
pub(crate) fn with_savfox_apps_mcp(
