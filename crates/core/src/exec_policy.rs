...
use savfox_exec_policy::{
    AmendError, Decision, Error as ExecPolicyRuleError, Evaluation, Policy, PolicyParser,
    RuleMatch, blocking_append_allow_prefix_rule,
};
...
use crate::config_loader::{ConfigLayerStack, ConfigLayerStackOrdering};
...
pub enum ExecPolicyError {
    ...
    ParsePolicy {
        path: String,
        source: savfox_exec_policy::Error,
    },
}
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(savfox_home);
...
pub async fn load_exec_policy(config_stack: &ConfigLayerStack) -> Result<Policy, ExecPolicyError> {
    ...
    // Include disabled project layers so .savfox/rules still applies when
...
fn default_policy_path(savfox_home: &Path) -> PathBuf {
    savfox_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}

/// Derive a proposed execpolicy amendment when a command requires user approval
/// - If any execpolicy rule prompts, return None, because an amendment would not skip that policy
///   requirement.
/// - Otherwise return the first heuristics Prompt.
/// - Examples:
/// - execpolicy: empty. Command: `["python"]`. Heuristics prompt -> `Some(vec!["python"])`.
/// - execpolicy: empty. Command: `["bash", "-c", "cd /some/folder && prog1 --option1 arg1 && prog2
///   --option2 arg2"]`. Parsed commands include `cd /some/folder`, `prog1 --option1 arg1`, and
///   `prog2 --option2 arg2`. If heuristics allow `cd` but prompt on `prog1`, we return
///   `Some(vec!["prog1", "--option1", "arg1"])`.
/// - execpolicy: contains a `prompt for prefix ["prog2"]` rule. For the same command as above, we
///   return `None` because an execpolicy prompt still applies even if we amend execpolicy to allow
///   ["prog1", "--option1", "arg1"].
...
/// - Note: we only use this amendment when the command fails to run in sandbox and savfox prompts
///   the user to run outside the sandbox
/// - The purpose of this amendment is to bypass sandbox for similar commands in the future
/// - If any execpolicy rule matches, return None, because we would already be running command
///   outside the sandbox
...
mod tests {
    ...
    fn config_stack_for_dot_savfox_folder(dot_savfox_folder: &Path) -> ConfigLayerStack {
    ...
    async fn returns_empty_policy_when_feature_disabled() {
        ...
        let config_stack = config_stack_for_dot_savfox_folder(temp_dir.path());
    ...
    async fn loads_rules_from_disabled_project_layers() -> anyhow::Result<()> {
        ...
        let project_dot_savfox_folder = AbsolutePathBuf::from_absolute_path(project_dir.path())?;
        let layers = vec![ConfigLayerEntry::new_disabled(
            ConfigLayerSource::Project {
                dot_savfox_folder: project_dot_savfox_folder,
            },
            TomlValue::Table(Default::default()),
            "trust disabled",
        )];
...
"#;
    ...
    async fn append_execpolicy_amendment_updates_policy_and_file() {
        ...
        manager
            .append_amendment_and_update(savfox_home.path(), &ExecPolicyAmendment::from(prefix))
            .await
        ...
        let contents = fs::read_to_string(default_policy_path(savfox_home.path()))
...
"#
    ...
    async fn append_execpolicy_amendment_rejects_empty_prefix() {
        ...
        let result = manager
            .append_amendment_and_update(savfox_home.path(), &ExecPolicyAmendment::from(vec![]))
            .await;
