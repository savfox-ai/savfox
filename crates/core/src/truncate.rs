...
use savfox_protocol::openai_models::{TruncationMode, TruncationPolicyConfig};
use savfox_protocol::protocol::TruncationPolicy as ProtocolTruncationPolicy;
...
impl TruncationPolicy {
    ...
    /// - For `Bytes`, this is an approximate token budget using the global bytes-per-token
    ///   heuristic.
    ...
    /// - For `Tokens`, this is an approximate byte budget using the global bytes-per-token
    ///   heuristic.
...
mod tests {
    ...
    use super::{
        ...
        truncate_function_output_items_with_policy, truncate_text, truncate_with_token_budget,
    };
