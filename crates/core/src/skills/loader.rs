...
use crate::config_loader::{
    ConfigLayerStack, ConfigLayerStackOrdering, default_project_root_markers, merge_toml_values,
    project_root_markers_from_config,
};
use crate::skills::model::{
    SkillDependencies, SkillError, SkillInterface, SkillLoadOutcome, SkillMetadata,
    SkillToolDependency,
};
        ...
        config_layer_stack.get_layers(ConfigLayerStackOrdering::HighestPrecedenceFirst, true)
    {
        ...
        match &layer.name {
            ...
            ConfigLayerSource::User { .. } => {
                // Deprecated user skills location (`$SAVFOX_HOME/skills`), kept for backward
                // compatibility.
                roots.push(SkillRoot {
                    path: config_folder.as_path().join(SKILLS_DIR_NAME),
                ...
                // Embedded system skills are cached under `$SAVFOX_HOME/skills/.system` and are a
            ...
            ConfigLayerSource::System { .. } => {
                // The system config layer lives under `/etc/savfox/` on Unix, so treat
                // `/etc/savfox/skills` as admin-scoped skills.
                roots.push(SkillRoot {
                    path: config_folder.as_path().join(SKILLS_DIR_NAME),
...
fn discover_skills_under_root(root: &Path, scope: SkillScope, outcome: &mut SkillLoadOutcome) {
    ...
    // Follow symlinked directories for user, admin, and repo skills. System skills are written by
    // Savfox itself.
...
mod tests {
    ...
    use super::*;
    use crate::config::{
        CONFIG_TOML_FILE, ConfigBuilder, ConfigOverrides, ConfigToml, ProjectConfig,
    };
    use crate::config_loader::{
    ...
    -> anyhow::Result<()> {
        ...
        let system_folder = tmp.path().join("etc/savfox");
        let home_folder = tmp.path().join("home");
        let user_folder = home_folder.join("savfox");
    ...
    fn skill_roots_from_layer_stack_includes_disabled_project_layers() -> anyhow::Result<()> {
        ...
        let dot_savfox = project_root.join(".savfox");
        fs::create_dir_all(&dot_savfox)?;

        let user_file = AbsolutePathBuf::from_absolute_path(user_folder.join("config.toml"))?;
        let project_dot_savfox = AbsolutePathBuf::from_absolute_path(&dot_savfox)?;
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_savfox_folder: project_dot_savfox,
                },
                TomlValue::Table(toml::map::Map::new()),
                "marked untrusted",
            ),
        ...
        assert_eq!(
            got,
            vec![
                (SkillScope::Repo, dot_savfox.join("skills")),
                (SkillScope::User, user_folder.join("skills")),
                (
                    SkillScope::User,
                    home_folder.join(AGENTS_DIR_NAME).join(SKILLS_DIR_NAME)
    ...
    fn write_skill(savfox_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
        write_skill_at(&savfox_home.path().join("skills"), dir, name, description)
    }
    ...
    fn write_system_skill(
        ...
        write_skill_at(
            &savfox_home.path().join("skills/.system"),
            dir,
            name,
            description,
        )
    ...
    async fn loads_skill_dependencies_metadata_from_yaml() {
        let savfox_home = tempfile::tempdir().expect("tempdir");
        let skill_path = write_skill(&savfox_home, "demo", "dep-skill", "from json");
...
"#,
        ...
        let cfg = make_config(&savfox_home).await;
    ...
    async fn loads_skill_interface_metadata_from_yaml() {
        let savfox_home = tempfile::tempdir().expect("tempdir");
        let skill_path = write_skill(&savfox_home, "demo", "ui-skill", "from json");
        let skill_dir = skill_path.parent().expect("skill dir");
...
"#,
    ...
    async fn loads_skills_via_symlinked_subdir_for_user_scope() {
        ...
        fs::create_dir_all(savfox_home.path().join("skills")).unwrap();
        symlink_dir(shared.path(), &savfox_home.path().join("skills/shared"));
    ...
    async fn does_not_loop_on_symlink_cycle_for_user_scope() {
        ...
        // Create a cycle:
        //   $SAVFOX_HOME/skills/cycle/loop -> $SAVFOX_HOME/skills/cycle
        let cycle_dir = savfox_home.path().join("skills/cycle");
    ...
    async fn loads_skills_via_symlinked_subdir_for_repo_scope() {
        ...
        let cfg = make_config_for_cwd(&savfox_home, repo_dir.path().to_path_buf()).await;
        let outcome = load_skills(&cfg);
    ...
    async fn system_scope_ignores_symlinked_subdir() {
        ...
        let system_root = savfox_home.path().join("skills/.system");
    ...
    async fn loads_valid_skill() {
        let savfox_home = tempfile::tempdir().expect("tempdir");
        let skill_path = write_skill(&savfox_home, "demo", "demo-skill", "does things\ncarefully");
        let cfg = make_config(&savfox_home).await;
    ...
    async fn skips_hidden_and_invalid() {
        let savfox_home = tempfile::tempdir().expect("tempdir");
        let hidden_dir = savfox_home.path().join("skills/.hidden");
        ...
        let invalid_dir = savfox_home.path().join("skills/invalid");
    ...
    async fn enforces_length_limits() {
        ...
        write_skill(&savfox_home, "max-len", "max-len", &max_desc);
        let cfg = make_config(&savfox_home).await;
        ...
        write_skill(&savfox_home, "too-long", "too-long", &too_long_desc);
        let outcome = load_skills(&cfg);
        assert_eq!(outcome.skills.len(), 1);
        assert_eq!(outcome.errors.len(), 1);
        assert!(
    ...
    #[tokio::test]
    async fn loads_skills_from_agents_dir_without_savfox_dir() {
        let savfox_home = tempfile::tempdir().expect("tempdir");
        let repo_dir = tempfile::tempdir().expect("tempdir");
        mark_as_git_repo(repo_dir.path());
    ...
    #[tokio::test]
    async fn loads_skills_from_all_savfox_dirs_under_project_root() {
        let savfox_home = tempfile::tempdir().expect("tempdir");
        let repo_dir = tempfile::tempdir().expect("tempdir");
        mark_as_git_repo(repo_dir.path());
        ...
        let cfg = make_config_for_cwd(&savfox_home, nested_dir).await;
    ...
    #[tokio::test]
    async fn loads_skills_from_savfox_dir_when_not_git_repo() {
        let savfox_home = tempfile::tempdir().expect("tempdir");
        ...
        let cfg = make_config_for_cwd(&savfox_home, work_dir.path().to_path_buf()).await;
    ...
    async fn keeps_duplicate_names_from_repo_and_user() {
        ...
        let user_skill_path = write_skill(&savfox_home, "user", "dupe-skill", "from user");
    ...
    #[tokio::test]
    async fn keeps_duplicate_names_from_nested_savfox_dirs() {
        let savfox_home = tempfile::tempdir().expect("tempdir");
        let repo_dir = tempfile::tempdir().expect("tempdir");
        mark_as_git_repo(repo_dir.path());
    ...
    async fn repo_skills_search_does_not_escape_repo_root() {
        ...
        let cfg = make_config_for_cwd(&savfox_home, repo_dir).await;
    ...
    async fn loads_skills_when_cwd_is_file_in_repo() {
        ...
        let cfg = make_config_for_cwd(&savfox_home, file_path).await;
    ...
    async fn loads_skills_from_system_cache_when_present() {
        ...
        let skill_path = write_system_skill(&savfox_home, "system", "system-skill", "from system");
