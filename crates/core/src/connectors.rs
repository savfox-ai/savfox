...
pub use savfox_app_server_protocol::AppInfo;
...
use crate::mcp::{SAVFOX_APPS_MCP_SERVER_NAME, with_savfox_apps_mcp};
use crate::mcp_connection_manager::{DEFAULT_STARTUP_TIMEOUT, McpConnectionManager};
use crate::{AuthManager, SandboxState};
...
pub async fn list_accessible_connectors_from_mcp_tools(
    ...
    let mcp_servers = with_savfox_apps_mcp(HashMap::new(), true, auth.as_ref(), config);
    ...
    if let Some(cfg) = mcp_servers.get(SAVFOX_APPS_MCP_SERVER_NAME) {
        let timeout = cfg.startup_timeout_sec.unwrap_or(DEFAULT_STARTUP_TIMEOUT);
        mcp_connection_manager
            .wait_for_server_ready(SAVFOX_APPS_MCP_SERVER_NAME, timeout)
            .await;
    }
...
pub fn connector_install_url(name: &str, connector_id: &str) -> String {
    ...
    format!("https://taidge.com/apps/{slug}/{connector_id}")
}
