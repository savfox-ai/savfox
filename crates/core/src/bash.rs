...
use crate::shell::{ShellType, detect_shell_type};
    ...
    while let Some(node) = stack.pop() {
        ...
        if node.is_named() {
            ...
            if !(ALLOWED_PUNCT_TOKENS.contains(&kind) || kind.trim().is_empty()) {
                // If it's a quote token or operator it's allowed above; we also allow whitespace
                // tokens. Any other punctuation like parentheses, braces,
                // redirects, backticks, etc are rejected.
                return None;
            }
        }
