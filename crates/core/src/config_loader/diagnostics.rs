...
use savfox_utils_absolute_path::AbsolutePathBufGuard;
use serde_path_to_error::{Path as SerdePath, Segment as SerdeSegment};
use toml_edit::{Document, Item, Table, Value};

use super::{ConfigLayerEntry, ConfigLayerStack, ConfigLayerStackOrdering};
...
fn config_path_for_layer(layer: &ConfigLayerEntry) -> Option<PathBuf> {
    match &layer.name {
        ...
        ConfigLayerSource::Project { dot_savfox_folder } => {
            Some(dot_savfox_folder.as_path().join(CONFIG_TOML_FILE))
        }
