...
use super::{LoaderOverrides, load_config_layers_state};
use crate::config::{
    CONFIG_TOML_FILE, CONFIG_YAML_FILE, ConfigBuilder, ConfigOverrides, ConfigToml,
...
#[tokio::test]
async fn loads_user_yaml_config_when_toml_missing() {
    let tmp = tempdir().expect("tempdir");
    std::fs::write(
...
"#,
    ...
    let user_layer = layers.get_user_layer().expect("user layer");
    match &user_layer.name {
        super::ConfigLayerSource::User { file } => {
            assert!(file.as_path().ends_with(CONFIG_YAML_FILE));
        }
...
async fn prefers_toml_when_multiple_user_config_formats_exist() {
    ...
    match &user_layer.name {
        super::ConfigLayerSource::User { file } => {
            assert!(file.as_path().ends_with(CONFIG_TOML_FILE));
        }
        other => panic!("unexpected user layer source: {other:?}"),
    }
    assert_eq!(
...
#[tokio::test]
async fn expands_env_placeholders_in_config_values() {
    let tmp = tempdir().expect("tempdir");
    std::fs::write(
...
#[tokio::test]
async fn errors_when_required_env_placeholder_is_missing() {
    let tmp = tempdir().expect("tempdir");
    std::fs::write(
        tmp.path().join(CONFIG_TOML_FILE),
    ...
    assert!(
        err.to_string().contains("missing required env var"),
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .savfox_home(tmp.path().to_path_buf())
    ...
    let config_error = config_error_from_io(&err);
    let _guard = savfox_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_user_layer()
        .expect("expected a user layer even when SAVFOX_HOME/config.toml does not exist");
    assert_eq!(
        &ConfigLayerEntry {
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".savfox")).await?;
    ...
    .await?;
    tokio::fs::write(
        nested.join(".savfox").join(CONFIG_TOML_FILE),
        "foo = \"child\"\n",
    )
    .await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            super::ConfigLayerSource::Project { dot_savfox_folder } => Some(dot_savfox_folder),
            _ => None,
        })
        .collect();
    ...
    assert_eq!(
        ...
        nested.join(".savfox").as_path()
    );
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".savfox").as_path()
    );
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_savfox_and_override_in_order() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    ...
    .await?;
    tokio::fs::write(nested.join(".savfox").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".savfox").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".savfox").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let savfox_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&savfox_home).await?;
    tokio::fs::write(savfox_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_savfox_exists_without_config_toml() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: super::ConfigLayerSource::Project {
                dot_savfox_folder: AbsolutePathBuf::from_absolute_path(
                    project_root.join(".savfox")
                )?,
            },
            config: TomlValue::Table(toml::map::Map::new()),
            version: version_for_toml(&TomlValue::Table(toml::map::Map::new())),
...
#[tokio::test]
async fn savfox_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let savfox_home = home_dir.join(".savfox");
    tokio::fs::create_dir_all(&savfox_home).await?;
    tokio::fs::write(savfox_home.join(CONFIG_TOML_FILE), "foo = \"user\"\n").await?;
...
#[tokio::test]
async fn savfox_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_savfox = project_root.join(".savfox");
    let nested_dot_savfox = nested.join(".savfox");

    tokio::fs::create_dir_all(&nested_dot_savfox).await?;
    tokio::fs::create_dir_all(project_root.join(".git")).await?;
    tokio::fs::write(
        nested_dot_savfox.join(CONFIG_TOML_FILE),
        "foo = \"child\"\n",
    )
    .await?;

    tokio::fs::create_dir_all(&project_dot_savfox).await?;
    ...
    .await?;
    let user_config_path = project_dot_savfox.join(CONFIG_TOML_FILE);
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: super::ConfigLayerSource::Project {
                dot_savfox_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_savfox)?,
            },
...
async fn project_layers_disabled_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    let savfox_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&savfox_home_untrusted).await?;
    make_config_for_test(
        &savfox_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        None,
    )
    .await?;
    let untrusted_config_path = savfox_home_untrusted.join(CONFIG_TOML_FILE);
    ...
    tokio::fs::create_dir_all(&savfox_home_unknown).await?;
    tokio::fs::write(
        savfox_home_unknown.join(CONFIG_TOML_FILE),
        "foo = \"user\"\n",
    )
    .await?;
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".savfox")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".savfox").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let savfox_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&savfox_home).await?;
        let config_path = savfox_home.join(CONFIG_TOML_FILE);

        if let Some(trust_level) = trust_level {
            make_config_for_test(&savfox_home, &project_root, trust_level, None).await?;
...
mod requirements_exec_policy_tests {
    ...
    use super::super::requirements_exec_policy::{
        RequirementsExecPolicyDecisionToml, RequirementsExecPolicyParseError,
        RequirementsExecPolicyPatternTokenToml, RequirementsExecPolicyPrefixRuleToml,
    ...
    fn config_stack_for_dot_savfox_folder_with_requirements(
        dot_savfox_folder: &Path,
        requirements: ConfigRequirements,
    ) -> ConfigLayerStack {
        let dot_savfox_folder = AbsolutePathBuf::from_absolute_path(dot_savfox_folder)
            .expect("absolute dot_savfox_folder");
        let layer = ConfigLayerEntry::new(
            ConfigLayerSource::Project { dot_savfox_folder },
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_savfox_folder_with_requirements(temp_dir.path(), requirements);
