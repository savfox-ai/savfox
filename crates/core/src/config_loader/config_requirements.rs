...
pub use savfox_config::RequirementSource;
use savfox_config::{Constrained, ConstraintError};
use savfox_protocol::config_types::SandboxMode;
use savfox_protocol::protocol::{AskForApproval, SandboxPolicy};
use serde::Deserialize;

use super::requirements_exec_policy::{RequirementsExecPolicy, RequirementsExecPolicyToml};
...
/// Base config deserialized from /etc/savfox/requirements.toml or MDM.
...
mod tests {
    ...
    fn constraint_error_includes_requirement_source() -> Result<()> {
        ...
        let requirements_toml_file = if cfg!(windows) {
            "C:\\etc\\savfox\\requirements.toml"
        } else {
    ...
    fn exec_policy_error_includes_requirement_source() -> Result<()> {
        ...
        let requirements_toml_file =
            AbsolutePathBuf::from_absolute_path("/etc/savfox/requirements.toml")?;
        let source_location = RequirementSource::SystemRequirementsToml {
            file: requirements_toml_file,
        };
