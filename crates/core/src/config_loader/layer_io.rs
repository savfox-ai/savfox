...
use super::diagnostics::{config_error_from_toml, io_error_from_config_error};
#[cfg(target_os = "macos")]
use super::macos::load_managed_admin_config_layer;

#[cfg(unix)]
const SAVFOX_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/savfox/managed_config.toml";
...
pub(super) struct LoadedConfigLayers {
    /// If present, data read from a file such as `/etc/savfox/managed_config.toml`.
...
pub(super) async fn load_config_layers_internal(
    ...
    let managed_config_path = AbsolutePathBuf::from_absolute_path(
        managed_config_path.unwrap_or_else(|| managed_config_default_path(savfox_home)),
    )?;
...
pub(super) fn managed_config_default_path(savfox_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = savfox_home;
        PathBuf::from(SAVFOX_MANAGED_CONFIG_SYSTEM_PATH)
    }

    #[cfg(not(unix))]
    {
        savfox_home.join("managed_config.toml")
    }
}
