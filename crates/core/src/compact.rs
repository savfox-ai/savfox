...
use savfox_protocol::models::{ContentItem, ResponseInputItem, ResponseItem};
...
use crate::{ModelProviderInfo, Prompt};
...
async fn run_compact_task_inner(
    ...
    loop {
        ...
        match attempt_result {
            Ok(()) => {
                if truncated_count > 0 {
                    sess.notify_background_event(
                        turn_context.as_ref(),
                        format!(
                            "Trimmed {truncated_count} older session item(s) before compacting so the prompt fits the model context window."
                        ),
                    )
                    .await;
                }
            ...
            Err(SavfoxError::Interrupted) => {
                return;
            }
            Err(e @ SavfoxError::ContextWindowExceeded) => {
                if turn_input_len > 1 {
