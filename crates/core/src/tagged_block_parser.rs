...
///   matching any tag prefix (after `trim_start`).
/// - If it stops matching a tag prefix, the buffered line is immediately emitted as text and we
///   continue in "plain text" mode until the next newline.
/// - When a full line is available, we compare it to the open/close tags; tag lines emit
///   TagStart/TagEnd, otherwise the line is emitted as text.
/// - `finish()` flushes any buffered line and auto-closes an unterminated tag, which keeps
///   streaming resilient to missing closing tags.
...
mod tests {
    ...
    use super::{TagSpec, TaggedLineParser, TaggedLineSegment};
