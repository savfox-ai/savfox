...
use crate::rollout::list::find_session_path_by_id_str;
use crate::shell::{Shell, ShellType, get_shell};
...
impl ShellSnapshot {
    pub fn start_snapshotting(
        ...
        let snapshot_span = info_span!("shell_snapshot", session_id = %snapshot_session_id);
        tokio::spawn(
            async move {
    ...
    async fn try_new(savfox_home: &Path, session_id: SessionId, shell: &Shell) -> Option<Self> {
        ...
        tokio::spawn(async move {
            if let Err(err) = cleanup_stale_snapshots(&savfox_home, cleanup_session_id).await {
...
async fn run_script_with_timeout(
    ...
    savfox_utils::pty::process_group::configure_command_pre_exec(&mut handler, true);
...
pub async fn cleanup_stale_snapshots(
    ...
    let snapshot_dir = savfox_home.join(SNAPSHOT_DIR);
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path = find_session_path_by_id_str(savfox_home, session_id).await?;
...
mod tests {
    ...
    fn bash_snapshot_filters_invalid_exports() -> Result<()> {
        let output = Command::new("/bin/bash")
            ...
            .env("VALID_NAME", "ok")
            .env("PWD", "/tmp/stale")
            .env("NEXTEST_BIN_EXE_savfox-write-config-schema", "/path/to/bin")
            .env("BAD-NAME", "broken")
            .output()?;
        ...
        assert!(!stdout.contains("NEXTEST_BIN_EXE_savfox-write-config-schema"));
    ...
    async fn try_new_creates_and_deletes_snapshot_file() -> Result<()> {
        ...
        let snapshot = ShellSnapshot::try_new(dir.path(), SessionId::new(), &shell)
            .await
    ...
    async fn snapshot_shell_does_not_inherit_stdin() -> Result<()> {
        ...
        let output = run_script_with_timeout(&shell, &script, SNAPSHOT_TIMEOUT, true)
            .await
    ...
    async fn write_rollout_stub(savfox_home: &Path, session_id: SessionId) -> Result<PathBuf> {
        let dir = savfox_home.join("sessions");
    ...
    async fn cleanup_stale_snapshots_removes_stale_rollouts() -> Result<()> {
        ...
        let rollout_path = write_rollout_stub(savfox_home, stale_session).await?;
    ...
    async fn cleanup_stale_snapshots_skips_active_session() -> Result<()> {
        ...
        let rollout_path = write_rollout_stub(savfox_home, active_session).await?;
        fs::write(&active_snapshot, "active").await?;

        set_file_mtime(&rollout_path, SNAPSHOT_RETENTION + Duration::from_secs(60))?;

        cleanup_stale_snapshots(savfox_home, active_session).await?;
