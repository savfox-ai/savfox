...
use savfox_protocol::protocol::{RolloutItem, SessionSource};
pub use savfox_state::LogEntry;
use savfox_state::{DB_METRIC_COMPARE_ERROR, STATE_DB_FILENAME, SessionMetadataBuilder};
...
use crate::rollout::list::{Cursor, SessionSortKey};
use crate::rollout::metadata;

/// Core-facing handle to the optional SQLite-backed state runtime.
pub type StateDbHandle = Arc<savfox_state::StateRuntime>;
...
pub(crate) async fn init_if_enabled(
    ...
    let state_path = config.savfox_home.join(STATE_DB_FILENAME);
    if !config.features.enabled(Feature::Sqlite) {
        return None;
    }
    let existed = tokio::fs::try_exists(&state_path).await.unwrap_or(false);
    let runtime = match savfox_state::StateRuntime::init(
    ...
    .await
    {
        ...
        Err(err) => {
            warn!(
                ...
                config.savfox_home.display()
            );
            if let Some(otel) = otel {
                otel.counter("savfox.db.init", 1, &[("status", "init_error")]);
            }
            return None;
        }
    };
...
pub async fn get_state_db(config: &Config, otel: Option<&OtelManager>) -> Option<StateDbHandle> {
    ...
    savfox_state::StateRuntime::init(
        config.savfox_home.clone(),
        config.model_provider_id.clone(),
        otel.cloned(),
    )
...
pub async fn open_if_present(savfox_home: &Path, default_provider: &str) -> Option<StateDbHandle> {
    let db_path = savfox_home.join(STATE_DB_FILENAME);
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<savfox_state::Anchor> {
    let cursor = cursor?;
    ...
    .with_nanosecond(0)?;
    Some(savfox_state::Anchor { ts, id })
}

/// List session ids from SQLite for parity checks without rollout scanning.
#[allow(clippy::too_many_arguments)]
pub async fn list_session_ids_db(
    context: Option<&savfox_state::StateRuntime>,
    savfox_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.savfox_home() != savfox_home {
        warn!(
            "state db savfox_home mismatch: expected {}, got {}",
            ctx.savfox_home().display(),
            savfox_home.display()
        );
    }
    ...
    match ctx
        .list_session_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                SessionSortKey::CreatedAt => savfox_state::SortKey::CreatedAt,
                SessionSortKey::UpdatedAt => savfox_state::SortKey::UpdatedAt,
            },
        ...
        .await
    {
        Ok(ids) => Some(ids),
        Err(err) => {
            warn!("state db list_session_ids failed during {stage}: {err}");
            None
        }
    }
}

/// Look up the rollout path for a session id using SQLite.
pub async fn find_rollout_path_by_id(
    ...
    let ctx = context?;
    ctx.find_rollout_path_by_id(session_id, archived_only)
        .await
        .unwrap_or_else(|err| {
...
/// Get dynamic tools for a session id using SQLite.
pub async fn get_dynamic_tools(
    ...
    let ctx = context?;
    match ctx.get_dynamic_tools(session_id).await {
        Ok(tools) => tools,
        Err(err) => {
...
/// Persist dynamic tools for a session id using SQLite, if none exist yet.
pub async fn persist_dynamic_tools(
    ...
    if let Err(err) = ctx.persist_dynamic_tools(session_id, tools).await {
...
pub async fn reconcile_rollout(
    ...
    builder: Option<&SessionMetadataBuilder>,
    items: &[RolloutItem],
) {
    let Some(ctx) = context else {
        return;
    ...
    if let Err(err) = ctx.upsert_session(&outcome.metadata).await {
        warn!(
...
pub fn record_discrepancy(stage: &str, reason: &str) {
    ...
    if let Some(metric) = savfox_otel::metrics::global() {
