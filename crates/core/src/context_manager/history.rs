...
use savfox_protocol::protocol::{TokenUsage, TokenUsageInfo};

use crate::context_manager::normalize;
use crate::instructions::{SkillInstructions, UserInstructions};
use crate::savfox::TurnContext;
use crate::session_prefix::is_session_prefix;
use crate::truncate::{
    TruncationPolicy, approx_token_count, approx_tokens_from_byte_count,
    truncate_function_output_items_with_policy, truncate_text,
};
use crate::user_shell_command::is_user_shell_command_text;

/// Transcript of session history
...
impl ContextManager {
    ...
    ///
    /// This mirrors session-rollback semantics:
    ...
    fn get_trailing_savfox_generated_items_tokens(&self) -> i64 {
        let mut total = 0i64;
        for item in self.items.iter().rev() {
            if !is_savfox_generated_item(item) {
                break;
            }
    ...
    pub(crate) fn get_total_token_usage(&self, server_reasoning_included: bool) -> i64 {
        let last_tokens = self
            ...
            .unwrap_or(0);
        let trailing_savfox_generated_tokens = self.get_trailing_savfox_generated_items_tokens();
        if server_reasoning_included {
            last_tokens.saturating_add(trailing_savfox_generated_tokens)
        } else {
            last_tokens
                .saturating_add(self.get_non_last_reasoning_items_tokens())
                .saturating_add(trailing_savfox_generated_tokens)
        }
    }
...
pub(crate) fn is_savfox_generated_item(item: &ResponseItem) -> bool {
    matches!(
        item,
