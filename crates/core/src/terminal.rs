...
/// - If `TERM_PROGRAM=tmux`, the tmux client term type/name are used instead. The client term type
///   is split on whitespace to extract a program name plus optional version (for example, `ghostty
///   1.2.3`), while the client term name becomes the `TERM` capability string.
