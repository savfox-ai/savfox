        ...
        match lower.as_str() {
            ...
            _ if lower.starts_with("-command:") || lower.starts_with("/command:") => {
                if idx + 1 != args.len() {
                    ...
                    // Examples rejected here: "pwsh -Command:dir C:\\" and "powershell /Command:dir
                    // C:\\" with trailing args.
                    return None;
                }
            ...
            | "-workingdirectory" => {
                // Examples rejected here: "pwsh -EncodedCommand ..." and "powershell -File
                // script.ps1".
                return None;
            }
...
/// as a flat argv vector when possible. If parsing fails or the AST includes unsupported
/// constructs, we conservatively reject the command instead of trying to split it manually.
fn parse_with_powershell_ast(executable: &str, script: &str) -> PowershellParseOutcome {
    ...
    match Command::new(executable)
        ...
        .env("SAVFOX_POWERSHELL_PAYLOAD", &encoded_script)
        .output()
    {
...
fn is_safe_powershell_command(words: &[String]) -> bool {
    ...
    for w in words.iter() {
        ...
        if matches!(
            ...
            // Examples rejected here: "Write-Output (Set-Content foo6.txt 'abc')" and "Get-Content
            // (New-Item bar.txt)".
            return false;
        }
    }
    ...
    match command.as_str() {
        ...
        | "copy-item" | "rename-item" | "start-process" | "stop-process" => {
            // Examples rejected here: "pwsh -Command 'Set-Content notes.txt data'" and "pwsh
            // -Command 'Remove-Item temp.log'".
            false
        }
...
fn is_safe_ripgrep(words: &[String]) -> bool {
    ...
    !words.iter().skip(1).any(|arg| {
        ...
        // Examples rejected here: "pwsh -Command 'rg --pre cat pattern'" and "pwsh -Command 'rg
        // --search-zip pattern'".
        ...
        if arg.starts_with('-') {
            if arg.eq_ignore_ascii_case("-c") || arg.eq_ignore_ascii_case("--config") {
                if iter.next().is_none() {
                    // Examples rejected here: "pwsh -Command 'git -c'" and "pwsh -Command 'git
                    // --config'".
                    return false;
                }
                continue;
            ...
            if arg.eq_ignore_ascii_case("--git-dir") || arg.eq_ignore_ascii_case("--work-tree") {
                if iter.next().is_none() {
                    // Examples rejected here: "pwsh -Command 'git --git-dir'" and "pwsh -Command
                    // 'git --work-tree'".
                    return false;
                }
                continue;
