...
use crate::token_data::{
    IdTokenInfo, KnownPlan as InternalKnownPlan, PlanType as InternalPlanType, TokenData,
    parse_id_token,
};
...
struct ChatgptAuthState {
    ...
    client: SavfoxHttpClient,
}

impl PartialEq for SavfoxAuth {
...
pub const REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR: &str = "SAVFOX_REFRESH_TOKEN_URL_OVERRIDE";
...
impl SavfoxAuth {
    fn from_auth_dot_json(
        ...
        match auth_mode {
            ApiAuthMode::Chatgpt => {
                let storage = create_auth_storage(savfox_home.to_path_buf(), storage_mode);
    ...
    fn from_api_key_with_client(api_key: &str, _client: SavfoxHttpClient) -> Self {
...
impl ChatgptAuth {
    ...
    fn client(&self) -> &SavfoxHttpClient {
        &self.state.client
    }
}
...
pub fn logout(
    ...
    let storage = create_auth_storage(savfox_home.to_path_buf(), auth_credentials_store_mode);
    storage.delete()
}
...
pub fn login_with_api_key(
    ...
    save_auth(savfox_home, &auth_dot_json, auth_credentials_store_mode)
}
...
fn logout_all_stores(
    ...
    if auth_credentials_store_mode == AuthCredentialsStoreMode::Ephemeral {
        return logout(savfox_home, AuthCredentialsStoreMode::Ephemeral);
    }
    let removed_ephemeral = logout(savfox_home, AuthCredentialsStoreMode::Ephemeral)?;
    let removed_managed = logout(savfox_home, auth_credentials_store_mode)?;
...
fn load_auth(
    ...
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<Option<SavfoxAuth>> {
    let build_auth = |auth_dot_json: AuthDotJson, storage_mode| {
        let client = crate::default_client::create_client();
        SavfoxAuth::from_auth_dot_json(savfox_home, auth_dot_json, storage_mode, client)
    };

    // API key via env var takes precedence over any other auth method.
    if enable_savfox_api_key_env && let Some(api_key) = read_savfox_api_key_from_env() {
        let client = crate::default_client::create_client();
        return Ok(Some(SavfoxAuth::from_api_key_with_client(
            api_key.as_str(),
            client,
        )));
    }
...
impl Debug for CachedAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CachedAuth")
            .field(
                "auth_mode",
                &self.auth.as_ref().map(SavfoxAuth::api_auth_mode),
            )
            .field(
                "external_refresher",
...
// UnauthorizedRecovery is a state machine that handles an attempt to refresh the authentication
// when requests to API fail with 401 status code.
...
// 1. Attempt to reload the auth data from disk. We only reload if the account id matches the one
//    the current process is running as.
...
//
// For external ChatGPT auth tokens (chatgptAuthTokens), UnauthorizedRecovery does not touch disk or
// refresh tokens locally. Instead it calls the ExternalAuthRefresher
// (account/chatgptAuthTokens/refresh) to ask the parent app for new tokens, stores them in the
// ephemeral auth store, and retries once.
...
impl UnauthorizedRecovery {
    fn new(manager: Arc<AuthManager>) -> Self {
        ...
        let expected_account_id = cached_auth.as_ref().and_then(SavfoxAuth::get_account_id);
        let mode = if cached_auth
            .as_ref()
            .is_some_and(SavfoxAuth::is_external_chatgpt_tokens)
        {
...
impl AuthManager {
    ...
    pub fn from_auth_for_testing(auth: SavfoxAuth) -> Arc<Self> {
        ...
        Arc::new(Self {
            savfox_home: PathBuf::from("non-existent"),
            inner: RwLock::new(cached),
            enable_savfox_api_key_env: false,
    ...
    fn reload_if_account_id_matches(&self, expected_account_id: Option<&str>) -> ReloadOutcome {
        ...
        let new_auth = self.load_auth_from_storage();
        let new_account_id = new_auth.as_ref().and_then(SavfoxAuth::get_account_id);
    ...
    fn auths_equal(a: Option<&SavfoxAuth>, b: Option<&SavfoxAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => a == b,
            _ => false,
    ...
    fn load_auth_from_storage(&self) -> Option<SavfoxAuth> {
        load_auth(
            &self.savfox_home,
            self.enable_savfox_api_key_env,
    ...
    pub async fn refresh_token(&self) -> Result<(), RefreshTokenError> {
        ...
        match auth {
            SavfoxAuth::ChatgptAuthTokens(_) => {
                self.refresh_external_auth(ExternalAuthRefreshReason::Unauthorized)
                    .await
            }
            SavfoxAuth::Chatgpt(chatgpt_auth) => {
    ...
    pub fn logout(&self) -> std::io::Result<bool> {
        let removed = logout_all_stores(&self.savfox_home, self.auth_credentials_store_mode)?;
    ...
    pub fn get_auth_mode(&self) -> Option<ApiAuthMode> {
        self.auth_cached().as_ref().map(SavfoxAuth::api_auth_mode)
    }

    pub fn get_internal_auth_mode(&self) -> Option<AuthMode> {
        self.auth_cached()
            .as_ref()
            .map(SavfoxAuth::internal_auth_mode)
    }

    async fn refresh_if_stale(&self, auth: &SavfoxAuth) -> Result<bool, RefreshTokenError> {
        let chatgpt_auth = match auth {
            SavfoxAuth::Chatgpt(chatgpt_auth) => chatgpt_auth,
            _ => return Ok(false),
        };
    ...
    async fn refresh_external_auth(
        ...
        let previous_account_id = self
            .auth_cached()
            .as_ref()
            .and_then(SavfoxAuth::get_account_id);
...
mod tests {
    ...
    use crate::token_data::{
        IdTokenInfo, KnownPlan as InternalKnownPlan, PlanType as InternalPlanType,
    };

    #[tokio::test]
    async fn refresh_without_id_token() {
        let savfox_home = tempdir().unwrap();
    ...
    fn missing_auth_json_returns_none() {
        let dir = tempdir().unwrap();
        let auth = SavfoxAuth::from_auth_storage(dir.path(), AuthCredentialsStoreMode::File)
    ...
    async fn pro_account_with_no_api_key_uses_chatgpt_auth() {
        ...
        let auth = super::load_auth(savfox_home.path(), false, AuthCredentialsStoreMode::File)
            .unwrap()
            .unwrap();
    ...
    async fn enforce_login_restrictions_logs_out_for_method_mismatch() {
        ...
        let config = build_config(savfox_home.path(), Some(ForcedLoginMethod::Chatgpt), None).await;
    ...
    async fn enforce_login_restrictions_blocks_env_api_key_when_chatgpt_required() {
        let _guard = EnvVarGuard::set(SAVFOX_API_KEY_ENV_VAR, "sk-env");
        let savfox_home = tempdir().unwrap();
