...
///
/// Rollouts can contain `SessionRolledBack` markers. Those markers indicate that the
/// last N user turns were removed from the effective session history; we apply them here so
...
mod tests {
    ...
    #[test]
    fn truncates_rollout_from_start_applies_session_rollback_markers() {
        let rollout_items = vec![
            RolloutItem::ResponseItem(user_msg("u1")),
            RolloutItem::ResponseItem(assistant_msg("a1")),
            RolloutItem::ResponseItem(user_msg("u2")),
            RolloutItem::ResponseItem(assistant_msg("a2")),
            RolloutItem::EventMsg(EventMsg::SessionRolledBack(SessionRolledBackEvent {
                num_turns: 1,
            })),
