...
use savfox_protocol::protocol::{
    AskForApproval, RolloutItem, SandboxPolicy, SessionMetaLine, SessionSource,
};
use savfox_state::{
    BackfillStats, DB_ERROR_METRIC, DB_METRIC_BACKFILL, DB_METRIC_BACKFILL_DURATION_MS,
    ExtractionOutcome, SessionMetadataBuilder, apply_rollout_item,
};
...
pub(crate) fn builder_from_items(
    ...
    let id = SessionId::from_string(&uuid.to_string()).ok()?;
    Some(SessionMetadataBuilder::new(
        id,
        rollout_path.to_path_buf(),
        created_at,
...
pub(crate) async fn extract_metadata_from_rollout(
    ...
    let (items, _session_id, parse_errors) =
...
pub(crate) async fn backfill_sessions(
    runtime: &savfox_state::StateRuntime,
    config: &Config,
    otel: Option<&OtelManager>,
) {
    let timer = otel.and_then(|otel| otel.start_timer(DB_METRIC_BACKFILL_DURATION_MS, &[]).ok());
    let sessions_root = config.savfox_home.join(rollout::SESSIONS_SUBDIR);
    let archived_root = config.savfox_home.join(rollout::ARCHIVED_SESSIONS_SUBDIR);
    ...
    for (path, archived) in rollout_paths {
        ...
        match extract_metadata_from_rollout(&path, config.model_provider_id.as_str(), otel).await {
            Ok(outcome) => {
                ...
                if let Err(err) = runtime.upsert_session(&metadata).await {
