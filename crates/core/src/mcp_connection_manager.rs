...
use savfox_protocol::protocol::{
    Event, EventMsg, McpStartupCompleteEvent, McpStartupFailure, McpStartupStatus,
    McpStartupUpdateEvent, SandboxPolicy,
};
...
use crate::savfox::INITIAL_SUBMIT_ID;
...
pub const MCP_SANDBOX_STATE_CAPABILITY: &str = "savfox/sandbox-state";

/// Custom MCP request to push sandbox state updates.
/// When used, the `params` field of the notification is [`SandboxState`].
pub const MCP_SANDBOX_STATE_METHOD: &str = "savfox/sandbox-state/update";
...
impl McpConnectionManager {
    ...
    pub async fn list_all_tools(&self) -> HashMap<String, ToolInfo> {
        ...
        for (server_name, managed_client) in &self.clients {
            let client = if server_name == SAVFOX_APPS_MCP_SERVER_NAME {
                // Avoid blocking on savfox_apps_mcp startup; use tools only when ready.
...
fn normalize_savfox_apps_tool_title(
    server_name: &str,
        ...
        .map(|tool| {
            ...
            if let Some(title) = tool_def.title.as_deref() {
                let normalized_title =
                    normalize_savfox_apps_tool_title(server_name, connector_name.as_deref(), title);
...
fn mcp_init_error_display(
    ...
    {
        format!(
            "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = SAVFOX_GITHUB_PERSONAL_ACCESS_TOKEN"
        )
    } else if is_mcp_client_auth_required_error(err) {
        format!(
            "The {server_name} MCP server is not logged in. Run `savfox mcp login {server_name}`."
        )
