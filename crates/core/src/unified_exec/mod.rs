...
//! - Uses the shared ToolOrchestrator to handle approval, sandbox selection, and retry semantics in
//!   a single, descriptive flow.
//! - Spawns the PTY from a sandbox‑transformed `ExecEnv`; on sandbox denial, retries without
//!   sandbox when policy allows (no re‑prompt thanks to caching).
//! - Uses the shared `is_likely_sandbox_denied` heuristic to keep denial messages consistent with
//!   other exec paths.
//!
...
mod tests {
    ...
    async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
        ...
        write_stdin(
            &session,
            process_id,
            "export SAVFOX_INTERACTIVE_SHELL_VAR=savfox\n",
            2_500,
        )
        .await?;
        ...
        let out_2 = write_stdin(
            &session,
            process_id,
            "echo $SAVFOX_INTERACTIVE_SHELL_VAR\n",
            2_500,
        )
        .await?;
        assert!(
            out_2.output.contains("savfox"),
    ...
    async fn multi_unified_exec_sessions() -> anyhow::Result<()> {
        ...
        let out_2 =
            exec_command(&session, &turn, "echo $SAVFOX_INTERACTIVE_SHELL_VAR", 2_500).await?;
    ...
    async fn unified_exec_timeouts() -> anyhow::Result<()> {
        ...
        write_stdin(
            &session,
            process_id,
            format!("export SAVFOX_INTERACTIVE_SHELL_VAR={TEST_VAR_VALUE}\n").as_str(),
            2_500,
        )
        .await?;
        ...
        let out_2 = write_stdin(
            &session,
            process_id,
            "sleep 5 && echo $SAVFOX_INTERACTIVE_SHELL_VAR\n",
            10,
        )
        .await?;
        assert!(
    ...
    async fn requests_with_large_timeout_are_capped() -> anyhow::Result<()> {
        ...
        let result = exec_command(&session, &turn, "echo savfox", 120_000).await?;

        assert!(result.process_id.is_some());
        assert!(result.output.contains("savfox"));
    ...
    async fn completed_commands_do_not_persist_sessions() -> anyhow::Result<()> {
        let (session, turn) = test_session_and_turn().await;
        let result = exec_command(&session, &turn, "echo savfox", 2_500).await?;
