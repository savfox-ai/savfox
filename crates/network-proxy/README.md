# savfox-network-proxy

`savfox-network-proxy` is Savfox's local network policy enforcement proxy. It runs:
...
`savfox-network-proxy` reads from Savfox's merged `config.toml` (via `savfox-core` config loading).
...
```bash
cargo run -p savfox-network-proxy --
```
...
`savfox-network-proxy` can be embedded as a library with a thin API:

```rust
use savfox_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
This section documents the protections implemented by `savfox-network-proxy`, and the boundaries of
