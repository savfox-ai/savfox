...
use savfox_core::config::{CONFIG_TOML_FILE, Constrained, ConstraintError, find_savfox_home};
use savfox_core::config_loader::{
    CloudRequirementsLoader, ConfigLayerStack, ConfigLayerStackOrdering, LoaderOverrides,
    RequirementSource, load_config_layers_state,
};
use serde::Deserialize;

use crate::config::{NetworkMode, NetworkProxyConfig};
use crate::policy::{DomainPattern, compile_globset};
#[cfg(test)]
pub(crate) use crate::runtime::network_proxy_state_for_policy;
pub use crate::runtime::{BlockedRequest, BlockedRequestArgs, NetworkProxyState};
use crate::runtime::{ConfigState, LayerMtime};

pub(crate) async fn build_config_state() -> Result<ConfigState> {
    // Load config through `savfox-core` so we inherit the same layer ordering and semantics as the
    // rest of Savfox (system/managed layers, user layers, session flags, etc.).
    let savfox_home = find_savfox_home().context("failed to resolve SAVFOX_HOME")?;
    ...
    .await
    .context("failed to load Savfox config")?;

    let cfg_path = savfox_home.join(CONFIG_TOML_FILE);
    ...
    stack
        ...
        .filter_map(|layer| {
            let path = match &layer.name {
                ...
                ConfigLayerSource::Project { dot_savfox_folder } => dot_savfox_folder
...
fn enforce_trusted_constraints(
    layers: &savfox_core::config_loader::ConfigLayerStack,
...
fn network_proxy_constraints_from_trusted_layers(
    ...
    for layer in layers.get_layers(
        savfox_core::config_loader::ConfigLayerStackOrdering::LowestPrecedenceFirst,
        false,
    ) {
