...
use crate::policy::{Host, is_loopback_host, is_non_public_ip, normalize_host};
use crate::reasons::{REASON_DENIED, REASON_NOT_ALLOWED, REASON_NOT_ALLOWED_LOCAL};
use crate::state::{
    NetworkProxyConstraints, build_config_state, validate_policy_against_constraints,
};
...
impl NetworkProxyState {
    ...
    pub async fn current_cfg(&self) -> Result<NetworkProxyConfig> {
        ...
        // to `config.toml` (including Savfox-managed writes) take effect without a restart.
    ...
    pub async fn is_unix_socket_allowed(&self, path: &str) -> Result<bool> {
        ...
        for allowed in &guard.config.network_proxy.policy.allow_unix_sockets {
            ...
            // If canonicalization fails (e.g., socket not created yet), fall back to raw
            // comparison.
...
mod tests {
    ...
    use super::*;
    use crate::config::{NetworkPolicy, NetworkProxyConfig, NetworkProxySettings};
    use crate::policy::compile_globset;
    use crate::state::{NetworkProxyConstraints, validate_policy_against_constraints};
