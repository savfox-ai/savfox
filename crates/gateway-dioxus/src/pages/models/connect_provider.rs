...
fn render_step_api_key(
    ...
    mut account_name: Signal<String>,
    ...
    // Compute the auto-generated account id preview.
    let account_name_val = account_name();
    let name_for_id = if account_name_val.trim().is_empty() {
        provider_name.to_string()
    } else {
        account_name_val.clone()
    };
    let account_id_preview = slugify_account_id(provider_id, &name_for_id);
    ...
    if is_openai {
        return rsx! {
            ...
            // Account name input (for multi-account support).
            div { class: "wiz-field",
                label { class: "wiz-field__label", "Account Name" }
                input {
                    r#type: "text",
                    class: "wiz-field__input",
                ...
                p { class: "wiz-field__hint",
                    "ID: {account_id_preview}"
                }
            }
        };
    }
...
fn render_step_summary(
    ...
    // Use account_name for the display name sent to the import RPC.
    // If empty, fall back to the provider's display name.
    let import_name = if account_name.trim().is_empty() {
        provider_name.to_string()
    } else {
    ...
    let provider_name_owned = import_name.clone();
