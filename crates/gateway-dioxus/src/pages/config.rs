...
fn config_inner(active_section: Option<String>) -> Element {
    ...
    let mut toml_error = use_signal(|| Option::<String>::None);
        ...
        div { class: "config-layout",
            ...
            aside { class: "config-sidebar",
                div { class: "config-sidebar__header",
                    ...
                    "Settings"
                    HelpTip { text: "Gateway configuration in TOML format. Use Form mode for guided editing or Raw mode for direct JSON editing.".to_string() }
                }
                    {match &validity {
            ...
            main { class: "config-main",
                ...
                div { class: "config-actions",
                    ...
                    div { class: "config-actions__right",
                        ...
                        button {
                            class: if has_changes && !saving() { "btn btn--sm primary save-pulse" } else { "btn btn--sm primary" },
                ...
                div { class: "config-content",
                    if mode() == "form" {
                        ...
                        {
                            let has_toml_error = toml_error().is_some();
                            rsx! {
                                div { class: "config-raw",
                                    textarea {
                                        class: if has_toml_error { "config-raw__textarea config-raw__textarea--error" } else { "config-raw__textarea" },
                                        value: "{edit_json}",
                                        oninput: move |e| {
                                            let text = e.value();
                                            edit_json.set(text.clone());
                                            // Validate TOML syntax on every edit
                                            match toml::from_str::<toml::Value>(&text) {
                                                Ok(_) => toml_error.set(None),
                                                Err(err) => toml_error.set(Some(err.to_string())),
                                            }
                                        },
                                        rows: 25,
                                    }
                                    ...
                                    div { class: "config-raw__actions",
                                        button {
                                            class: if has_toml_error { "btn primary btn--disabled" } else { "btn primary" },
                                            disabled: has_toml_error,
                                            onclick: {
                                                let ws = ws.clone();
                                                move |_| {
                                                    let raw = edit_json();
                                                    // Parse as TOML, then convert to JSON Value for the API
                                                    match toml::from_str::<toml::Value>(&raw) {
                                                        Ok(toml_val) => {
                                                            ...
                                                            match serde_json::to_value(&toml_val) {
                                                                ...
                                                                Err(e) => {
                                                                    error_msg.set(Some(format!("Failed to convert TOML: {e}")));
                                                                }
                                                            }
                                                        }
                                                        Err(e) => {
                                                            toml_error.set(Some(e.to_string()));
                                                            error_msg.set(Some(format!("Invalid TOML: {e}")));
                                                        }
                                                    }
                                                }
                                            },
                ...
                div { class: "config-section-card__titles",
                    h3 { class: "config-section-card__title",
                        ...
                        if !section_help.is_empty() {
                            HelpTip { text: section_help.clone() }
                        }
                    }
...
fn export_config_json(json_str: &str) {
    crate::utils::download::trigger_download("savfox-config.json", json_str, "application/json");
}
...
const CONFIG_PAGE_STYLES: &str = r#"
    ...
    .save-pulse {
        ...
        box-shadow: 0 0 8px color-mix(in srgb, var(--accent) 50%, transparent 50%);
    }

    /* TOML validation error styles for raw editor */
    .config-raw__textarea--error {
