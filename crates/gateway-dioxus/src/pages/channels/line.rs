...
#[component]
pub fn LineChannel() -> Element {
    let ws = use_context::<Signal<WsRpc>>();
    let mut status = use_signal(|| None::<LineStatus>);
    ...
    let load_status = move || {
        ...
        async move {
            ...
            let result = ws
                ...
                .call::<LineStatus>(
                    "channels.status",
                    Some(serde_json::json!({"channel": "line"})),
                )
                .await;
    ...
    rsx! {
        div { class: "channel-page",
            div { class: "channel-header",
                span { class: "channel-icon", dangerous_inner_html: crate::utils::icons::ICON_LINE }
                h2 { "Line" }
            }
                            ...
                            button {
                                ...
                                onclick: move |_| {
                                    ...
                                    async move {
                                        ...
                                        let _ = ws.read()
                                            .call::<serde_json::Value>("channels.logout", Some(serde_json::json!({"channel": "line"})))
                                            .await;
                            ...
                            button {
                                ...
                                onclick: move |_| {
                                    ...
                                    async move {
                                        action_loading.set(true);
                                        let _ = ws.read()
                                            .call::<serde_json::Value>("channels.login", Some(serde_json::json!({"channel": "line"})))
                                            .await;
                                        action_loading.set(false);
                                        load_status().await;
                                    }
