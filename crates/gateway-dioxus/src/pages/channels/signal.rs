...
#[component]
pub fn SignalChannel() -> Element {
    let ws = use_context::<Signal<WsRpc>>();
    let mut status = use_signal(|| None::<SignalStatus>);
    ...
    let load_status = move || {
        ...
        async move {
            ...
            let result = ws
                ...
                .call::<SignalStatus>(
                    "channels.status",
                    Some(serde_json::json!({"channel": "signal"})),
                )
                .await;
            ...
            div { class: "channels-toolbar",
                div { class: "channels-toolbar__left",
                    h2 { class: "channels-toolbar__title",
                        span { class: "channel-icon", dangerous_inner_html: crate::utils::icons::ICON_SIGNAL }
                        "Signal"
                    }
                    p { class: "channels-toolbar__subtitle", "signal-cli status and channel configuration." }
                }
            }
                                ...
                                button {
                                    ...
                                    onclick: move |_| {
                                        ...
                                        async move {
                                            ...
                                            let _ = ws.read()
                                                .call::<serde_json::Value>("channels.logout", Some(serde_json::json!({"channel": "signal"})))
                                                .await;
                                ...
                                button {
                                    ...
                                    onclick: move |_| {
                                        ...
                                        async move {
                                            action_loading.set(true);
                                            let _ = ws.read()
                                                .call::<serde_json::Value>("channels.login", Some(serde_json::json!({"channel": "signal"})))
                                                .await;
                                            action_loading.set(false);
                                            load_status().await;
                                        }
