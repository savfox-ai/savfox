...
where
    P: AsRef<Path>,
{
    let mcp_executable = savfox_utils::cargo_bin::cargo_bin("savfox-exec-mcp-server")?;
    let execve_wrapper = savfox_utils::cargo_bin::cargo_bin("savfox-execve-wrapper")?;
    ...
    let transport = TokioChildProcess::new(Command::new(&mcp_executable).configure(|cmd| {
        ...
        cmd.env("SAVFOX_HOME", savfox_home.as_ref());
...
pub async fn write_default_execpolicy<P>(policy: &str, savfox_home: P) -> anyhow::Result<()>
where
    P: AsRef<Path>,
{
    let policy_dir = savfox_home.as_ref().join("rules");
