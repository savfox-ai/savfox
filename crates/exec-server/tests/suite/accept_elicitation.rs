...
"#,
    ...
    let transport = create_transport(savfox_home.as_ref(), dotslash_cache).await?;
    ...
    let savfox_linux_sandbox_exe = if cfg!(target_os = "linux") {
        let savfox_linux_sandbox_exe = linux_sandbox_exe_folder.path().join("savfox-linux-sandbox");
        let savfox_cli = ensure_savfox_cli()?;
        symlink(&savfox_cli, &savfox_linux_sandbox_exe)?;
        Some(savfox_linux_sandbox_exe)
    } else {
        None
    };
    let response =
        notify_readable_sandbox(&project_root_path, savfox_linux_sandbox_exe, &service).await?;
...
fn ensure_savfox_cli() -> Result<PathBuf> {
    let savfox_cli = savfox_utils_cargo_bin::cargo_bin("savfox")?;

    let metadata = savfox_cli.metadata().with_context(|| {
        format!(
            "failed to read metadata for savfox binary at {}",
            savfox_cli.display()
        )
    })?;
    ensure!(
        metadata.is_file(),
        "expected savfox binary at {} to be a file; run `cargo build -p savfox-cli --bin savfox` before this test",
        savfox_cli.display()
    );
    ...
    let mode = metadata.permissions().mode();
        ...
        mode & 0o111 != 0,
        "savfox binary at {} is not executable (mode {mode:o}); run `cargo build -p savfox-cli --bin savfox` before this test",
        savfox_cli.display()
    );
