...
//! from us, and we give its fd number in the SAVFOX_ESCALATE_SOCKET environment variable.
//!
...
const SAVFOX_EXECVE_WRAPPER_EXE_NAME: &str = "savfox-execve-wrapper";
...
pub async fn main_mcp_server() -> anyhow::Result<()> {
    ...
    let execve_wrapper = match cli.execve_wrapper {
        ...
        None => {
            ...
            cwd.parent()
                .map(|p| p.join(SAVFOX_EXECVE_WRAPPER_EXE_NAME))
                .ok_or_else(|| {
...
async fn load_exec_policy() -> anyhow::Result<Policy> {
    let savfox_home = find_savfox_home().context("failed to resolve savfox_home for execpolicy")?;

    // TODO(mbolin): At a minimum, `cwd` should be configurable via
    // `savfox/sandbox-state/update` or some other custom MCP call.
    let cwd = None;
    let cli_overrides = Vec::new();
    let overrides = savfox_core::config_loader::LoaderOverrides::default();
    let config_layer_stack = savfox_core::config_loader::load_config_layers_state(
        &savfox_home,
        cwd,
        &cli_overrides,
        overrides,
        savfox_core::config_loader::CloudRequirementsLoader::default(),
    )
    .await?;

    savfox_core::load_exec_policy(&config_layer_stack)
        .await
