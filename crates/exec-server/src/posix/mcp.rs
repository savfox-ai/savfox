...
const SAVFOX_BASH_PATH_ENV_VAR: &str = "SAVFOX_BASH_PATH";

const SANDBOX_STATE_CAPABILITY_VERSION: &str = "1.0.0";

pub(crate) fn get_bash_path() -> Result<PathBuf> {
    std::env::var(SAVFOX_BASH_PATH_ENV_VAR)
        .map(PathBuf::from)
        .context(format!("{SAVFOX_BASH_PATH_ENV_VAR} must be set"))
}
...
impl ExecTool {
    ...
    /// Runs a shell command and returns its output. You MUST provide the workdir as an absolute
    /// path.
    #[tool]
    async fn shell(
        &self,
        ...
        let effective_timeout = Duration::from_millis(
            params
                .timeout_ms
                .unwrap_or(savfox_core::exec::DEFAULT_EXEC_COMMAND_TIMEOUT_MS),
        );
...
#[derive(Default)]
pub struct SavfoxSandboxStateUpdateMethod;

impl rmcp::model::ConstString for SavfoxSandboxStateUpdateMethod {
...
impl ServerHandler for ExecTool {
    ...
    async fn on_custom_request(
        ...
        if method != MCP_SANDBOX_STATE_METHOD {
            return Err(McpError::method_not_found::<SavfoxSandboxStateUpdateMethod>());
        }
