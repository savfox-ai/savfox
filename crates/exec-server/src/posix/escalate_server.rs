...
use crate::posix::escalate_protocol::{
    BASH_EXEC_WRAPPER_ENV_VAR, ESCALATE_SOCKET_ENV_VAR, EscalateAction, EscalateRequest,
    EscalateResponse, SuperExecMessage, SuperExecResult,
};
use crate::posix::escalation_policy::EscalationPolicy;
use crate::posix::mcp::ExecParams;
use crate::posix::socket::{AsyncDatagramSocket, AsyncSocket};
...
impl EscalateServer {
    ...
    pub async fn exec(
        ...
        let result = process_exec_tool_call(
            ...
            &sandbox_state.savfox_linux_sandbox_exe,
            None,
        )
        .await?;
        escalate_task.abort();
...
mod tests {
    ...
    async fn handle_escalate_session_respects_run_in_sandbox_decision() -> anyhow::Result<()> {
        ...
        for i in 0..10 {
            ...
            env.insert(format!("SAVFOX_TEST_VAR{i}"), value);
        }
