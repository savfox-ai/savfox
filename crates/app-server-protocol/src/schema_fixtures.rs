...
/// Regenerates `schema/typescript/` and `schema/json/` under the provided output root.
///
/// This is intended to be used by tooling or ad-hoc export commands.
/// It deletes any previously generated files in the provided destination so stale
...
fn canonicalize_json(value: &Value) -> Value {
    match value {
        Value::Array(items) => {
            ...
            // only by `schema_fixtures_are_stable_across_generations` to compare one generated
            // schema tree against another freshly generated output. Some parts of schema generation
            // end up with non-deterministic ordering across platforms (often due to map
            // iteration order upstream), which can cause Windows CI failures even when
            // the generated schema is semantically equivalent.
            //
