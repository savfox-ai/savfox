...
#[derive(Parser, Debug)]
#[command(about = "Generate app-server schema fixtures into a target directory")]
struct Args {
...
fn main() -> Result<()> {
    ...
    savfox_app_server_protocol::write_schema_fixtures_with_options(
        &schema_root,
        args.prettier.as_deref(),
        savfox_app_server_protocol::SchemaFixtureOptions {
