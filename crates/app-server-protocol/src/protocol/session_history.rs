...
use crate::protocol::v1::{SessionItem, Turn, TurnError, TurnStatus, UserInput};
...
impl SessionHistoryBuilder {
    ...
    /// See `should_persist_event_msg` in `savfox-rs/core/rollout/policy.rs`.
    fn handle_event(&mut self, event: &EventMsg) {
        match event {
            ...
            EventMsg::SessionRolledBack(payload) => self.handle_session_rollback(payload),
    ...
    fn handle_user_message(&mut self, payload: &UserMessageEvent) {
        ...
        turn.items.push(SessionItem::UserMessage { id, content });
    ...
    fn handle_agent_message(&mut self, text: String) {
        ...
        self.ensure_turn()
            .items
            .push(SessionItem::AgentMessage { id, text });
    }
    ...
    fn handle_agent_reasoning(&mut self, payload: &AgentReasoningEvent) {
        ...
        if let Some(SessionItem::Reasoning { summary, .. }) = self.ensure_turn().items.last_mut() {
        ...
        let id = self.next_item_id();
        self.ensure_turn().items.push(SessionItem::Reasoning {
            id,
    ...
    fn handle_agent_reasoning_raw_content(&mut self, payload: &AgentReasoningRawContentEvent) {
        ...
        if let Some(SessionItem::Reasoning { content, .. }) = self.ensure_turn().items.last_mut() {
    ...
    fn handle_item_completed(&mut self, payload: &ItemCompletedEvent) {
        if let savfox_protocol::items::TurnItem::Plan(plan) = &payload.item {
            if plan.text.is_empty() {
                return;
            }
            let id = self.next_item_id();
            self.ensure_turn().items.push(SessionItem::Plan {
                id,
    ...
    fn handle_session_rollback(&mut self, payload: &SessionRolledBackEvent) {
        self.finish_current_turn();
...
mod tests {
    ...
    #[test]
    fn drops_last_turns_on_session_rollback() {
        let events = vec![
            EventMsg::UserMessage(UserMessageEvent {
                message: "First".into(),
                images: None,
            ...
            EventMsg::SessionRolledBack(SessionRolledBackEvent { num_turns: 1 }),
            EventMsg::UserMessage(UserMessageEvent {
                message: "Third".into(),
                images: None,
                text_elements: Vec::new(),
    ...
    #[test]
    fn session_rollback_clears_all_turns_when_num_turns_exceeds_history() {
        let events = vec![
            EventMsg::UserMessage(UserMessageEvent {
                message: "One".into(),
                images: None,
            ...
            EventMsg::SessionRolledBack(SessionRolledBackEvent { num_turns: 99 }),
        ];
