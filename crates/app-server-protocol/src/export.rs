...
pub fn generate_ts_with_options(
    ...
    // Ensure our header is present on all TS files (root + subdirs like v1/).
...
pub fn generate_json_with_experimental(out_dir: &Path, experimental_api: bool) -> Result<()> {
    ...
    write_pretty_json(
        out_dir.join("savfox_app_server_protocol.schemas.json"),
        &bundle,
    )?;
...
fn build_schema_bundle(schemas: Vec<GeneratedSchema>) -> Result<Value> {
    ...
    root.insert(
        "title".to_owned(),
        Value::String("SavfoxAppServerProtocol".into()),
    );
    ...
    T: JsonSchema,
{
    ...
    // the TypeScript layout and write to out_dir/v1/Type.json. Otherwise
    // write alongside the root files.
    ...
    let namespace = raw_namespace.map(str::to_string);
    Ok(GeneratedSchema {
        namespace,
...
mod tests {
    ...
    fn generated_ts_has_no_optional_nullable_fields() -> Result<()> {
        ...
        let output_dir = std::env::temp_dir().join(format!("savfox_ts_types_{}", Uuid::now_v7()));
    ...
    fn generate_ts_with_experimental_api_retains_experimental_entries() -> Result<()> {
        let output_dir =
            std::env::temp_dir().join(format!("savfox_ts_types_experimental_{}", Uuid::now_v7()));
        fs::create_dir(&output_dir)?;
    ...
    #[test]
    fn stable_schema_filter_removes_mock_session_start_field() -> Result<()> {
        let output_dir = std::env::temp_dir().join(format!("savfox_schema_{}", Uuid::now_v7()));
        fs::create_dir(&output_dir)?;
        let schema = write_json_schema_with_return::<v1::SessionStartParams>(
            &output_dir,
            "SessionStartParams",
        )?;
        ...
        let (_, def_schema) = definitions
            .iter()
            .find(|(name, _)| definition_matches_type(name, "SessionStartParams"))
            .expect("SessionStartParams definition should exist");
        let properties = def_schema["properties"]
            .as_object()
            .expect("SessionStartParams should have properties");
    ...
    fn experimental_type_fields_ts_filter_handles_interface_shape() -> Result<()> {
        let output_dir = std::env::temp_dir().join(format!("savfox_ts_filter_{}", Uuid::now_v7()));
...
"#;
    ...
    fn generate_json_filters_experimental_fields_and_methods() -> Result<()> {
        ...
        let bundle_json =
            fs::read_to_string(output_dir.join("savfox_app_server_protocol.schemas.json"))?;
