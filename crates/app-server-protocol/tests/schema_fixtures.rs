...
fn schema_root() -> Result<std::path::PathBuf> {
    ...
    let typescript_index = savfox_utils_cargo_bin::find_resource!("schema/typescript/index.ts")
    ...
    let json_bundle = savfox_utils_cargo_bin::find_resource!(
        "schema/json/savfox_app_server_protocol.schemas.json"
    )
    .context("resolve JSON schema bundle")?;
    let json_root = json_bundle
        .parent()
        .and_then(|p| p.parent())
        .context("derive schema root from schema/json/savfox_app_server_protocol.schemas.json")?;
    anyhow::ensure!(
