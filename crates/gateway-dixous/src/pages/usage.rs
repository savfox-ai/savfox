...
fn export_csv(entries: &[UsageCostEntry]) {
    let mut csv = String::from(
        "session_id,model,input_tokens,output_tokens,total_tokens,cost,estimated_cost\n",
    );
    for e in entries {
        let model = e.model.as_deref().unwrap_or("");
        ...
        let cost = e.cost.map(|c| format!("{c:.6}")).unwrap_or_default();
        let est = format!("{:.6}", session_entry_cost(e));
        csv.push_str(&format!(
            "{},{},{},{},{},{},{}\n",
            e.session_id, model, inp, out, total, cost, est,
        ));
    }
