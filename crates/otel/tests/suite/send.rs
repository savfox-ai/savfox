...
use crate::harness::{
    attributes_to_map, build_metrics_with_defaults, find_metric, histogram_data, latest_metrics,
};
...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "savfox-cli"), ("env", "prod")])?;

    metrics.counter("savfox.turns", 1, &[("model", "gpt-5.1"), ("env", "dev")])?;
    metrics.histogram("savfox.tool_latency", 25, &[("tool", "shell")])?;
    metrics.shutdown()?;

    let resource_metrics = latest_metrics(&exporter);

    let counter = find_metric(&resource_metrics, "savfox.turns").expect("counter metric missing");
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "savfox.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        match find_metric(&resource_metrics, "savfox.tool_latency").and_then(|metric| match metric
            .data()
        {
...
fn send_merges_default_tags_per_line() -> Result<()> {
    ...
    metrics.counter("savfox.alpha", 1, &[("env", "dev"), ("component", "alpha")])?;
    metrics.counter(
        "savfox.beta",
        2,
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "savfox.alpha").expect("savfox.alpha metric missing");
    ...
    let beta_metric =
        find_metric(&resource_metrics, "savfox.beta").expect("savfox.beta metric missing");
...
fn shutdown_flushes_in_memory_exporter() -> Result<()> {
    ...
    metrics.counter("savfox.turns", 1, &[])?;
    metrics.shutdown()?;
