...
use crate::harness::{attributes_to_map, build_metrics_with_defaults, find_metric, latest_metrics};

// Ensures OtelManager attaches metadata tags when forwarding metrics.
#[test]
fn manager_attaches_metadata_tags_to_metrics() -> Result<()> {
    let (metrics, exporter) = build_metrics_with_defaults(&[("service", "savfox-cli")])?;
    ...
    manager.counter("savfox.session_started", 1, &[("source", "tui")]);
    manager.shutdown_metrics()?;

    let resource_metrics = latest_metrics(&exporter);
    let metric =
        find_metric(&resource_metrics, "savfox.session_started").expect("counter metric missing");
