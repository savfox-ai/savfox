...
use savfox_api_client::{ApiError, ResponseEvent};
...
use crate::metrics::names::{
    API_CALL_COUNT_METRIC, API_CALL_DURATION_METRIC, SSE_EVENT_COUNT_METRIC,
    SSE_EVENT_DURATION_METRIC, TOOL_CALL_COUNT_METRIC, TOOL_CALL_DURATION_METRIC,
    WEBSOCKET_EVENT_COUNT_METRIC, WEBSOCKET_EVENT_DURATION_METRIC, WEBSOCKET_REQUEST_COUNT_METRIC,
    WEBSOCKET_REQUEST_DURATION_METRIC,
};
use crate::otel_provider::traceparent_context_from_env;
pub use crate::{OtelEventMetadata, OtelManager, ToolDecisionSource};
...
impl OtelManager {
    ...
    pub fn conversation_starts(
        ...
        tracing::event!(
            tracing::Level::INFO,
            event.name = "savfox.conversation_starts",
    ...
    pub fn record_websocket_request(&self, duration: Duration, error: Option<&str>) {
        ...
        tracing::event!(
            tracing::Level::INFO,
            event.name = "savfox.websocket_request",
            event.timestamp = %timestamp(),
            conversation.id = %self.metadata.conversation_id,
            app.version = %self.metadata.app_version,
            auth_mode = self.metadata.auth_mode,
    ...
    pub fn record_websocket_event(
        ...
        tracing::event!(
            tracing::Level::INFO,
            event.name = "savfox.websocket_event",
            event.timestamp = %timestamp(),
            event.kind = %kind_str,
            conversation.id = %self.metadata.conversation_id,
            app.version = %self.metadata.app_version,
    ...
    pub fn tool_decision(
        ...
        tracing::event!(
            tracing::Level::INFO,
            event.name = "savfox.tool_decision",
            event.timestamp = %timestamp(),
            conversation.id = %self.metadata.conversation_id,
            app.version = %self.metadata.app_version,
            auth_mode = self.metadata.auth_mode,
