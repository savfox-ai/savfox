...
//!
//! For Linux, we use linux-native-async-persistent which uses both keyutils and
//! async-secret-service (see below) for storage. See the docs for the keyutils_persistent module
//! for a full explanation of why both are used. Because this store uses the async-secret-service,
//! you must specify the additional features required by that store
//!
//! async-secret-service provides access to the DBus-based Secret Service storage on Linux, FreeBSD,
//! and OpenBSD. This is an asynchronous keystore that always encrypts secrets when they are
//! transferred across the bus. If DBus isn't installed the keystore will fall back to the json file
//! because we don't use the "vendored" feature.
//!
//! If the keyring is not available or fails, we fall back to SAVFOX_HOME/.credentials.json which is
//! consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "Savfox MCP Credentials";
...
/// Determine where Savfox should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum OAuthCredentialsStoreMode {
    /// `Keyring` when available; otherwise, `File`.
    /// Credentials stored in the keyring will only be readable by Savfox unless the user explicitly
    /// grants access via OS-level keyring access.
    #[default]
    Auto,
    /// SAVFOX_HOME/.credentials.json
