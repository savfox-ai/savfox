...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of Savfox TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Savfox TUI):
    /// - Build this binary: `cargo build -p savfox-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `savfox mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Savfox TUI, ask it to call:
...
/// Scenarios for `image_scenario`, intended to exercise Savfox TUI handling of MCP image outputs.
///
