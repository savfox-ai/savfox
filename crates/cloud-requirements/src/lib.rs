...
use savfox_core::config_loader::{CloudRequirementsLoader, ConfigRequirementsToml};
use savfox_protocol::account::PlanType;
use tokio::time::timeout;

/// This blocks savfox startup, so must be short.
...
trait RequirementsFetcher: Send + Sync {
    ...
    /// Result.
    async fn fetch_requirements(&self, auth: &SavfoxAuth) -> Option<String>;
}
...
impl CloudRequirementsService {
    ...
    async fn fetch_with_timeout(&self) -> Option<ConfigRequirementsToml> {
        let _timer =
            savfox_otel::start_global_timer("savfox.cloud_requirements.fetch.duration_ms", &[]);
...
mod tests {
    ...
    impl RequirementsFetcher for StaticFetcher {
        async fn fetch_requirements(&self, _auth: &SavfoxAuth) -> Option<String> {
            self.contents.clone()
        }
    }
