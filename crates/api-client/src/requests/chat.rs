        ...
        for (idx, item) in input.iter().enumerate() {
            match item {
                ResponseItem::Message { role, content, .. } => {
                    // Map OpenAI-specific "developer" role to the standard
                    // "system" role so third-party Chat Completions providers
                    // (e.g. ZhiPu GLM, DeepSeek) don't reject it.
                    ...
                    // Merge consecutive same-role messages (system or user)
                    // into a single message.  Many third-party providers
                    // (ZhiPu, Qwen, etc.) enforce strict role alternation and
