...
// Additional mcp-specific data to be added to a [`savfox_core::protocol::Event`] as
// notification.params._meta MCP Spec: https://modelcontextprotocol.io/specification/2025-06-18/basic#meta
...
pub(crate) struct OutgoingNotificationMeta {
    ...
    /// Because multiple sessions may be multiplexed over a single MCP connection,
    /// include the `sessionId` in the notification meta.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
mod tests {
    ...
    #[tokio::test]
    async fn test_send_event_as_notification_with_meta_and_session_id() -> Result<()> {
        let (outgoing_tx, mut outgoing_rx) = mpsc::unbounded_channel::<OutgoingMessage>();
        let outgoing_message_sender = OutgoingMessageSender::new(outgoing_tx);
