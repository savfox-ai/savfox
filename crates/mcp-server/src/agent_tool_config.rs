...
impl SavfoxToolCallReplyParam {
    pub(crate) fn get_session_id(&self) -> anyhow::Result<SessionId> {
        if let Some(session_id) = &self.session_id {
            ...
            Err(anyhow::anyhow!(
                "either sessionId or conversationId must be provided"
            ))
        }
    }
}
        ...
        let expected_tool_json = serde_json::json!({
          ...
          "inputSchema": {
            "properties": {
              ...
              "sessionId": {
                "description": "The session id for this Savfox session. This field is required, but we keep it optional here for backward compatibility for clients that still use conversationId.",
                "type": "string"
              }
            },
            "required": [
