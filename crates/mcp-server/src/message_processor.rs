...
impl MessageProcessor {
    ...
    async fn handle_initialize(
        ...
        let server_info = Implementation {
            name: "savfox-mcp-server".to_string(),
    ...
    async fn handle_tool_call_savfox(
                    ...
                    Err(e) => {
                        let result = CallToolResult {
                            content: vec![rmcp::model::Content::text(format!(
                                "Failed to load Savfox configuration from overrides: {e}"
                            ))],
                            structured_content: None,
                            is_error: Some(true),
                            meta: None,
        ...
        task::spawn(async move {
            ...
            crate::savfox_tool_runner::run_savfox_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
    ...
    async fn handle_tool_call_savfox_session_reply(
        ...
        let savfox = match self.session_manager.get_session(session_id).await {
            ...
            Err(_) => {
                ...
                let result = crate::savfox_tool_runner::create_call_tool_result_with_session_id(
                    session_id,
        ...
        tokio::spawn({
            ...
            async move {
                crate::savfox_tool_runner::run_savfox_tool_session_reply(
                    session_id,
                    savfox,
                    outgoing,
                    request_id,
