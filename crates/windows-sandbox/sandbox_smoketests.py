...
# Run a suite of smoke tests against the Windows sandbox via the Savfox CLI
...
def _resolve_savfox_cmd() -> List[str]:
    """Resolve the Savfox CLI to invoke `savfox sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run Savfox.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "savfox.exe",
        ws_root / "target" / "release" / "savfox.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "savfox.exe",
            cargo_base / "release" / "savfox.exe",
        ])
    ...
    if shutil.which("savfox"):
        return ["savfox"]

    raise FileNotFoundError(
        "Savfox CLI not found. Build it first, e.g.\n"
        "  cargo build -p savfox-cli --release\n"
        "or for debug:\n"
        "  cargo build -p savfox-cli\n"
    )

SAVFOX_CMD = _resolve_savfox_cmd()
print(SAVFOX_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to savfox CLI flags
    ...
    argv = [*SAVFOX_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\savfox_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.savfox artifacts) denied
    savfox_home = Path(os.environ["USERPROFILE"]) / ".savfox"
    cap_sid_target = savfox_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .savfox\\policy.json"], WS_ROOT)
    add("WS: .savfox cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .savfox policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > savfox_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://savfox-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(
