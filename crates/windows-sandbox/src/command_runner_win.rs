...
use savfox_windows_sandbox::{
    SandboxPolicy, allow_null_device, convert_string_sid_to_sid, create_process_as_user,
    create_readonly_token_with_cap_from, create_workspace_write_token_with_cap_from,
    get_current_token_for_restriction, hide_current_user_profile_dir, log_note, parse_policy,
    to_wide,
};
...
struct RunnerRequest {
    ...
    // Writable location for logs (sandbox user's .savfox).
    savfox_home: PathBuf,
...
pub fn main() -> Result<()> {
    ...
    let log_dir = Some(req.savfox_home.as_path());
    hide_current_user_profile_dir(req.savfox_home.as_path());
    log_note(
        &format!(
            "runner start cwd={} cmd={:?} real_savfox_home={}",
            req.cwd.display(),
            req.command,
            req.real_savfox_home.display()
        ),
        Some(&req.savfox_home),
    );
