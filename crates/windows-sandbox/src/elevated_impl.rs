mod windows_impl {
    ...
    use crate::env::{ensure_non_interactive_pager, inherit_path_env, normalize_null_device_env};
    use crate::identity::require_logon_sandbox_creds;
    use crate::logging::{log_failure, log_note, log_start, log_success};
    use crate::policy::{SandboxPolicy, parse_policy};
    ...
    use windows_sys::Win32::System::Pipes::{PIPE_READMODE_BYTE, PIPE_TYPE_BYTE, PIPE_WAIT};
    ...
    /// Creates the sandbox user's Savfox home directory if it does not already exist.
    fn ensure_savfox_home_exists(p: &Path) -> Result<()> {
    ...
    /// Locates `savfox-command-runner.exe` next to the current binary.
    fn find_runner_exe() -> PathBuf {
        if let Ok(exe) = std::env::current_exe() {
            if let Some(dir) = exe.parent() {
                let candidate = dir.join("savfox-command-runner.exe");
                if candidate.exists() {
                    return candidate;
                }
            }
        }
        PathBuf::from("savfox-command-runner.exe")
    }

    /// Generates a unique named-pipe path used to communicate with the runner process.
    fn pipe_name(suffix: &str) -> String {
        let mut rng = SmallRng::from_os_rng();
        format!(r"\\.\pipe\savfox-runner-{:x}-{}", rng.gen::<u128>(), suffix)
    }
    ...
    struct RunnerPayload {
        ...
        // Writable log dir for sandbox user (.savfox in sandbox profile).
        savfox_home: PathBuf,
        // Real user's SAVFOX_HOME for shared data (caps, config).
        real_savfox_home: PathBuf,
        cap_sid: String,
    ...
    pub fn run_windows_sandbox_capture(
        ...
        let sandbox_base = savfox_home.join(".sandbox");
        ensure_savfox_home_exists(&sandbox_base)?;

        let logs_base_dir: Option<&Path> = Some(sandbox_base.as_path());
        log_start(&command, logs_base_dir);
        let sandbox_creds =
            require_logon_sandbox_creds(&policy, sandbox_policy_cwd, cwd, &env_map, savfox_home)?;
        ...
        let caps = load_or_create_cap_sids(savfox_home)?;
        ...
        let runner_cmdline = runner_exe
            .to_str()
            .map(|s| s.to_string())
            .unwrap_or_else(|| "savfox-command-runner.exe".to_string());
        ...
        let payload = RunnerPayload {
            ...
            savfox_home: sandbox_base.clone(),
            real_savfox_home: savfox_home.to_path_buf(),
