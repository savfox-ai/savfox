...
use crate::allow::{AllowDenyPaths, compute_allow_paths};
use crate::logging::log_note;
use crate::policy::SandboxPolicy;
use crate::setup_error::{
    SetupErrorCode, SetupFailure, clear_setup_error_report, failure, read_setup_error_report,
};

pub const SETUP_VERSION: u32 = 5;
pub const OFFLINE_USERNAME: &str = "SavfoxSandboxOffline";
pub const ONLINE_USERNAME: &str = "SavfoxSandboxOnline";
const ERROR_CANCELLED: u32 = 1223;
const SECURITY_BUILTIN_DOMAIN_RID: u32 = 0x0000_0020;
const DOMAIN_ALIAS_RID_ADMINS: u32 = 0x0000_0220;

pub fn sandbox_dir(savfox_home: &Path) -> PathBuf {
    savfox_home.join(".sandbox")
}

pub fn sandbox_secrets_dir(savfox_home: &Path) -> PathBuf {
    savfox_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(savfox_home: &Path) -> PathBuf {
    sandbox_dir(savfox_home).join("setup_marker.json")
}

pub fn sandbox_users_path(savfox_home: &Path) -> PathBuf {
    sandbox_secrets_dir(savfox_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh(
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| savfox_home.to_path_buf());
    log_note(
        &format!(
...
fn find_setup_exe() -> PathBuf {
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            let candidate = dir.join("savfox-windows-sandbox-setup.exe");
            if candidate.exists() {
                return candidate;
            }
        }
    }
    PathBuf::from("savfox-windows-sandbox-setup.exe")
}
...
fn report_helper_failure(
    ...
    match read_setup_error_report(savfox_home) {
...
fn run_setup_exe(
    ...
    let cleared_report = match clear_setup_error_report(savfox_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
    ...
    if !needs_elevation {
        ...
        if let Err(err) = clear_setup_error_report(savfox_home) {
            log_note(
                &format!(
...
pub fn run_elevated_setup(
    ...
    let sbx_dir = sandbox_dir(savfox_home);
    ...
    run_setup_exe(&payload, needs_elevation, savfox_home)
}
...
fn build_payload_roots(
    ...
    let write_roots = filter_sensitive_write_roots(write_roots, savfox_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, savfox_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to SAVFOX_HOME or anything under SAVFOX_HOME/.sandbox.
    // These locations contain sandbox control/state and must remain tamper-resistant.
    let savfox_home_key = crate::audit::normalize_path_key(savfox_home);
    let sbx_dir_key = crate::audit::normalize_path_key(&sandbox_dir(savfox_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let secrets_dir_key = crate::audit::normalize_path_key(&sandbox_secrets_dir(savfox_home));
