...
pub fn sandbox_setup_is_complete(savfox_home: &Path) -> bool {
    let marker_ok = matches!(load_marker(savfox_home), Ok(Some(marker)) if marker.version_matches());
    if !marker_ok {
        return false;
    }
    matches!(load_users(savfox_home), Ok(Some(users)) if users.version_matches())
}

fn load_marker(savfox_home: &Path) -> Result<Option<SetupMarker>> {
    let path = setup_marker_path(savfox_home);
...
fn load_users(savfox_home: &Path) -> Result<Option<SandboxUsersFile>> {
    let path = sandbox_users_path(savfox_home);
...
fn select_identity(policy: &SandboxPolicy, savfox_home: &Path) -> Result<Option<SandboxIdentity>> {
    let _marker = match load_marker(savfox_home)? {
        Some(m) if m.version_matches() => m,
        _ => return Ok(None),
    };
    let users = match load_users(savfox_home)? {
...
pub fn require_logon_sandbox_creds(
    ...
    let sandbox_dir = crate::setup::sandbox_dir(savfox_home);
    ...
    // NOTE: Do not add SAVFOX_HOME/.sandbox to `needed_write`; it must remain non-writable by the
    ...
    let mut identity = match load_marker(savfox_home)? {
        Some(marker) if marker.version_matches() => {
            _existing_marker = Some(marker.clone());
            let selected = select_identity(policy, savfox_home)?;
            if selected.is_none() {
                setup_reason =
    ...
    if identity.is_none() {
        ...
        identity = select_identity(policy, savfox_home)?;
    }
    // Always refresh ACLs (non-elevated) for current roots via the setup binary.
    crate::setup::run_setup_refresh(policy, policy_cwd, command_cwd, env_map, savfox_home)?;
