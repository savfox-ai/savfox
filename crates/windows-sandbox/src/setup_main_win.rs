...
use savfox_windows_sandbox::{
    LOG_FILE_NAME, SETUP_VERSION, SetupErrorCode, SetupErrorReport, SetupFailure,
    convert_string_sid_to_sid, ensure_allow_mask_aces_with_inheritance, ensure_allow_write_aces,
    extract_setup_failure, hide_newly_created_users, load_or_create_cap_sids, log_note,
    path_mask_allows, sandbox_dir, sandbox_secrets_dir, string_from_sid_bytes, to_wide,
...
mod read_acl_mutex;
mod sandbox_users;
use read_acl_mutex::{acquire_read_acl_mutex, read_acl_mutex_exists};
use sandbox_users::{
    provision_sandbox_users, resolve_sandbox_users_group_sid, resolve_sid, sid_bytes_to_psid,
};
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(savfox_home) = std::env::var("SAVFOX_HOME") {
            let sbx_dir = sandbox_dir(Path::new(&savfox_home));
...
fn real_main() -> Result<()> {
    ...
    let sbx_dir = sandbox_dir(&payload.savfox_home);
    ...
    if let Err(err) = &result {
        ...
        if let Err(write_err) = write_setup_error_report(&payload.savfox_home, &report) {
            let _ = log_line(
                &mut log,
...
fn run_setup_full(payload: &Payload, log: &mut File, sbx_dir: &Path) -> Result<()> {
    ...
    let caps = load_or_create_cap_sids(&payload.savfox_home).map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
    ...
    if !refresh_only {
        lock_sandbox_dir(
            &sandbox_dir(&payload.savfox_home),
            &payload.real_user,
            &sandbox_group_sid,
            GRANT_ACCESS,
            log,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_dir(&payload.savfox_home).display()
                ),
            ))
        })?;
        lock_sandbox_dir(
            &sandbox_secrets_dir(&payload.savfox_home),
            &payload.real_user,
            &sandbox_group_sid,
            DENY_ACCESS,
            log,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_secrets_dir(&payload.savfox_home).display()
                ),
            ))
        })?;
        let legacy_users = sandbox_dir(&payload.savfox_home).join("sandbox_users.json");
