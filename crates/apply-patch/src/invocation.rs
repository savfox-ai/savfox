...
use crate::parser::{Hunk, ParseError, parse_patch};
use crate::{
    ApplyPatchAction, ApplyPatchArgs, ApplyPatchError, ApplyPatchFileChange, ApplyPatchFileUpdate,
    IoError, MaybeApplyPatchVerified, unified_diff_from_chunks,
};
...
/// - Parsed with Tree‑sitter Bash and a strict query that uses anchors so the heredoc‑redirected
///   statement is the only top‑level statement.
/// - The connector between `cd` and `apply_patch` must be `&&` (not `|` or `||`).
/// - Exactly one positional `word` argument is allowed for `cd` (no flags, no quoted strings, no
///   second argument).
/// - The apply command is validated in‑query via `#any-of?` to allow `apply_patch` or `applypatch`.
...
fn extract_apply_patch_from_bash(
    ...
    // - dots (`.`) between named nodes enforces adjacency among named children and anchor to the
    //   start/end of the expression.
    // - we match a single redirected_statement directly under program with leading and trailing
    //   anchors (`.`). This ensures it is the only top-level statement (so prefixes like `echo
    //   ...;` or suffixes like `... && echo done` do not match).
    //
