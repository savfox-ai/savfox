...
const TEST_ORIGINATOR: &str = "savfox_vscode";
...
async fn turn_start_emits_user_message_item_with_text_elements() -> Result<()> {
    ...
    let user_message_item = timeout(DEFAULT_READ_TIMEOUT, async {
        loop {
            ...
            if let SessionItem::UserMessage { .. } = item_started.item {
                return Ok::<SessionItem, anyhow::Error>(item_started.item);
            }
        }
    })
    .await??;
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three Savfox turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
    ...
    // Start a turn with only input and session_id set (no overrides).
    let turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            session_id: session.id.clone(),
            input: vec![V2UserInput::Text {
    ...
    assert_eq!(started.session_id, session.id);
    assert_eq!(
        started.turn.status,
        savfox_app_server_protocol::TurnStatus::InProgress
    );
    ...
    assert_eq!(completed.session_id, session.id);
...
#[tokio::test]
async fn turn_start_change_personality_mid_session_v2() -> Result<()> {
    skip_if_no_network!(Ok(()));
...
async fn turn_start_accepts_local_image_input() -> Result<()> {
    // Two Savfox turns hit the mock model (session start + turn/start).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ];
    ...
    let image_path = savfox_home.path().join("image.png");
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let mut mcp = McpProcess::new(savfox_home.as_path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": savfox_core::protocol::ReviewDecision::Approved }),
    )
    .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("savfox/event/task_complete"),
    )
    .await??;
    timeout(
        DEFAULT_READ_TIMEOUT,
...
async fn turn_start_exec_approval_decline_v2() -> Result<()> {
    ...
    let started_command_execution = timeout(DEFAULT_READ_TIMEOUT, async {
        loop {
            ...
            if let SessionItem::CommandExecution { .. } = started.item {
                return Ok::<SessionItem, anyhow::Error>(started.item);
            }
        }
    })
    .await??;
    let SessionItem::CommandExecution { id, status, .. } = started_command_execution else {
    ...
    assert_eq!(params.session_id, session.id);
    ...
    let completed_command_execution = timeout(DEFAULT_READ_TIMEOUT, async {
        loop {
            ...
            if let SessionItem::CommandExecution { .. } = completed.item {
                return Ok::<SessionItem, anyhow::Error>(completed.item);
            }
        }
    })
    .await??;
    let SessionItem::CommandExecution {
        id,
        status,
        exit_code,
        aggregated_output,
...
async fn turn_start_updates_sandbox_and_cwd_between_turns_v2() -> Result<()> {
    ...
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(savfox_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(savfox_app_server_protocol::SandboxPolicy::WorkspaceWrite {
    ...
    let command_exec_item = timeout(DEFAULT_READ_TIMEOUT, async {
        loop {
            ...
            let item_started: ItemStartedNotification =
                serde_json::from_value(params).expect("deserialize item/started notification");
            if matches!(item_started.item, SessionItem::CommandExecution { .. }) {
                return Ok::<SessionItem, anyhow::Error>(item_started.item);
            }
        }
    })
...
"#;
    ...
    let started_file_change = timeout(DEFAULT_READ_TIMEOUT, async {
        loop {
            ...
            let started: ItemStartedNotification =
                serde_json::from_value(started_notif.params.clone().expect("item/started params"))?;
            if let SessionItem::FileChange { .. } = started.item {
                return Ok::<SessionItem, anyhow::Error>(started.item);
            }
        }
    })
    .await??;
    let SessionItem::FileChange {
        ref id,
        status,
        ref changes,
    } = started_file_change
    ...
    assert_eq!(output_delta.session_id, session.id);
    ...
    let completed_file_change = timeout(DEFAULT_READ_TIMEOUT, async {
        loop {
            ...
            if let SessionItem::FileChange { .. } = completed.item {
                return Ok::<SessionItem, anyhow::Error>(completed.item);
            }
        }
    })
    .await??;
    let SessionItem::FileChange { ref id, status, .. } = completed_file_change else {
        unreachable!("loop ensures we break on file change items");
    };
    assert_eq!(id, "patch-call");
...
"#;
    ...
    // First turn: expect FileChangeRequestApproval, respond with AcceptForSession, and verify the
    // file exists.
    let turn_1_req = mcp
        .send_turn_start_request(TurnStartParams {
            session_id: session.id.clone(),
    ...
    .await??;
    let SessionItem::FileChange { id, status, .. } = started_file_change_1 else {
        unreachable!("loop ensures we break on file change items");
    };
    ...
    // Second turn: apply a patch to the same file. Approval should be skipped due to
    // AcceptForSession.
    let turn_2_req = mcp
        .send_turn_start_request(TurnStartParams {
            session_id: session.id.clone(),
    ...
    .await??;
    let SessionItem::FileChange { id, status, .. } = started_file_change_2 else {
        unreachable!("loop ensures we break on file change items");
    };
