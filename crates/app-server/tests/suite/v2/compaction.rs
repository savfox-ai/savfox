...
//! 2) Act: start a session and submit multiple turns to trigger auto-compaction.
...
use app_test_support::{
    ChatGptAuthFixture, McpProcess, to_response, write_chatgpt_auth,
...
use savfox_app_server_protocol::{
    ...
    TurnStartParams, TurnStartResponse, UserInput as V2UserInput,
};
...
async fn auto_compaction_local_emits_started_and_completed_items() -> Result<()> {
    ...
    for message in ["first", "second", "third"] {
        send_turn_and_wait(&mut mcp, &session_id, message).await?;
    }

    let started = wait_for_context_compaction_started(&mut mcp).await?;
    let completed = wait_for_context_compaction_completed(&mut mcp).await?;

    let SessionItem::ContextCompaction { id: started_id } = started.item else {
        unreachable!("started item should be context compaction");
    };
    let SessionItem::ContextCompaction { id: completed_id } = completed.item else {
...
async fn start_session(mcp: &mut McpProcess) -> Result<String> {
    ...
    let session_resp: JSONRPCResponse = timeout(
        ...
        mcp.read_stream_until_response_message(RequestId::Integer(session_id)),
    )
    .await??;
...
async fn wait_for_context_compaction_started(
    ...
    loop {
        ...
        if let SessionItem::ContextCompaction { .. } = started.item {
            return Ok(started);
        }
    }
}
...
async fn wait_for_context_compaction_completed(
    ...
    loop {
        ...
        if let SessionItem::ContextCompaction { .. } = completed.item {
            return Ok(completed);
        }
    }
}
