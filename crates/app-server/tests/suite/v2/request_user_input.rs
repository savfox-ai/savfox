...
use savfox_app_server_protocol::{
    JSONRPCResponse, RequestId, ServerRequest, SessionStartParams, SessionStartResponse,
...
async fn request_user_input_round_trip() -> Result<()> {
    let savfox_home = tempfile::TempDir::new()?;
    let responses = vec![
    ...
    let session_start_id = mcp
        .send_session_start_request(SessionStartParams {
            model: Some("mock-model".to_owned()),
            ..Default::default()
        })
        .await?;
    let session_start_resp: JSONRPCResponse = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_response_message(RequestId::Integer(session_start_id)),
    )
    .await??;
    let SessionStartResponse { session, .. } = to_response(session_start_resp)?;

    let turn_start_id = mcp
        .send_turn_start_request(TurnStartParams {
            session_id: session.id.clone(),
            input: vec![V2UserInput::Text {
                text: "ask something".to_owned(),
