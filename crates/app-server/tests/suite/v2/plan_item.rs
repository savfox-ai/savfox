...
use savfox_app_server_protocol::{
    ...
    TurnCompletedNotification, TurnStartParams, TurnStartResponse, TurnStatus,
...
async fn plan_mode_uses_proposed_plan_block_for_plan_item() -> Result<()> {
    ...
    let plan_items = completed_items
        ...
        .filter_map(|item| match item {
            SessionItem::Plan { .. } => Some(item.clone()),
            _ => None,
        })
        .collect::<Vec<_>>();
    ...
    assert!(
        completed_items
            .iter()
            .any(|item| matches!(item, SessionItem::AgentMessage { .. })),
...
async fn plan_mode_without_proposed_plan_does_not_emit_plan_item() -> Result<()> {
    ...
    let has_plan_item = completed_items
        .iter()
        .any(|item| matches!(item, SessionItem::Plan { .. }));
...
async fn start_plan_mode_turn(mcp: &mut McpProcess) -> Result<savfox_app_server_protocol::Turn> {
    ...
    .await??;
    let session = to_response::<SessionStartResponse>(session_resp)?.session;
