...
use savfox_app_server_protocol::{
    ItemCompletedNotification, ItemStartedNotification, JSONRPCError, JSONRPCNotification,
    JSONRPCResponse, RequestId, ReviewDelivery, ReviewStartParams, ReviewStartResponse,
    ReviewTarget, SessionItem, SessionStartParams, SessionStartResponse, TurnStatus,
};
use serde_json::json;
...
async fn review_start_runs_review_turn_and_emits_code_review_item() -> Result<()> {
    ...
    let session_id = start_default_session(&mut mcp).await?;
    ...
    assert_eq!(review_session_id, session_id.clone());
    let turn_id = turn.id.clone();
    assert_eq!(turn.status, TurnStatus::InProgress);

    // Confirm we see the EnteredReviewMode marker on the main session.
    ...
    for _ in 0..10 {
        ...
        match started.item {
            SessionItem::EnteredReviewMode { id, review } => {
    ...
    for _ in 0..10 {
        ...
        match completed.item {
            SessionItem::ExitedReviewMode { id, review } => {
                assert_eq!(id, turn_id);
...
#[tokio::test]
async fn review_start_with_detached_delivery_returns_new_session_id() -> Result<()> {
    let review_payload = json!({
        "findings": [],
    ...
    assert_ne!(
        ...
        "detached review should run on a different session"
    );
