...
async fn init_mcp(savfox_home: &Path) -> Result<McpProcess> {
...
async fn list_sessions(
    ...
    list_sessions_with_sort(mcp, cursor, limit, providers, source_kinds, None, archived).await
}

async fn list_sessions_with_sort(
    mcp: &mut McpProcess,
    cursor: Option<String>,
    limit: Option<u32>,
    providers: Option<Vec<String>>,
    source_kinds: Option<Vec<SessionSourceKind>>,
    sort_key: Option<SessionSortKey>,
    archived: Option<bool>,
) -> Result<SessionListResponse> {
    let request_id = mcp
    ...
    .await??;
    to_response::<SessionListResponse>(resp)
}
...
fn create_minimal_config(savfox_home: &std::path::Path) -> std::io::Result<()> {
...
#[tokio::test]
async fn session_list_pagination_next_cursor_none_on_last_page() -> Result<()> {
    let savfox_home = TempDir::new()?;
    create_minimal_config(savfox_home.path())?;
...
#[tokio::test]
async fn session_list_respects_provider_filter() -> Result<()> {
    let savfox_home = TempDir::new()?;
    create_minimal_config(savfox_home.path())?;
    ...
    assert_eq!(next_cursor, None);
    let session = &data[0];
    assert_eq!(session.preview, "X");
    assert_eq!(session.model_provider, "other_provider");
    let expected_ts = chrono::DateTime::parse_from_rfc3339("2025-01-02T11:00:00Z")?.timestamp();
    assert_eq!(session.created_at, expected_ts);
    assert_eq!(session.updated_at, expected_ts);
    assert_eq!(session.cwd, PathBuf::from("/"));
    assert_eq!(session.cli_version, "0.0.0");
    assert_eq!(session.source, SessionSource::Cli);
    assert_eq!(session.git_info, None);
...
#[tokio::test]
async fn session_list_empty_source_kinds_defaults_to_interactive_only() -> Result<()> {
    let savfox_home = TempDir::new()?;
    create_minimal_config(savfox_home.path())?;
    ...
    assert_eq!(next_cursor, None);
    let ids: Vec<_> = data.iter().map(|session| session.id.as_str()).collect();
...
#[tokio::test]
async fn session_list_filters_by_source_kind_subagent_session_spawn() -> Result<()> {
    let savfox_home = TempDir::new()?;
    create_minimal_config(savfox_home.path())?;
    ...
    let parent_session_id = SessionId::from_string(&Uuid::new_v4().to_string())?;
    let subagent_id = create_fake_rollout_with_source(
        ...
        Some("mock_provider"),
        None,
        CoreSessionSource::SubAgent(SubAgentSource::SessionSpawn {
            parent_session_id,
            depth: 1,
        }),
    )?;
...
#[tokio::test]
async fn session_list_filters_by_subagent_variant() -> Result<()> {
    let savfox_home = TempDir::new()?;
    create_minimal_config(savfox_home.path())?;
...
#[tokio::test]
async fn session_list_fetches_until_limit_or_exhausted() -> Result<()> {
    let savfox_home = TempDir::new()?;
    create_minimal_config(savfox_home.path())?;
    ...
    // Request 8 sessions for the target provider; the matches only start on the
    ...
    assert!(
        data.iter()
            .all(|session| session.model_provider == "target_provider"),
        "all returned sessions must match the requested provider"
    );
    assert_eq!(
        next_cursor, None,
...
#[tokio::test]
async fn session_list_enforces_max_limit() -> Result<()> {
    let savfox_home = TempDir::new()?;
    create_minimal_config(savfox_home.path())?;
...
#[tokio::test]
async fn session_list_stops_when_not_enough_filtered_results_exist() -> Result<()> {
    let savfox_home = TempDir::new()?;
    create_minimal_config(savfox_home.path())?;
    ...
    // Request more sessions than exist after filtering; expect all matches to be
    ...
    assert_eq!(
        data.len(),
        7,
        "all available filtered sessions should be returned"
    );
    assert!(
        data.iter()
            .all(|session| session.model_provider == "target_provider"),
...
#[tokio::test]
async fn session_list_includes_git_info() -> Result<()> {
    let savfox_home = TempDir::new()?;
    create_minimal_config(savfox_home.path())?;
    ...
    let SessionListResponse { data, .. } = list_sessions(
        &mut mcp,
        None,
        Some(10),
        Some(vec!["mock_provider".to_string()]),
...
#[tokio::test]
async fn session_list_default_sorts_by_created_at() -> Result<()> {
    let savfox_home = TempDir::new()?;
    create_minimal_config(savfox_home.path())?;
    ...
    let SessionListResponse { data, .. } = list_sessions_with_sort(
        &mut mcp,
        None,
        Some(10),
        Some(vec!["mock_provider".to_string()]),
...
#[tokio::test]
async fn session_list_sort_updated_at_orders_by_mtime() -> Result<()> {
    let savfox_home = TempDir::new()?;
    create_minimal_config(savfox_home.path())?;
    ...
    set_rollout_mtime(
        rollout_path(savfox_home.path(), "2025-01-01T10-00-00", &id_old).as_path(),
        "2025-01-03T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(savfox_home.path(), "2025-01-01T11-00-00", &id_mid).as_path(),
        "2025-01-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(savfox_home.path(), "2025-01-01T12-00-00", &id_new).as_path(),
...
#[tokio::test]
async fn session_list_updated_at_paginates_with_cursor() -> Result<()> {
    let savfox_home = TempDir::new()?;
    create_minimal_config(savfox_home.path())?;
    ...
    set_rollout_mtime(
        rollout_path(savfox_home.path(), "2025-02-01T10-00-00", &id_a).as_path(),
        "2025-02-03T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(savfox_home.path(), "2025-02-01T11-00-00", &id_b).as_path(),
        "2025-02-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(savfox_home.path(), "2025-02-01T12-00-00", &id_c).as_path(),
    ...
    .await?;
    let ids_page1: Vec<_> = page1.iter().map(|session| session.id.as_str()).collect();
    ...
    .await?;
    let ids_page2: Vec<_> = page2.iter().map(|session| session.id.as_str()).collect();
...
#[tokio::test]
async fn session_list_created_at_tie_breaks_by_uuid() -> Result<()> {
    let savfox_home = TempDir::new()?;
    create_minimal_config(savfox_home.path())?;
...
#[tokio::test]
async fn session_list_updated_at_tie_breaks_by_uuid() -> Result<()> {
    let savfox_home = TempDir::new()?;
    create_minimal_config(savfox_home.path())?;
...
#[tokio::test]
async fn session_list_updated_at_uses_mtime() -> Result<()> {
    let savfox_home = TempDir::new()?;
    create_minimal_config(savfox_home.path())?;
    ...
    set_rollout_mtime(
        rollout_path(savfox_home.path(), "2025-02-01T10-00-00", &session_id).as_path(),
...
#[tokio::test]
async fn session_list_archived_filter() -> Result<()> {
    let savfox_home = TempDir::new()?;
    create_minimal_config(savfox_home.path())?;
    ...
    let archived_dir = savfox_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
    fs::create_dir_all(&archived_dir)?;
    let archived_source = rollout_path(savfox_home.path(), "2025-03-01T09-00-00", &archived_id);
...
#[tokio::test]
async fn session_list_invalid_cursor_returns_error() -> Result<()> {
    let savfox_home = TempDir::new()?;
    create_minimal_config(savfox_home.path())?;
