...
async fn test_send_message_success() -> Result<()> {
    // Spin up a mock responses server that immediately ends the Savfox turn.
    // Two Savfox turns hit the mock model (session start + send-user-message). Provide two SSE
    // responses.
    ...
    // Create a temporary Savfox home with config pointing at the mock server.
...
async fn send_message(
    ...
    let raw_attempt = tokio::time::timeout(
        ...
        mcp.read_stream_until_notification_message("savfox/event/raw_response_item"),
    )
    .await;
    assert!(
        raw_attempt.is_err(),
...
async fn test_send_message_session_not_found() -> Result<()> {
    // Start MCP without creating a Savfox session
    let savfox_home = TempDir::new()?;
    let mut mcp = McpProcess::new(savfox_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
#[expect(clippy::expect_used)]
async fn read_raw_response_item(mcp: &mut McpProcess, conversation_id: SessionId) -> ResponseItem {
    // TODO: Switch to rawResponseItem/completed once we migrate to app server v2 in savfox web.
    loop {
        ...
        .await
        .expect("savfox/event/raw_response_item notification timeout")
        .expect("savfox/event/raw_response_item notification resp");

        let serde_json::Value::Object(params) = raw_notification
            .params
            .expect("savfox/event/raw_response_item should have params")
        else {
            panic!("savfox/event/raw_response_item should have params");
        };
