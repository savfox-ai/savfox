...
use savfox_app_server_protocol::{
    ...
    ServerNotification, SessionConfiguredNotification,
};
...
async fn test_list_and_resume_conversations() -> Result<()> {
    // Prepare a temporary SAVFOX_HOME with a few fake rollout files.
    ...
    // Expect a savfox/event notification with msg.type == sessionConfigured
