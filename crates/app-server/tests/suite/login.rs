...
use savfox_app_server_protocol::{
    GetAuthStatusParams, GetAuthStatusResponse, JSONRPCError, JSONRPCResponse,
    LoginChatGptResponse, LogoutChatGptResponse, RequestId,
};
...
fn create_config_toml_forced_login(savfox_home: &Path, forced_method: &str) -> std::io::Result<()> {
    let config_toml = savfox_home.join("config.toml");
    let contents = format!(
        r#"
model = "mock-model"
...
async fn login_chatgpt_includes_forced_workspace_query_param() -> Result<()> {
    let savfox_home = TempDir::new()?;
    create_config_toml_forced_workspace(savfox_home.path(), "ws-forced")?;
