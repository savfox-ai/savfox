...
use savfox_core::protocol::TurnAbortReason;
...
async fn shell_command_interruption() -> anyhow::Result<()> {
    ...
    // (MSYS/GNU coreutils may be absent) and the failure causes the tool call to finish
    // immediately, which triggers a second model request before the test sends the explicit
    // follow-up. That prematurely consumes the second mocked SSE response and leads to a third
    // POST (panic: no response for 2). Powershell Start-Sleep is always available on Windows
    // runners. On Unix we keep using `sleep`.
...
fn create_config_toml(savfox_home: &Path, server_uri: String) -> std::io::Result<()> {
