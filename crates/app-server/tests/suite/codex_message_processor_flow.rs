...
use savfox_app_server_protocol::ExecCommandApprovalParams;
use savfox_app_server_protocol::InputItem;
use savfox_app_server_protocol::JSONRPCNotification;
...
use savfox_app_server_protocol::ServerRequest;
...
async fn test_savfox_jsonrpc_conversation_flow() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    // Temporary Savfox home with config pointing at the mock server.
    ...
    let send_user_id = mcp
        .send_send_user_message_request(SendUserMessageParams {
            conversation_id,
            items: vec![savfox_app_server_protocol::InputItem::Text {
                text: "text".to_string(),
    ...
    let task_started_notification: JSONRPCNotification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("savfox/event/task_started"),
    )
    .await??;
    ...
    let remove_listener_id = mcp
        .send_remove_session_listener_request(RemoveConversationListenerParams { subscription_id })
        .await?;
...
async fn test_send_user_turn_updates_sandbox_and_cwd_between_turns() -> Result<()> {
    ...
    let exec_begin_notification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("savfox/event/exec_command_begin"),
    )
    .await??;
