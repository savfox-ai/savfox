...
`savfox app-server` is the interface Savfox uses to power rich interfaces such as the [Savfox VS Code extension](https://marketplace.visualstudio.com/items?itemName=openai.chatgpt).
...
Similar to [MCP](https://modelcontextprotocol.io/), `savfox app-server` supports bidirectional communication, streaming JSONL over stdio. The protocol is JSON-RPC 2.0, though the `"jsonrpc":"2.0"` header is omitted.

## Message Schema

Currently, you can dump a TypeScript version of the schema using `savfox app-server generate-ts`, or a JSON Schema bundle via `savfox app-server generate-json-schema`. Each output is specific to the version of Savfox you used to run the command, so the generated artifacts are guaranteed to match that version.

```
savfox app-server generate-ts --out DIR
savfox app-server generate-json-schema --out DIR
```

## Core Primitives

The API exposes three top level primitives representing an interaction between a user and Savfox:

- **Thread**: A conversation between a user and the Savfox agent. Each thread contains multiple turns.
...
- Initialize once: Immediately after launching the savfox app-server process, send an `initialize` request with your client metadata, then emit an `initialized` notification. Any other request before this handshake gets rejected.
...
Applications building on top of `savfox app-server` should identify themselves via the `clientInfo` parameter.
...
added to a known clients list. For more context: https://savfox.ai/admin/api-reference#tag/Logs:-Savfox
...
```json
{
  ...
  "params": {
    "clientInfo": {
      "name": "savfox_vscode",
      "title": "Savfox VS Code Extension",
      "version": "0.1.0"
    }
  }
}
...
- `session/start` — create a new session; emits `thread/started` and auto-subscribes you to turn/item events for that thread.
- `session/resume` — reopen an existing thread by id so subsequent `turn/start` calls append to it.
- `session/fork` — fork an existing thread into a new thread id by copying the stored history; emits `thread/started` and auto-subscribes you to turn/item events for the new thread.
- `session/list` — page through stored rollouts; supports cursor-based pagination and optional `modelProviders` filtering.
- `session/loaded/list` — list the thread ids currently loaded in memory.
- `session/read` — read a stored thread by id without resuming it; optionally include turns via `includeTurns`.
- `session/archive` — move a thread’s rollout file into the archived directory; returns `{}` on success.
- `session/name/set` — set or update a thread’s user-facing name; returns `{}` on success. Thread names are not required to be unique; name lookups resolve to the most recently updated thread.
- `session/unarchive` — move an archived rollout file back into the sessions directory; returns the restored `thread` on success.
- `session/rollback` — drop the last N turns from the agent’s in-memory context and persist a rollback marker in the rollout so future resumes see the pruned history; returns the updated `thread` (with `turns` populated) on success.
- `turn/start` — add user input to a thread and begin Savfox generation; responds with the initial `turn` object and streams `turn/started`, `item/*`, and `turn/completed` notifications.
- `turn/interrupt` — request cancellation of an in-flight turn by `(thread_id, turn_id)`; success is an empty `{}` response and the turn finishes with `status: "interrupted"`.
- `review/start` — kick off Savfox’s automated reviewer for a thread; responds like `turn/start` and emits `item/started`/`item/completed` notifications with `enteredReviewMode` and `exitedReviewMode` items, plus a final assistant `agentMessage` containing the review.
...
Start a fresh thread when you need a new Savfox conversation.
...
Turns attach user input (text or images) to a thread and trigger Savfox generation. The `input` field is a list of discriminated unions:
...
{ "method": "turn/start", "id": 33, "params": {
    ...
    "input": [
        ...
        { "type": "skill", "name": "skill-creator", "path": "/Users/me/.savfox/skills/skill-creator/SKILL.md" }
    ]
} }
{ "id": 33, "result": { "turn": {
    "id": "turn_457",
...
The server requests cancellations for running subprocesses, then emits a `turn/completed` event with `status: "interrupted"`. Rely on the `turn/completed` to know when Savfox-side cleanup is done.

### Example: Request a code review

Use `review/start` to run Savfox’s reviewer on the currently checked-out project. The request takes the thread id plus a `target` describing what should be reviewed:

- `{"type":"uncommittedChanges"}` — staged, unstaged, and untracked files.
- `{"type":"baseBranch","branch":"main"}` — diff against the provided branch’s upstream (see prompt for the exact `git merge-base`/`git diff` instructions Savfox will run).
...
Savfox streams the usual `turn/started` notification followed by an `item/started`
...
- For clients that are already sandboxed externally, set `sandboxPolicy` to `{"type":"externalSandbox","networkAccess":"enabled"}` (or omit `networkAccess` to keep it restricted). Savfox will not enforce its own sandbox in this mode; it tells the model it has full file-system access and passes the `networkAccess` state through `environment_context`.
...
- `turn/completed` — `{ turn }` where `turn.status` is `completed`, `interrupted`, or `failed`; failures carry `{ error: { message, savfoxErrorInfo?, additionalDetails? } }`.
...
- `contextCompaction` — `{id}` emitted when savfox compacts the conversation history. This can happen automatically.
- `compacted` - `{threadId, turnId}` when savfox compacts the conversation history. This can happen automatically. **Deprecated:** Use `contextCompaction` instead.
...
`error` event is emitted whenever the server hits an error mid-turn (for example, upstream model errors or quota limits). Carries the same `{ error: { message, savfoxErrorInfo?, additionalDetails? } }` payload as `turn.status: "failed"` and may precede that terminal notification.

`savfoxErrorInfo` maps to the `SavfoxErrorInfo` enum. Common values:
...
When an upstream HTTP status is available (for example, from the Responses API or a provider), it is forwarded in `httpStatusCode` on the relevant `savfoxErrorInfo` variant.

## Approvals

Certain actions (shell commands or modifying files) may require explicit user approval depending on the user's config. When `turn/start` is used, the app-server drives an approval flow by sending a server-initiated JSON-RPC request to the client. The client must respond to tell Savfox whether to proceed. UIs should present these requests inline with the active turn so users can review the proposed command or diff before choosing.
  ...
  "params": {
    ...
    "input": [
      ...
      },
      {
        "type": "skill",
        "name": "skill-creator",
        "path": "/Users/me/.savfox/skills/skill-creator/SKILL.md"
      }
    ]
  }
}
...
{ "id": 25, "result": {
    "data": [{
        ...
        "skills": [
            {
              "name": "skill-creator",
              "description": "Create or update a Savfox skill",
              "enabled": true,
              "interface": {
                "displayName": "Skill Creator",
                "shortDescription": "Create or update a Savfox skill",
                "iconSmall": "icon.svg",
    ...
    "data": [
        {
            ...
            "installUrl": "https://savfox.ai/apps/demo-app/demo-app",
            "isAccessible": true
        }
    ],
    "nextCursor": null
...
Savfox supports these authentication modes. The current mode is surfaced in `account/updated` (`authMode`) and can be inferred from `account/read`.

- **API key (`apiKey`)**: Caller supplies an OpenAI API key via `account/login/start` with `type: "apiKey"`. The API key is saved and used for API requests.
- **ChatGPT managed (`chatgpt`)** (recommended): Savfox owns the ChatGPT OAuth flow and refresh tokens. Start via `account/login/start` with `type: "chatgpt"`; Savfox persists tokens to disk and refreshes them automatically.
...
- `requiresOpenaiAuth` reflects the active provider; when `false`, Savfox can run without OpenAI credentials.
...
1. Start:
   ...
   { "id": 3, "result": { "type": "chatgpt", "loginId": "<uuid>", "authUrl": "https://savfox.ai/…&redirect_uri=http%3A%2F%2Flocalhost%3A<port>%2Fauth%2Fcallback" } }
   ```
...
4. Export protocol schema to a temporary directory for inspection when needed:

   ```bash
   savfox app-server generate-ts --out .tmp/app-server-schema/ts --experimental
   savfox app-server generate-json-schema --out .tmp/app-server-schema/json --experimental
   ```
