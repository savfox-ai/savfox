...
use savfox_core::auth::{AuthCredentialsStoreMode, load_auth_dot_json};
use savfox_login_oauth::{ServerOptions, run_device_code_login};
use serde_json::json;
...
async fn device_code_login_integration_succeeds() -> anyhow::Result<()> {
    ...
    let opts = server_opts(&savfox_home, issuer, AuthCredentialsStoreMode::File);
...
async fn device_code_login_integration_handles_error_payload() -> anyhow::Result<()> {
    ...
    // Accept either the specific error payload, a 400, or a 404 (since the client may return 404 if
