...
use savfox_login_oauth::{ServerOptions, run_login_server};
...
#[tokio::test]
async fn creates_missing_savfox_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_savfox_home = first_tmp.path().to_path_buf();
    ...
    let second_savfox_home = second_tmp.path().to_path_buf();
