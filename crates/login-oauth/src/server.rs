...
pub struct ServerOptions {
    ...
    /// Display name of the provider shown on the success page (e.g. "OpenAI").
...
impl LoginServer {
    pub async fn block_until_done(self) -> io::Result<()> {
        self.server_handle
            .await
            .map_err(|err| io::Error::other(format!("login server session panicked: {err:?}")))?
    }
...
pub fn run_login_server(opts: ServerOptions) -> io::Result<LoginServer> {
    ...
    let server_handle = {
        ...
        tokio::spawn(async move {
            ...
            // Ensure that the server is unblocked so the session dedicated to
        ...
        "/auth/callback" => {
                ...
                .await
            {
                Ok(tokens) => {
                    ...
                    let success_url = compose_success_url(actual_port, &opts.provider_name);
...
pub(crate) async fn exchange_code_for_tokens(
    ...
    let client = savfox_core::default_client::try_build_reqwest_client()?;
    let resp = client
...
pub(crate) async fn persist_tokens_async(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        save_auth(&savfox_home, &auth, auth_credentials_store_mode)
    })
    .await
