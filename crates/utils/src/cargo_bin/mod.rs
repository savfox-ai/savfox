...
/// Bazel sets this when runfiles directories are disabled, which we do on all platforms for
/// consistency.
...
/// In `bazel test`, `CARGO_BIN_EXE_*` env vars are rlocationpaths, intended to be consumed by
/// `rlocation`. This helper allows callers to transparently support both.
#[allow(deprecated)]
...
/// site.)
///
/// This is expected to be used exclusively in test code because Savfox CLI is a
...
macro_rules! find_resource {
    ($resource:expr) => {{
        ...
        if $crate::runfiles_available() {
            ...
            // - we inject `BAZEL_PACKAGE` as a compile-time environment variable that points to
            //   native.package_name()
...
pub fn repo_root() -> io::Result<PathBuf> {
    let marker = if runfiles_available() {
        ...
        let marker_path = option_env!("SAVFOX_REPO_ROOT_MARKER")
            .map(PathBuf::from)
            .ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::NotFound,
                    "SAVFOX_REPO_ROOT_MARKER was not set at compile time",
                )
            })?;
        ...
        resolve_cargo_runfile(Path::new("cargo-bin/repo_root.marker"))?
    };
    let mut root = marker;
    for _ in 0..4 {
        root = root
