...
use super::{GhostCommit, GitToolingError};

/// Default commit message used for ghost commits when none is provided.
const DEFAULT_COMMIT_MESSAGE: &str = "savfox snapshot";
...
/// Create a ghost commit capturing the current state of the repository's working tree along with a
/// report.
pub fn create_ghost_commit_with_report(
    ...
    let large_untracked_dirs = existing_untracked
        ...
        .collect::<Vec<_>>();
    let index_tempdir = Builder::new().prefix("savfox-utils-git-index-").tempdir()?;
    ...
    //   GIT_INDEX_FILE=/tmp/index git commit-tree <tree> -p <parent> -m "savfox snapshot"
...
/// Captures the untracked and ignored entries under `repo_root`, optionally limited by
/// `repo_prefix`. Returns the result as an `UntrackedSnapshot`.
...
fn default_commit_identity() -> Vec<(OsString, OsString)> {
    vec![
        ...
        (
            ...
            OsString::from("snapshot@savfox.local"),
        ),
        (
            OsString::from("GIT_COMMITTER_NAME"),
            OsString::from("Savfox Snapshot"),
        ),
        (
...
mod tests {
    ...
    use super::super::operations::run_git_for_stdout;
    use super::*;
    ...
    fn restore_from_subdirectory_restores_files_relatively() -> Result<(), GitToolingError> {
        ...
        assert!(!workspace.join("savfox-rs").exists());
    ...
    fn restore_from_subdirectory_preserves_parent_vscode() -> Result<(), GitToolingError> {
        ...
        let workspace = repo.join("savfox-rs");
