...
Typed clients for Savfox/OpenAI APIs built on top of the generic transport in `savfox-client`.
...
- **Prompted endpoints (Chat + Responses)**
  - Input: a single `Prompt` plus endpoint-specific options.
    - `Prompt` (re-exported as `savfox_api::Prompt`) carries:
...
- **Compaction endpoint**
  - Input: `CompactionInput<'a>` (re-exported as `savfox_api::CompactionInput`):
    - `model: &str`.
...
All HTTP details (URLs, headers, retry/backoff policies, SSE framing) are encapsulated in `savfox-api` and `savfox-client`. Callers construct prompts/inputs using protocol types and work with typed streams of `ResponseEvent` or compacted `ResponseItem` values.
