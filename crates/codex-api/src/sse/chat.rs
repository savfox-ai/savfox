...
use savfox_client::StreamResponse;
...
pub async fn process_chat_sse<S>(
    ...
    S: Stream<Item = Result<bytes::Bytes, savfox_client::TransportError>> + Unpin,
{
    ...
    loop {
        ...
        // eprintln!("[SSE RESPONSE] {}", sse.data);
...
mod tests {
    ...
    #[tokio::test]
    async fn coalesces_chunked_text_and_reasoning_into_single_entries() {
        let delta_one = json!({
            "choices": [{
                "delta": {
                    "reasoning": "because ",
        ...
        let body = build_body(&[delta_one, delta_two, finish]);
        let events = collect_events(&body).await;

        let mut assistant_done_text: Option<String> = None;
        let mut reasoning_done_text: Option<String> = None;
            ...
            match ev {
                ResponseEvent::OutputItemDone(ResponseItem::Message { content, .. }) => {
                    ...
                    if let Some(ContentItem::OutputText { text }) = content.first() {
                        assistant_done_text = Some(text.clone());
                    }
                }
                ResponseEvent::OutputItemDone(ResponseItem::Reasoning {
                    content: Some(content),
                    ...
                    assert_eq!(content.len(), 1);
                    if let Some(ReasoningItemContent::ReasoningText { text }) = content.first() {
                        reasoning_done_text = Some(text.clone());
                    }
                }
                _ => {}
            }
        ...
        assert_eq!(assistant_done_text.as_deref(), Some("Hello world"));
        assert_eq!(reasoning_done_text.as_deref(), Some("because it works"));
    }
}
