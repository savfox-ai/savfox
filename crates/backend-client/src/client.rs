...
use crate::types::{
    CodeTaskDetailsResponse, ConfigFileResponse, CreditStatusDetails, PaginatedListTaskListItem,
    RateLimitStatusPayload, RateLimitWindowSnapshot, TurnAttemptsSiblingTurnsResponse,
};
...
impl Client {
    pub fn new(base_url: impl Into<String>) -> Result<Self> {
        ...
        if (base_url.starts_with("https://taidge.com")
    ...
    pub fn from_auth(base_url: impl Into<String>, auth: &SavfoxAuth) -> Result<Self> {
        let token = auth.get_token().map_err(anyhow::Error::from)?;
        let mut client = Self::new(base_url)?
            .with_user_agent(get_savfox_user_agent())
    ...
    fn headers(&self) -> HeaderMap {
        ...
        if let Some(ua) = &self.user_agent {
            ...
            h.insert(USER_AGENT, HeaderValue::from_static("savfox-cli"));
        }
    ...
    pub async fn get_rate_limits(&self) -> Result<RateLimitSnapshot> {
        let url = match self.path_style {
            PathStyle::SavfoxApi => format!("{}/api/savfox/usage", self.base_url),
    ...
    pub async fn list_tasks(
        ...
        let url = match self.path_style {
            PathStyle::SavfoxApi => format!("{}/api/savfox/tasks/list", self.base_url),
    ...
    pub async fn get_task_details_with_body(
        ...
        let url = match self.path_style {
            PathStyle::SavfoxApi => format!("{}/api/savfox/tasks/{}", self.base_url, task_id),
    ...
    pub async fn list_sibling_turns(
        ...
        let url = match self.path_style {
            PathStyle::SavfoxApi => format!(
                "{}/api/savfox/tasks/{}/turns/{}/sibling_turns",
    ...
    /// Fetch the managed requirements file from savfox-backend.
    ///
    /// `GET /api/savfox/config/requirements` (Savfox API style) or
    ...
    pub async fn get_config_requirements_file(&self) -> Result<ConfigFileResponse> {
        let url = match self.path_style {
            PathStyle::SavfoxApi => format!("{}/api/savfox/config/requirements", self.base_url),
    ...
    pub async fn create_task(&self, request_body: serde_json::Value) -> Result<String> {
        let url = match self.path_style {
            PathStyle::SavfoxApi => format!("{}/api/savfox/tasks", self.base_url),
