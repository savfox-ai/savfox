...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> savfox_core::exec::ExecToolCallOutput {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_savfox-linux-sandbox");
    let savfox_linux_sandbox_exe = Some(PathBuf::from(sandbox_program));
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_savfox-linux-sandbox");
    let savfox_linux_sandbox_exe: Option<PathBuf> = Some(PathBuf::from(sandbox_program));
    ...
    let output = match result {
        Ok(output) => output,
        Err(SavfoxError::Sandbox(SandboxErr::Denied { output })) => *output,
        _ => {
