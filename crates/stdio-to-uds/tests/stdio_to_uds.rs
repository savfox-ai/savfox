...
fn pipes_stdin_and_stdout_through_socket() -> anyhow::Result<()> {
    ...
    let server_session = thread::spawn(move || -> anyhow::Result<()> {
        ...
        tx.send(received)
            .map_err(|_| anyhow::anyhow!("failed to send received bytes to test session"))?;
        connection
    ...
    Command::new(savfox_utils::cargo_bin::cargo_bin("savfox-stdio-to-uds")?)
        .arg(&socket_path)
        .write_stdin("request")
        .assert()
        .success()
    ...
    let server_result = server_session
        .join()
        .map_err(|_| anyhow::anyhow!("server session panicked"))?;
