...
use savfox_core::protocol::DeprecationNoticeEvent;
use savfox_core::{
    LMSTUDIO_OSS_PROVIDER_ID, OLLAMA_CHAT_PROVIDER_ID, OLLAMA_OSS_PROVIDER_ID, WireApi,
};
...
pub fn get_default_model_for_oss_provider(provider_id: &str) -> Option<&'static str> {
    match provider_id {
        ...
        OLLAMA_OSS_PROVIDER_ID | OLLAMA_CHAT_PROVIDER_ID => Some(savfox_ollama::DEFAULT_OSS_MODEL),
        _ => None,
    }
}
...
pub async fn ensure_oss_provider_ready(
    ...
    match provider_id {
        LMSTUDIO_OSS_PROVIDER_ID => {
            savfox_lmstudio::ensure_oss_ready(config)
                .await
                .map_err(|e| std::io::Error::other(format!("OSS setup failed: {e}")))?;
        }
        OLLAMA_OSS_PROVIDER_ID | OLLAMA_CHAT_PROVIDER_ID => {
            savfox_ollama::ensure_oss_ready(config)
                .await
                .map_err(|e| std::io::Error::other(format!("OSS setup failed: {e}")))?;
        }
        _ => {
